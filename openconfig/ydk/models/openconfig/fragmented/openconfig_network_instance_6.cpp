...
{
    std::ostringstream path_buffer;
    path_buffer << "prefix" <<"[prefix='" <<prefix <<"']" <<"[mt-id='" <<mt_id <<"']";
    return path_buffer.str();
}
...
{
    for (std::size_t index=0; index<undefined_tlv.size(); index++)
    {
...
std::map<std::string, std::shared_ptr<Entity>> NetworkInstances::NetworkInstance::Protocols::Protocol::Isis::Levels::Level::LinkStateDatabase::Lsp::UndefinedTlvs::get_children() const
{
    ...
    char count=0;
    count = 0;
    for (auto const & c : undefined_tlv)
    {
        if(children.find(c->get_segment_path()) == children.end())
            children[c->get_segment_path()] = c;
        else
...
{
    std::ostringstream path_buffer;
    path_buffer << "undefined-tlv" <<"[type='" <<type <<"']";
    return path_buffer.str();
}
...
{
    std::ostringstream path_buffer;
    path_buffer << "interface" <<"[interface-id='" <<interface_id <<"']";
    return path_buffer.str();
}
...
{
    std::ostringstream path_buffer;
    path_buffer << "adjacency" <<"[system-id='" <<system_id <<"']";
    return path_buffer.str();
}
...
{
    std::ostringstream path_buffer;
    path_buffer << "prefix-sid" <<"[prefix='" <<prefix <<"']";
    return path_buffer.str();
}
...
{
    std::ostringstream path_buffer;
    path_buffer << "adjacency-sid" <<"[neighbor='" <<neighbor <<"']" <<"[sid-id='" <<sid_id <<"']";
    return path_buffer.str();
}
