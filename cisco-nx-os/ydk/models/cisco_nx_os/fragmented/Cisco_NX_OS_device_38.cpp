...
System::RpmItems::RtmapItems::RuleList::EntItems::EntryList::SevpnItems::SevpnItems()
    :
    ip{YType::str, "ip"},
    name{YType::str, "name"},
    descr{YType::str, "descr"},
    type{YType::enumeration, "type"}
{
    ...
    yang_name = "sevpn-items"; yang_parent_name = "Entry-list"; is_top_level_class = false; has_list_ancestor = true; 
}
...
bool System::RpmItems::RtmapItems::RuleList::EntItems::EntryList::SevpnItems::has_data() const
{
    if (is_presence_container) return true;
    return ip.is_set
	|| name.is_set
...
bool System::RpmItems::RtmapItems::RuleList::EntItems::EntryList::SevpnItems::has_operation() const
{
    return is_set(yfilter)
...
std::string System::RpmItems::RtmapItems::RuleList::EntItems::EntryList::SevpnItems::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "sevpn-items";
    return path_buffer.str();
}

std::vector<std::pair<std::string, LeafData> > System::RpmItems::RtmapItems::RuleList::EntItems::EntryList::SevpnItems::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};
...
std::shared_ptr<ydk::Entity> System::RpmItems::RtmapItems::RuleList::EntItems::EntryList::SevpnItems::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> System::RpmItems::RtmapItems::RuleList::EntItems::EntryList::SevpnItems::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
void System::RpmItems::RtmapItems::RuleList::EntItems::EntryList::SevpnItems::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
    if(value_path == "ip")
    {
        ip = value;
...
void System::RpmItems::RtmapItems::RuleList::EntItems::EntryList::SevpnItems::set_filter(const std::string & value_path, YFilter yfilter)
{
    if(value_path == "ip")
    {
        ip.yfilter = yfilter;
...
bool System::RpmItems::RtmapItems::RuleList::EntItems::EntryList::SevpnItems::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "ip" || name == "name" || name == "descr" || name == "type")
        return true;
    return false;
}
...
System::IpSrcIfItems::IpSrcIfItems()
    ...
    , ftp_items(std::make_shared<System::IpSrcIfItems::FtpItems>())
    , telnet_items(std::make_shared<System::IpSrcIfItems::TelnetItems>())
    , ssh_items(std::make_shared<System::IpSrcIfItems::SshItems>())
    , ping_items(std::make_shared<System::IpSrcIfItems::PingItems>())
    , traceroute_items(std::make_shared<System::IpSrcIfItems::TracerouteItems>())
{
    dns_items->parent = this;
    tftp_items->parent = this;
    ftp_items->parent = this;
    telnet_items->parent = this;
    ssh_items->parent = this;
    ping_items->parent = this;
    traceroute_items->parent = this;
...
bool System::IpSrcIfItems::has_data() const
{
	...
	|| (ftp_items !=  nullptr && ftp_items->has_data())
	|| (telnet_items !=  nullptr && telnet_items->has_data())
	|| (ssh_items !=  nullptr && ssh_items->has_data())
	|| (ping_items !=  nullptr && ping_items->has_data())
	|| (traceroute_items !=  nullptr && traceroute_items->has_data());
}
...
bool System::IpSrcIfItems::has_operation() const
{
	...
	|| (ftp_items !=  nullptr && ftp_items->has_operation())
	|| (telnet_items !=  nullptr && telnet_items->has_operation())
	|| (ssh_items !=  nullptr && ssh_items->has_operation())
	|| (ping_items !=  nullptr && ping_items->has_operation())
	|| (traceroute_items !=  nullptr && traceroute_items->has_operation());
}
...
std::shared_ptr<ydk::Entity> System::IpSrcIfItems::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    ...
    if(child_yang_name == "ftp-items")
    {
        if(ftp_items == nullptr)
        {
            ftp_items = std::make_shared<System::IpSrcIfItems::FtpItems>();
        }
        return ftp_items;
    ...
    if(child_yang_name == "ping-items")
    {
        if(ping_items == nullptr)
        {
            ping_items = std::make_shared<System::IpSrcIfItems::PingItems>();
        }
        return ping_items;
    ...
    if(child_yang_name == "traceroute-items")
    {
        if(traceroute_items == nullptr)
        {
            traceroute_items = std::make_shared<System::IpSrcIfItems::TracerouteItems>();
        }
        return traceroute_items;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> System::IpSrcIfItems::get_children() const
{
    ...
    if(ping_items != nullptr)
    {
        _children["ping-items"] = ping_items;
    }

    if(traceroute_items != nullptr)
    {
        _children["traceroute-items"] = traceroute_items;
    }
...
{
    if(name == "dns-items" || name == "tftp-items" || name == "ftp-items" || name == "telnet-items" || name == "ssh-items" || name == "ping-items" || name == "traceroute-items")
        return true;
    return false;
}
...
System::IpSrcIfItems::FtpItems::FtpItems()
    :
    ftp_list(this, {"vrf"})
{

    yang_name = "ftp-items"; yang_parent_name = "ipSrcIf-items"; is_top_level_class = false; has_list_ancestor = false; 
}
...
bool System::IpSrcIfItems::FtpItems::has_data() const
{
    if (is_presence_container) return true;
    for (std::size_t index=0; index<ftp_list.len(); index++)
    {
        if(ftp_list[index]->has_data())
            return true;
    }
    return false;
}

bool System::IpSrcIfItems::FtpItems::has_operation() const
{
    for (std::size_t index=0; index<ftp_list.len(); index++)
    {
        if(ftp_list[index]->has_operation())
            return true;
    }
    return is_set(yfilter);
}

std::string System::IpSrcIfItems::FtpItems::get_absolute_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "Cisco-NX-OS-device:System/ipSrcIf-items/" << get_segment_path();
    return path_buffer.str();
...
std::string System::IpSrcIfItems::FtpItems::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "ftp-items";
    return path_buffer.str();
}

std::vector<std::pair<std::string, LeafData> > System::IpSrcIfItems::FtpItems::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};
...
std::shared_ptr<ydk::Entity> System::IpSrcIfItems::FtpItems::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "Ftp-list")
    {
        auto ent_ = std::make_shared<System::IpSrcIfItems::FtpItems::FtpList>();
        ent_->parent = this;
        ftp_list.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> System::IpSrcIfItems::FtpItems::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : ftp_list.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
void System::IpSrcIfItems::FtpItems::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
}

void System::IpSrcIfItems::FtpItems::set_filter(const std::string & value_path, YFilter yfilter)
{
}

bool System::IpSrcIfItems::FtpItems::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "Ftp-list")
        return true;
    return false;
...
System::IpSrcIfItems::FtpItems::FtpList::FtpList()
    :
    vrf{YType::str, "vrf"},
    srcif{YType::str, "srcIf"}
{

    yang_name = "Ftp-list"; yang_parent_name = "ftp-items"; is_top_level_class = false; has_list_ancestor = false; 
}
...
bool System::IpSrcIfItems::FtpItems::FtpList::has_data() const
{
    if (is_presence_container) return true;
    return vrf.is_set
	|| srcif.is_set;
...
bool System::IpSrcIfItems::FtpItems::FtpList::has_operation() const
{
    return is_set(yfilter)
	|| ydk::is_set(vrf.yfilter)
	|| ydk::is_set(srcif.yfilter);
...
std::string System::IpSrcIfItems::FtpItems::FtpList::get_absolute_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "Cisco-NX-OS-device:System/ipSrcIf-items/ftp-items/" << get_segment_path();
    return path_buffer.str();
}

std::string System::IpSrcIfItems::FtpItems::FtpList::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "Ftp-list";
    ADD_KEY_TOKEN(vrf, "vrf");
    return path_buffer.str();
}

std::vector<std::pair<std::string, LeafData> > System::IpSrcIfItems::FtpItems::FtpList::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};
...
std::shared_ptr<ydk::Entity> System::IpSrcIfItems::FtpItems::FtpList::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> System::IpSrcIfItems::FtpItems::FtpList::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
void System::IpSrcIfItems::FtpItems::FtpList::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
    if(value_path == "vrf")
    {
        vrf = value;
...
void System::IpSrcIfItems::FtpItems::FtpList::set_filter(const std::string & value_path, YFilter yfilter)
{
    if(value_path == "vrf")
    {
        vrf.yfilter = yfilter;
...
bool System::IpSrcIfItems::FtpItems::FtpList::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "vrf" || name == "srcIf")
        return true;
    return false;
...
System::IpSrcIfItems::TelnetItems::TelnetItems()
    :
    telnet_list(this, {"vrf"})
{

    yang_name = "telnet-items"; yang_parent_name = "ipSrcIf-items"; is_top_level_class = false; has_list_ancestor = false; 
}
...
bool System::IpSrcIfItems::TelnetItems::has_data() const
{
    if (is_presence_container) return true;
    for (std::size_t index=0; index<telnet_list.len(); index++)
    {
        if(telnet_list[index]->has_data())
            return true;
    }
    return false;
}

bool System::IpSrcIfItems::TelnetItems::has_operation() const
{
    for (std::size_t index=0; index<telnet_list.len(); index++)
    {
        if(telnet_list[index]->has_operation())
            return true;
    }
    return is_set(yfilter);
}

std::string System::IpSrcIfItems::TelnetItems::get_absolute_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "Cisco-NX-OS-device:System/ipSrcIf-items/" << get_segment_path();
    return path_buffer.str();
...
std::string System::IpSrcIfItems::TelnetItems::get_segment_path() const
{
    std::ostringstream path_buffer;
...
std::vector<std::pair<std::string, LeafData> > System::IpSrcIfItems::TelnetItems::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};
...
std::shared_ptr<ydk::Entity> System::IpSrcIfItems::TelnetItems::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "Telnet-list")
    {
        auto ent_ = std::make_shared<System::IpSrcIfItems::TelnetItems::TelnetList>();
        ent_->parent = this;
        telnet_list.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> System::IpSrcIfItems::TelnetItems::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : telnet_list.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
void System::IpSrcIfItems::TelnetItems::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
}

void System::IpSrcIfItems::TelnetItems::set_filter(const std::string & value_path, YFilter yfilter)
{
}

bool System::IpSrcIfItems::TelnetItems::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "Telnet-list")
        return true;
    return false;
...
System::IpSrcIfItems::TelnetItems::TelnetList::TelnetList()
    :
    vrf{YType::str, "vrf"},
    srcif{YType::str, "srcIf"}
{

    yang_name = "Telnet-list"; yang_parent_name = "telnet-items"; is_top_level_class = false; has_list_ancestor = false; 
}
...
bool System::IpSrcIfItems::TelnetItems::TelnetList::has_data() const
{
    if (is_presence_container) return true;
    return vrf.is_set
	|| srcif.is_set;
...
bool System::IpSrcIfItems::TelnetItems::TelnetList::has_operation() const
{
    return is_set(yfilter)
	|| ydk::is_set(vrf.yfilter)
	|| ydk::is_set(srcif.yfilter);
...
std::string System::IpSrcIfItems::TelnetItems::TelnetList::get_absolute_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "Cisco-NX-OS-device:System/ipSrcIf-items/telnet-items/" << get_segment_path();
    return path_buffer.str();
}

std::string System::IpSrcIfItems::TelnetItems::TelnetList::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "Telnet-list";
    ADD_KEY_TOKEN(vrf, "vrf");
    return path_buffer.str();
}

std::vector<std::pair<std::string, LeafData> > System::IpSrcIfItems::TelnetItems::TelnetList::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};
...
std::shared_ptr<ydk::Entity> System::IpSrcIfItems::TelnetItems::TelnetList::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> System::IpSrcIfItems::TelnetItems::TelnetList::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
void System::IpSrcIfItems::TelnetItems::TelnetList::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
    if(value_path == "vrf")
    {
        vrf = value;
...
void System::IpSrcIfItems::TelnetItems::TelnetList::set_filter(const std::string & value_path, YFilter yfilter)
{
    if(value_path == "vrf")
    {
        vrf.yfilter = yfilter;
...
bool System::IpSrcIfItems::TelnetItems::TelnetList::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "vrf" || name == "srcIf")
        return true;
    return false;
...
System::IpSrcIfItems::SshItems::SshItems()
    :
    ssh_list(this, {"vrf"})
{

    yang_name = "ssh-items"; yang_parent_name = "ipSrcIf-items"; is_top_level_class = false; has_list_ancestor = false; 
}
...
bool System::IpSrcIfItems::SshItems::has_data() const
{
    if (is_presence_container) return true;
    for (std::size_t index=0; index<ssh_list.len(); index++)
    {
        if(ssh_list[index]->has_data())
            return true;
    }
    return false;
}

bool System::IpSrcIfItems::SshItems::has_operation() const
{
    for (std::size_t index=0; index<ssh_list.len(); index++)
    {
        if(ssh_list[index]->has_operation())
            return true;
    }
    return is_set(yfilter);
}

std::string System::IpSrcIfItems::SshItems::get_absolute_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "Cisco-NX-OS-device:System/ipSrcIf-items/" << get_segment_path();
    return path_buffer.str();
...
std::string System::IpSrcIfItems::SshItems::get_segment_path() const
{
    std::ostringstream path_buffer;
...
std::vector<std::pair<std::string, LeafData> > System::IpSrcIfItems::SshItems::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};
...
std::shared_ptr<ydk::Entity> System::IpSrcIfItems::SshItems::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "Ssh-list")
    {
        auto ent_ = std::make_shared<System::IpSrcIfItems::SshItems::SshList>();
        ent_->parent = this;
        ssh_list.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> System::IpSrcIfItems::SshItems::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : ssh_list.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
void System::IpSrcIfItems::SshItems::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
}

void System::IpSrcIfItems::SshItems::set_filter(const std::string & value_path, YFilter yfilter)
{
}

bool System::IpSrcIfItems::SshItems::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "Ssh-list")
        return true;
    return false;
...
System::IpSrcIfItems::SshItems::SshList::SshList()
    :
    vrf{YType::str, "vrf"},
    srcif{YType::str, "srcIf"}
{

    yang_name = "Ssh-list"; yang_parent_name = "ssh-items"; is_top_level_class = false; has_list_ancestor = false; 
}
...
bool System::IpSrcIfItems::SshItems::SshList::has_data() const
{
    if (is_presence_container) return true;
    return vrf.is_set
	|| srcif.is_set;
...
bool System::IpSrcIfItems::SshItems::SshList::has_operation() const
{
    return is_set(yfilter)
	|| ydk::is_set(vrf.yfilter)
	|| ydk::is_set(srcif.yfilter);
...
std::string System::IpSrcIfItems::SshItems::SshList::get_absolute_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "Cisco-NX-OS-device:System/ipSrcIf-items/ssh-items/" << get_segment_path();
    return path_buffer.str();
}

std::string System::IpSrcIfItems::SshItems::SshList::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "Ssh-list";
    ADD_KEY_TOKEN(vrf, "vrf");
    return path_buffer.str();
}

std::vector<std::pair<std::string, LeafData> > System::IpSrcIfItems::SshItems::SshList::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};
...
std::shared_ptr<ydk::Entity> System::IpSrcIfItems::SshItems::SshList::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> System::IpSrcIfItems::SshItems::SshList::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
void System::IpSrcIfItems::SshItems::SshList::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
    if(value_path == "vrf")
    {
        vrf = value;
...
void System::IpSrcIfItems::SshItems::SshList::set_filter(const std::string & value_path, YFilter yfilter)
{
    if(value_path == "vrf")
    {
        vrf.yfilter = yfilter;
...
bool System::IpSrcIfItems::SshItems::SshList::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "vrf" || name == "srcIf")
        return true;
    return false;
...
System::IpSrcIfItems::PingItems::PingItems()
    :
    ping_list(this, {"vrf"})
{

    yang_name = "ping-items"; yang_parent_name = "ipSrcIf-items"; is_top_level_class = false; has_list_ancestor = false; 
}
...
bool System::IpSrcIfItems::PingItems::has_data() const
{
    if (is_presence_container) return true;
    for (std::size_t index=0; index<ping_list.len(); index++)
    {
        if(ping_list[index]->has_data())
            return true;
    }
    return false;
}

bool System::IpSrcIfItems::PingItems::has_operation() const
{
    for (std::size_t index=0; index<ping_list.len(); index++)
    {
        if(ping_list[index]->has_operation())
            return true;
    }
    return is_set(yfilter);
}

std::string System::IpSrcIfItems::PingItems::get_absolute_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "Cisco-NX-OS-device:System/ipSrcIf-items/" << get_segment_path();
    return path_buffer.str();
...
std::string System::IpSrcIfItems::PingItems::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "ping-items";
    return path_buffer.str();
}

std::vector<std::pair<std::string, LeafData> > System::IpSrcIfItems::PingItems::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};
...
std::shared_ptr<ydk::Entity> System::IpSrcIfItems::PingItems::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "Ping-list")
    {
        auto ent_ = std::make_shared<System::IpSrcIfItems::PingItems::PingList>();
        ent_->parent = this;
        ping_list.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> System::IpSrcIfItems::PingItems::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : ping_list.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
void System::IpSrcIfItems::PingItems::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
}

void System::IpSrcIfItems::PingItems::set_filter(const std::string & value_path, YFilter yfilter)
{
}

bool System::IpSrcIfItems::PingItems::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "Ping-list")
        return true;
    return false;
...
System::IpSrcIfItems::PingItems::PingList::PingList()
    :
    vrf{YType::str, "vrf"},
    srcif{YType::str, "srcIf"}
{

    yang_name = "Ping-list"; yang_parent_name = "ping-items"; is_top_level_class = false; has_list_ancestor = false; 
}
...
bool System::IpSrcIfItems::PingItems::PingList::has_data() const
{
    if (is_presence_container) return true;
    return vrf.is_set
	|| srcif.is_set;
...
bool System::IpSrcIfItems::PingItems::PingList::has_operation() const
{
    return is_set(yfilter)
	|| ydk::is_set(vrf.yfilter)
	|| ydk::is_set(srcif.yfilter);
...
std::string System::IpSrcIfItems::PingItems::PingList::get_absolute_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "Cisco-NX-OS-device:System/ipSrcIf-items/ping-items/" << get_segment_path();
    return path_buffer.str();
}

std::string System::IpSrcIfItems::PingItems::PingList::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "Ping-list";
    ADD_KEY_TOKEN(vrf, "vrf");
    return path_buffer.str();
}

std::vector<std::pair<std::string, LeafData> > System::IpSrcIfItems::PingItems::PingList::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};
...
std::shared_ptr<ydk::Entity> System::IpSrcIfItems::PingItems::PingList::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> System::IpSrcIfItems::PingItems::PingList::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
void System::IpSrcIfItems::PingItems::PingList::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
    if(value_path == "vrf")
    {
        vrf = value;
...
void System::IpSrcIfItems::PingItems::PingList::set_filter(const std::string & value_path, YFilter yfilter)
{
    if(value_path == "vrf")
    {
        vrf.yfilter = yfilter;
...
bool System::IpSrcIfItems::PingItems::PingList::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "vrf" || name == "srcIf")
        return true;
    return false;
...
System::IpSrcIfItems::TracerouteItems::TracerouteItems()
    :
    traceroute_list(this, {"vrf"})
{

    yang_name = "traceroute-items"; yang_parent_name = "ipSrcIf-items"; is_top_level_class = false; has_list_ancestor = false; 
}
...
bool System::IpSrcIfItems::TracerouteItems::has_data() const
{
    if (is_presence_container) return true;
    for (std::size_t index=0; index<traceroute_list.len(); index++)
    {
        if(traceroute_list[index]->has_data())
            return true;
    }
    return false;
}

bool System::IpSrcIfItems::TracerouteItems::has_operation() const
{
    for (std::size_t index=0; index<traceroute_list.len(); index++)
    {
        if(traceroute_list[index]->has_operation())
            return true;
    }
    return is_set(yfilter);
}

std::string System::IpSrcIfItems::TracerouteItems::get_absolute_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "Cisco-NX-OS-device:System/ipSrcIf-items/" << get_segment_path();
    return path_buffer.str();
...
std::string System::IpSrcIfItems::TracerouteItems::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "traceroute-items";
    return path_buffer.str();
}

std::vector<std::pair<std::string, LeafData> > System::IpSrcIfItems::TracerouteItems::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};
...
std::shared_ptr<ydk::Entity> System::IpSrcIfItems::TracerouteItems::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "Traceroute-list")
    {
        auto ent_ = std::make_shared<System::IpSrcIfItems::TracerouteItems::TracerouteList>();
        ent_->parent = this;
        traceroute_list.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> System::IpSrcIfItems::TracerouteItems::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : traceroute_list.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
void System::IpSrcIfItems::TracerouteItems::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
}

void System::IpSrcIfItems::TracerouteItems::set_filter(const std::string & value_path, YFilter yfilter)
{
}

bool System::IpSrcIfItems::TracerouteItems::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "Traceroute-list")
        return true;
    return false;
}

System::IpSrcIfItems::TracerouteItems::TracerouteList::TracerouteList()
    :
    vrf{YType::str, "vrf"},
    srcif{YType::str, "srcIf"}
{

    yang_name = "Traceroute-list"; yang_parent_name = "traceroute-items"; is_top_level_class = false; has_list_ancestor = false; 
}
...
bool System::IpSrcIfItems::TracerouteItems::TracerouteList::has_data() const
{
    if (is_presence_container) return true;
    return vrf.is_set
	|| srcif.is_set;
...
bool System::IpSrcIfItems::TracerouteItems::TracerouteList::has_operation() const
{
    return is_set(yfilter)
	|| ydk::is_set(vrf.yfilter)
	|| ydk::is_set(srcif.yfilter);
...
std::string System::IpSrcIfItems::TracerouteItems::TracerouteList::get_absolute_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "Cisco-NX-OS-device:System/ipSrcIf-items/traceroute-items/" << get_segment_path();
    return path_buffer.str();
}

std::string System::IpSrcIfItems::TracerouteItems::TracerouteList::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "Traceroute-list";
    ADD_KEY_TOKEN(vrf, "vrf");
    return path_buffer.str();
}

std::vector<std::pair<std::string, LeafData> > System::IpSrcIfItems::TracerouteItems::TracerouteList::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};
...
std::shared_ptr<ydk::Entity> System::IpSrcIfItems::TracerouteItems::TracerouteList::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> System::IpSrcIfItems::TracerouteItems::TracerouteList::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
void System::IpSrcIfItems::TracerouteItems::TracerouteList::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
    if(value_path == "vrf")
    {
        vrf = value;
...
void System::IpSrcIfItems::TracerouteItems::TracerouteList::set_filter(const std::string & value_path, YFilter yfilter)
{
    if(value_path == "vrf")
    {
        vrf.yfilter = yfilter;
...
bool System::IpSrcIfItems::TracerouteItems::TracerouteList::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "vrf" || name == "srcIf")
        return true;
    return false;
...
System::SysmgrItems::SysmgrItems()
    ...
    , config_items(std::make_shared<System::SysmgrItems::ConfigItems>())
    , memalert_items(std::make_shared<System::SysmgrItems::MemalertItems>())
    , switchoveralert_items(std::make_shared<System::SysmgrItems::SwitchoveralertItems>())
    , core_items(std::make_shared<System::SysmgrItems::CoreItems>())
{
    ...
    memalert_items->parent = this;
    switchoveralert_items->parent = this;
...
{
    if (is_presence_container) return true;
    return (sys_items !=  nullptr && sys_items->has_data())
	|| (syscfg_items !=  nullptr && syscfg_items->has_data())
	|| (config_items !=  nullptr && config_items->has_data())
	|| (memalert_items !=  nullptr && memalert_items->has_data())
	|| (switchoveralert_items !=  nullptr && switchoveralert_items->has_data())
...
bool System::SysmgrItems::has_operation() const
{
	...
	|| (memalert_items !=  nullptr && memalert_items->has_operation())
	|| (switchoveralert_items !=  nullptr && switchoveralert_items->has_operation())
...
std::shared_ptr<ydk::Entity> System::SysmgrItems::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    ...
    if(child_yang_name == "memalert-items")
    {
        if(memalert_items == nullptr)
        {
            memalert_items = std::make_shared<System::SysmgrItems::MemalertItems>();
        }
        return memalert_items;
    }

    if(child_yang_name == "switchoveralert-items")
    {
        if(switchoveralert_items == nullptr)
        {
            switchoveralert_items = std::make_shared<System::SysmgrItems::SwitchoveralertItems>();
        }
        return switchoveralert_items;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> System::SysmgrItems::get_children() const
{
    ...
    if(memalert_items != nullptr)
    {
        _children["memalert-items"] = memalert_items;
    }

    if(switchoveralert_items != nullptr)
    {
        _children["switchoveralert-items"] = switchoveralert_items;
    }
...
{
    if(name == "sys-items" || name == "sysCfg-items" || name == "config-items" || name == "memalert-items" || name == "switchoveralert-items" || name == "core-items")
        return true;
    return false;
}
...
{
    if (is_presence_container) return true;
    return (sup_items !=  nullptr && sup_items->has_data())
...
{
    if(name == "sup-items" || name == "image-items")
        return true;
    return false;
}
...
System::SysmgrItems::ConfigItems::ConfigItems()
    :
    adminst{YType::enumeration, "adminSt"},
    tracemask{YType::uint32, "traceMask"},
    operst{YType::enumeration, "operSt"},
    fastreloadtimer{YType::uint32, "fastReloadTimer"}
        ,
    systemcore_items(std::make_shared<System::SysmgrItems::ConfigItems::SystemcoreItems>())
{
    systemcore_items->parent = this;

    yang_name = "config-items"; yang_parent_name = "sysmgr-items"; is_top_level_class = false; has_list_ancestor = false; 
}
...
bool System::SysmgrItems::ConfigItems::has_data() const
{
    if (is_presence_container) return true;
    return adminst.is_set
	|| tracemask.is_set
	|| operst.is_set
	|| fastreloadtimer.is_set
	|| (systemcore_items !=  nullptr && systemcore_items->has_data());
}

bool System::SysmgrItems::ConfigItems::has_operation() const
{
    return is_set(yfilter)
	|| ydk::is_set(adminst.yfilter)
	|| ydk::is_set(tracemask.yfilter)
	|| ydk::is_set(operst.yfilter)
	|| ydk::is_set(fastreloadtimer.yfilter)
	|| (systemcore_items !=  nullptr && systemcore_items->has_operation());
}

std::string System::SysmgrItems::ConfigItems::get_absolute_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "Cisco-NX-OS-device:System/sysmgr-items/" << get_segment_path();
    return path_buffer.str();
...
std::string System::SysmgrItems::ConfigItems::get_segment_path() const
{
    std::ostringstream path_buffer;
...
std::vector<std::pair<std::string, LeafData> > System::SysmgrItems::ConfigItems::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};

    if (adminst.is_set || is_set(adminst.yfilter)) leaf_name_data.push_back(adminst.get_name_leafdata());
    if (tracemask.is_set || is_set(tracemask.yfilter)) leaf_name_data.push_back(tracemask.get_name_leafdata());
    if (operst.is_set || is_set(operst.yfilter)) leaf_name_data.push_back(operst.get_name_leafdata());
    if (fastreloadtimer.is_set || is_set(fastreloadtimer.yfilter)) leaf_name_data.push_back(fastreloadtimer.get_name_leafdata());

    return leaf_name_data;

}

std::shared_ptr<ydk::Entity> System::SysmgrItems::ConfigItems::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "systemcore-items")
    {
        if(systemcore_items == nullptr)
        {
            systemcore_items = std::make_shared<System::SysmgrItems::ConfigItems::SystemcoreItems>();
        }
        return systemcore_items;
    }

    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> System::SysmgrItems::ConfigItems::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(systemcore_items != nullptr)
    {
        _children["systemcore-items"] = systemcore_items;
    }

    return _children;
}

void System::SysmgrItems::ConfigItems::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
    ...
    if(value_path == "traceMask")
    {
        tracemask = value;
        tracemask.value_namespace = name_space;
        tracemask.value_namespace_prefix = name_space_prefix;
    }
    if(value_path == "operSt")
    {
        operst = value;
    ...
    if(value_path == "fastReloadTimer")
    {
        fastreloadtimer = value;
        fastreloadtimer.value_namespace = name_space;
        fastreloadtimer.value_namespace_prefix = name_space_prefix;
    }
}

void System::SysmgrItems::ConfigItems::set_filter(const std::string & value_path, YFilter yfilter)
{
    if(value_path == "adminSt")
    {
    ...
    if(value_path == "fastReloadTimer")
    {
        fastreloadtimer.yfilter = yfilter;
    }
}

bool System::SysmgrItems::ConfigItems::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "systemcore-items" || name == "adminSt" || name == "traceMask" || name == "operSt" || name == "fastReloadTimer")
        return true;
    return false;
}

System::SysmgrItems::ConfigItems::SystemcoreItems::SystemcoreItems()
    :
    adminst{YType::enumeration, "adminSt"},
    uritype{YType::enumeration, "uriType"},
    ...
    operst{YType::enumeration, "operSt"}
{
    ...
    yang_name = "systemcore-items"; yang_parent_name = "config-items"; is_top_level_class = false; has_list_ancestor = false; 
}
...
bool System::SysmgrItems::ConfigItems::SystemcoreItems::has_data() const
{
    if (is_presence_container) return true;
    return adminst.is_set
	|| uritype.is_set
...
bool System::SysmgrItems::ConfigItems::SystemcoreItems::has_operation() const
{
    return is_set(yfilter)
	|| ydk::is_set(adminst.yfilter)
	|| ydk::is_set(uritype.yfilter)
...
std::string System::SysmgrItems::ConfigItems::SystemcoreItems::get_absolute_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "Cisco-NX-OS-device:System/sysmgr-items/config-items/" << get_segment_path();
    return path_buffer.str();
}

std::string System::SysmgrItems::ConfigItems::SystemcoreItems::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "systemcore-items";
    return path_buffer.str();
}

std::vector<std::pair<std::string, LeafData> > System::SysmgrItems::ConfigItems::SystemcoreItems::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};

    if (adminst.is_set || is_set(adminst.yfilter)) leaf_name_data.push_back(adminst.get_name_leafdata());
    if (uritype.is_set || is_set(uritype.yfilter)) leaf_name_data.push_back(uritype.get_name_leafdata());
...
std::shared_ptr<ydk::Entity> System::SysmgrItems::ConfigItems::SystemcoreItems::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> System::SysmgrItems::ConfigItems::SystemcoreItems::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
void System::SysmgrItems::ConfigItems::SystemcoreItems::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
    if(value_path == "adminSt")
    {
        adminst = value;
    ...
    if(value_path == "uriType")
    {
        uritype = value;
        uritype.value_namespace = name_space;
        uritype.value_namespace_prefix = name_space_prefix;
    }
...
void System::SysmgrItems::ConfigItems::SystemcoreItems::set_filter(const std::string & value_path, YFilter yfilter)
{
    if(value_path == "adminSt")
    {
        adminst.yfilter = yfilter;
...
bool System::SysmgrItems::ConfigItems::SystemcoreItems::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "adminSt" || name == "uriType" || name == "server" || name == "path" || name == "vrf" || name == "operSt")
        return true;
    return false;
}

System::SysmgrItems::MemalertItems::MemalertItems()
    :
    adminst{YType::enumeration, "adminSt"},
    applet{YType::str, "applet"},
    module{YType::uint32, "module"},
    major_{YType::uint32, "major"},
    minor{YType::uint32, "minor"},
    clear{YType::uint32, "clear"},
    operst{YType::enumeration, "operSt"}
{

    yang_name = "memalert-items"; yang_parent_name = "sysmgr-items"; is_top_level_class = false; has_list_ancestor = false; 
}
...
bool System::SysmgrItems::MemalertItems::has_data() const
{
    if (is_presence_container) return true;
    return adminst.is_set
	|| applet.is_set
...
bool System::SysmgrItems::MemalertItems::has_operation() const
{
    return is_set(yfilter)
	|| ydk::is_set(adminst.yfilter)
	|| ydk::is_set(applet.yfilter)
...
std::string System::SysmgrItems::MemalertItems::get_absolute_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "Cisco-NX-OS-device:System/sysmgr-items/" << get_segment_path();
    return path_buffer.str();
...
std::string System::SysmgrItems::MemalertItems::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "memalert-items";
    return path_buffer.str();
}

std::vector<std::pair<std::string, LeafData> > System::SysmgrItems::MemalertItems::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};

    if (adminst.is_set || is_set(adminst.yfilter)) leaf_name_data.push_back(adminst.get_name_leafdata());
    if (applet.is_set || is_set(applet.yfilter)) leaf_name_data.push_back(applet.get_name_leafdata());
...
std::shared_ptr<ydk::Entity> System::SysmgrItems::MemalertItems::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> System::SysmgrItems::MemalertItems::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
void System::SysmgrItems::MemalertItems::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
    if(value_path == "adminSt")
    {
        adminst = value;
    ...
    if(value_path == "applet")
    {
        applet = value;
        applet.value_namespace = name_space;
        applet.value_namespace_prefix = name_space_prefix;
    }
...
void System::SysmgrItems::MemalertItems::set_filter(const std::string & value_path, YFilter yfilter)
{
    if(value_path == "adminSt")
    {
        adminst.yfilter = yfilter;
...
bool System::SysmgrItems::MemalertItems::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "adminSt" || name == "applet" || name == "module" || name == "major" || name == "minor" || name == "clear" || name == "operSt")
        return true;
    return false;
}

System::SysmgrItems::SwitchoveralertItems::SwitchoveralertItems()
    :
    adminst{YType::enumeration, "adminSt"},
    applet{YType::str, "applet"},
    ...
    operst{YType::enumeration, "operSt"}
{
    ...
    yang_name = "switchoveralert-items"; yang_parent_name = "sysmgr-items"; is_top_level_class = false; has_list_ancestor = false; 
}
...
bool System::SysmgrItems::SwitchoveralertItems::has_data() const
{
    if (is_presence_container) return true;
    return adminst.is_set
	|| applet.is_set
...
bool System::SysmgrItems::SwitchoveralertItems::has_operation() const
{
    return is_set(yfilter)
	|| ydk::is_set(adminst.yfilter)
	|| ydk::is_set(applet.yfilter)
...
std::string System::SysmgrItems::SwitchoveralertItems::get_absolute_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "Cisco-NX-OS-device:System/sysmgr-items/" << get_segment_path();
    return path_buffer.str();
...
std::string System::SysmgrItems::SwitchoveralertItems::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "switchoveralert-items";
    return path_buffer.str();
}

std::vector<std::pair<std::string, LeafData> > System::SysmgrItems::SwitchoveralertItems::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};
...
std::shared_ptr<ydk::Entity> System::SysmgrItems::SwitchoveralertItems::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> System::SysmgrItems::SwitchoveralertItems::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
void System::SysmgrItems::SwitchoveralertItems::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
    if(value_path == "adminSt")
    {
        adminst = value;
...
void System::SysmgrItems::SwitchoveralertItems::set_filter(const std::string & value_path, YFilter yfilter)
{
    if(value_path == "adminSt")
    {
        adminst.yfilter = yfilter;
...
bool System::SysmgrItems::SwitchoveralertItems::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "adminSt" || name == "applet" || name == "count" || name == "time" || name == "operSt")
        return true;
    return false;
}
...
System::TcpudpItems::InstItems::InstItems()
    ...
    tcpmssbytes{YType::uint16, "tcpMssBytes"},
    tcpstartportrange{YType::uint16, "tcpStartPortRange"},
    tcpendportrange{YType::uint16, "tcpEndPortRange"},
    dscplop{YType::uint16, "dscpLop"}
        ,
    localport_items(std::make_shared<System::TcpudpItems::InstItems::LocalportItems>())
{
    localport_items->parent = this;
...
bool System::TcpudpItems::InstItems::has_data() const
{
	...
	|| tcpmssbytes.is_set
	|| tcpstartportrange.is_set
	|| tcpendportrange.is_set
	|| dscplop.is_set
	|| (localport_items !=  nullptr && localport_items->has_data());
}
...
bool System::TcpudpItems::InstItems::has_operation() const
{
	...
	|| ydk::is_set(tcpmssbytes.yfilter)
	|| ydk::is_set(tcpstartportrange.yfilter)
	|| ydk::is_set(tcpendportrange.yfilter)
	|| ydk::is_set(dscplop.yfilter)
	|| (localport_items !=  nullptr && localport_items->has_operation());
}
...
std::vector<std::pair<std::string, LeafData> > System::TcpudpItems::InstItems::get_name_leaf_data() const
{
    ...
    if (tcpmssbytes.is_set || is_set(tcpmssbytes.yfilter)) leaf_name_data.push_back(tcpmssbytes.get_name_leafdata());
    if (tcpstartportrange.is_set || is_set(tcpstartportrange.yfilter)) leaf_name_data.push_back(tcpstartportrange.get_name_leafdata());
    if (tcpendportrange.is_set || is_set(tcpendportrange.yfilter)) leaf_name_data.push_back(tcpendportrange.get_name_leafdata());
    if (dscplop.is_set || is_set(dscplop.yfilter)) leaf_name_data.push_back(dscplop.get_name_leafdata());
...
{
    if(child_yang_name == "localport-items")
    {
        if(localport_items == nullptr)
        {
            localport_items = std::make_shared<System::TcpudpItems::InstItems::LocalportItems>();
        }
        return localport_items;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> System::TcpudpItems::InstItems::get_children() const
{
    ...
    if(localport_items != nullptr)
    {
        _children["localport-items"] = localport_items;
    }
...
void System::TcpudpItems::InstItems::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
    ...
    if(value_path == "tcpMssBytes")
    {
        tcpmssbytes = value;
        tcpmssbytes.value_namespace = name_space;
        tcpmssbytes.value_namespace_prefix = name_space_prefix;
    }
    if(value_path == "tcpStartPortRange")
    {
        tcpstartportrange = value;
        tcpstartportrange.value_namespace = name_space;
        tcpstartportrange.value_namespace_prefix = name_space_prefix;
    }
    if(value_path == "tcpEndPortRange")
    {
        tcpendportrange = value;
        tcpendportrange.value_namespace = name_space;
        tcpendportrange.value_namespace_prefix = name_space_prefix;
    }
    if(value_path == "dscpLop")
    {
        dscplop = value;
        dscplop.value_namespace = name_space;
        dscplop.value_namespace_prefix = name_space_prefix;
    }
}
...
void System::TcpudpItems::InstItems::set_filter(const std::string & value_path, YFilter yfilter)
{
    ...
    if(value_path == "tcpMssBytes")
    {
        tcpmssbytes.yfilter = yfilter;
    }
    if(value_path == "tcpStartPortRange")
    {
        tcpstartportrange.yfilter = yfilter;
    }
    if(value_path == "tcpEndPortRange")
    {
        tcpendportrange.yfilter = yfilter;
    }
    if(value_path == "dscpLop")
    {
        dscplop.yfilter = yfilter;
    }
}

bool System::TcpudpItems::InstItems::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "localport-items" || name == "synWaitTime" || name == "pathMtuDiscovery" || name == "tcpMssBytes" || name == "tcpStartPortRange" || name == "tcpEndPortRange" || name == "dscpLop")
        return true;
    return false;
}

System::TcpudpItems::InstItems::LocalportItems::LocalportItems()
    :
    kstackstartport{YType::uint16, "kstackStartPort"},
    kstackendport{YType::uint16, "kstackEndPort"},
    netstackstartport{YType::uint16, "netstackStartPort"},
    netstackendport{YType::uint16, "netstackEndPort"},
    natstartport{YType::uint16, "natStartPort"},
    natendport{YType::uint16, "natEndPort"},
    name{YType::str, "name"},
    adminst{YType::enumeration, "adminSt"},
    operst{YType::enumeration, "operSt"},
    opererr{YType::str, "operErr"}
{
    ...
    yang_name = "localport-items"; yang_parent_name = "inst-items"; is_top_level_class = false; has_list_ancestor = false; 
}
...
bool System::TcpudpItems::InstItems::LocalportItems::has_data() const
{
    if (is_presence_container) return true;
    return kstackstartport.is_set
	|| kstackendport.is_set
	|| netstackstartport.is_set
	|| netstackendport.is_set
	|| natstartport.is_set
	|| natendport.is_set
	|| name.is_set
	|| adminst.is_set
...
bool System::TcpudpItems::InstItems::LocalportItems::has_operation() const
{
    return is_set(yfilter)
	|| ydk::is_set(kstackstartport.yfilter)
	|| ydk::is_set(kstackendport.yfilter)
	|| ydk::is_set(netstackstartport.yfilter)
	|| ydk::is_set(netstackendport.yfilter)
	|| ydk::is_set(natstartport.yfilter)
	|| ydk::is_set(natendport.yfilter)
	|| ydk::is_set(name.yfilter)
	|| ydk::is_set(adminst.yfilter)
	|| ydk::is_set(operst.yfilter)
...
std::string System::TcpudpItems::InstItems::LocalportItems::get_absolute_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "Cisco-NX-OS-device:System/tcpudp-items/inst-items/" << get_segment_path();
    return path_buffer.str();
}

std::string System::TcpudpItems::InstItems::LocalportItems::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "localport-items";
    return path_buffer.str();
}

std::vector<std::pair<std::string, LeafData> > System::TcpudpItems::InstItems::LocalportItems::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};

    if (kstackstartport.is_set || is_set(kstackstartport.yfilter)) leaf_name_data.push_back(kstackstartport.get_name_leafdata());
    if (kstackendport.is_set || is_set(kstackendport.yfilter)) leaf_name_data.push_back(kstackendport.get_name_leafdata());
    if (netstackstartport.is_set || is_set(netstackstartport.yfilter)) leaf_name_data.push_back(netstackstartport.get_name_leafdata());
    if (netstackendport.is_set || is_set(netstackendport.yfilter)) leaf_name_data.push_back(netstackendport.get_name_leafdata());
    if (natstartport.is_set || is_set(natstartport.yfilter)) leaf_name_data.push_back(natstartport.get_name_leafdata());
    if (natendport.is_set || is_set(natendport.yfilter)) leaf_name_data.push_back(natendport.get_name_leafdata());
    if (name.is_set || is_set(name.yfilter)) leaf_name_data.push_back(name.get_name_leafdata());
    if (adminst.is_set || is_set(adminst.yfilter)) leaf_name_data.push_back(adminst.get_name_leafdata());
    if (operst.is_set || is_set(operst.yfilter)) leaf_name_data.push_back(operst.get_name_leafdata());
...
std::shared_ptr<ydk::Entity> System::TcpudpItems::InstItems::LocalportItems::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> System::TcpudpItems::InstItems::LocalportItems::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
void System::TcpudpItems::InstItems::LocalportItems::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
    if(value_path == "kstackStartPort")
    {
        kstackstartport = value;
        kstackstartport.value_namespace = name_space;
        kstackstartport.value_namespace_prefix = name_space_prefix;
    }
    if(value_path == "kstackEndPort")
    {
        kstackendport = value;
        kstackendport.value_namespace = name_space;
        kstackendport.value_namespace_prefix = name_space_prefix;
    }
    if(value_path == "netstackStartPort")
    {
        netstackstartport = value;
        netstackstartport.value_namespace = name_space;
        netstackstartport.value_namespace_prefix = name_space_prefix;
    }
    if(value_path == "netstackEndPort")
    {
        netstackendport = value;
        netstackendport.value_namespace = name_space;
        netstackendport.value_namespace_prefix = name_space_prefix;
    }
    if(value_path == "natStartPort")
    {
        natstartport = value;
        natstartport.value_namespace = name_space;
        natstartport.value_namespace_prefix = name_space_prefix;
    }
    if(value_path == "natEndPort")
    {
        natendport = value;
        natendport.value_namespace = name_space;
        natendport.value_namespace_prefix = name_space_prefix;
    }
    if(value_path == "name")
    {
        name = value;
...
void System::TcpudpItems::InstItems::LocalportItems::set_filter(const std::string & value_path, YFilter yfilter)
{
    if(value_path == "kstackStartPort")
    {
        kstackstartport.yfilter = yfilter;
    }
    if(value_path == "kstackEndPort")
    {
        kstackendport.yfilter = yfilter;
    }
    if(value_path == "netstackStartPort")
    {
        netstackstartport.yfilter = yfilter;
    }
    if(value_path == "netstackEndPort")
    {
        netstackendport.yfilter = yfilter;
    }
    if(value_path == "natStartPort")
    {
        natstartport.yfilter = yfilter;
    }
    if(value_path == "natEndPort")
    {
        natendport.yfilter = yfilter;
    }
    if(value_path == "name")
    {
        name.yfilter = yfilter;
...
bool System::TcpudpItems::InstItems::LocalportItems::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "kstackStartPort" || name == "kstackEndPort" || name == "netstackStartPort" || name == "netstackEndPort" || name == "natStartPort" || name == "natEndPort" || name == "name" || name == "adminSt" || name == "operSt" || name == "operErr")
        return true;
    return false;
}
