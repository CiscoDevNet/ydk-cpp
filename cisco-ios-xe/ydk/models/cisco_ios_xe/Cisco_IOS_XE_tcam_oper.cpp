...
std::shared_ptr<ydk::Entity> TcamDetails::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "tcam-detail")
    {
        auto ent_ = std::make_shared<TcamDetails::TcamDetail>();
        ent_->parent = this;
        tcam_detail.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> TcamDetails::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : tcam_detail.entities())
    {
...
std::shared_ptr<ydk::Entity> TcamDetails::clone_ptr() const
{
...
std::shared_ptr<ydk::Entity> TcamDetails::TcamDetail::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> TcamDetails::TcamDetail::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
