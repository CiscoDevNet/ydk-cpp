...
std::shared_ptr<Entity> BreakoutPortOperData::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "port-breakout")
    {
        auto c = std::make_shared<BreakoutPortOperData::PortBreakout>();
        c->parent = this;
        port_breakout.append(c);
        return c;
    }
...
std::map<std::string, std::shared_ptr<Entity>> BreakoutPortOperData::get_children() const
{
    std::map<std::string, std::shared_ptr<Entity>> children{};
    char count=0;
    count = 0;
    for (auto c : port_breakout.entities())
    {
...
std::shared_ptr<Entity> BreakoutPortOperData::clone_ptr() const
{
...
std::shared_ptr<Entity> BreakoutPortOperData::PortBreakout::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<Entity>> BreakoutPortOperData::PortBreakout::get_children() const
{
    std::map<std::string, std::shared_ptr<Entity>> children{};
    char count=0;
    return children;
