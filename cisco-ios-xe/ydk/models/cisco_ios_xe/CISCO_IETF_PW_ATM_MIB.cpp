...
std::shared_ptr<ydk::Entity> CISCOIETFPWATMMIB::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, std::shared_ptr<ydk::Entity>> CISCOIETFPWATMMIB::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(cpwvcatmtable != nullptr)
    {
        _children["cpwVcAtmTable"] = cpwvcatmtable;
    }
...
std::shared_ptr<ydk::Entity> CISCOIETFPWATMMIB::clone_ptr() const
{
...
{
    if (is_presence_container) return true;
    for (std::size_t index=0; index<cpwvcatmentry.len(); index++)
    {
...
std::shared_ptr<ydk::Entity> CISCOIETFPWATMMIB::CpwVcAtmTable::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "cpwVcAtmEntry")
    {
        auto ent_ = std::make_shared<CISCOIETFPWATMMIB::CpwVcAtmTable::CpwVcAtmEntry>();
        ent_->parent = this;
        cpwvcatmentry.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> CISCOIETFPWATMMIB::CpwVcAtmTable::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : cpwvcatmentry.entities())
    {
...
std::shared_ptr<ydk::Entity> CISCOIETFPWATMMIB::CpwVcAtmTable::CpwVcAtmEntry::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> CISCOIETFPWATMMIB::CpwVcAtmTable::CpwVcAtmEntry::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
