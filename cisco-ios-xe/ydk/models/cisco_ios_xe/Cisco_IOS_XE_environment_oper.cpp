...
std::shared_ptr<ydk::Entity> EnvironmentSensors::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "environment-sensor")
    {
        auto ent_ = std::make_shared<EnvironmentSensors::EnvironmentSensor>();
        ent_->parent = this;
        environment_sensor.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> EnvironmentSensors::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : environment_sensor.entities())
    {
...
std::shared_ptr<ydk::Entity> EnvironmentSensors::clone_ptr() const
{
...
std::shared_ptr<ydk::Entity> EnvironmentSensors::EnvironmentSensor::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> EnvironmentSensors::EnvironmentSensor::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
