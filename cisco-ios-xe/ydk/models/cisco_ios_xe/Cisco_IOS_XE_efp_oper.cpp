...
std::shared_ptr<ydk::Entity> EfpStats::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "efp-stat")
    {
        auto ent_ = std::make_shared<EfpStats::EfpStat>();
        ent_->parent = this;
        efp_stat.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> EfpStats::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : efp_stat.entities())
    {
...
std::shared_ptr<ydk::Entity> EfpStats::clone_ptr() const
{
...
std::shared_ptr<ydk::Entity> EfpStats::EfpStat::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> EfpStats::EfpStat::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
