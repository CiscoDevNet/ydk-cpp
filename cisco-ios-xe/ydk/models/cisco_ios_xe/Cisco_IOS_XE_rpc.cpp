...
SwitchRpc::SwitchRpc()
    :
    input(std::make_shared<SwitchRpc::Input>())
	,output(std::make_shared<SwitchRpc::Output>())
{
    input->parent = this;
...
SwitchRpc::Input::Input()
    ...
    switch_number{YType::uint8, "switch-number"}
    	,
    statck(std::make_shared<SwitchRpc::Input::Statck>())
{
    statck->parent = this;
...
DefaultRpc::DefaultRpc()
    :
    input(std::make_shared<DefaultRpc::Input>())
	,output(std::make_shared<DefaultRpc::Output>())
{
    input->parent = this;
...
ReloadRpc::ReloadRpc()
    :
    output(std::make_shared<ReloadRpc::Output>())
{
    output->parent = this;
...
LicenseRpc::LicenseRpc()
    :
    input(std::make_shared<LicenseRpc::Input>())
	,output(std::make_shared<LicenseRpc::Output>())
{
    input->parent = this;
...
LicenseRpc::Input::Input()
    :
    smart(std::make_shared<LicenseRpc::Input::Smart>())
{
    smart->parent = this;
...
LicenseRpc::Input::Smart::Smart()
    ...
    deregister{YType::empty, "deregister"}
    	,
    register(std::make_shared<LicenseRpc::Input::Smart::Register>())
	,renew(std::make_shared<LicenseRpc::Input::Smart::Renew>())
{
...
bool LicenseRpc::Input::Smart::has_data() const
{
	...
	|| (register !=  nullptr && register->has_data())
...
bool LicenseRpc::Input::Smart::has_operation() const
{
	...
	|| (register !=  nullptr && register->has_operation())
...
LicenseRpc::Input::Smart::Register::Register()
    :
...
bool LicenseRpc::Input::Smart::Register::has_data() const
{
    return idtoken.is_set;
}

bool LicenseRpc::Input::Smart::Register::has_operation() const
{
    return is_set(operation)
	|| is_set(idtoken.operation);
}

std::string LicenseRpc::Input::Smart::Register::get_segment_path() const
{
    std::ostringstream path_buffer;
...
const EntityPath LicenseRpc::Input::Smart::Register::get_entity_path(Entity* ancestor) const
{
    std::ostringstream path_buffer;
    if (ancestor == nullptr)
    {
...
std::shared_ptr<Entity> LicenseRpc::Input::Smart::Register::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<Entity>> LicenseRpc::Input::Smart::Register::get_children() const
{
    std::map<std::string, std::shared_ptr<Entity>> children{};
    return children;
}

void LicenseRpc::Input::Smart::Register::set_value(const std::string & value_path, std::string value)
{
