...
Entity* Controller::Dpa::Nodes::Node::InternalTcamResources::NpuTcam::TcamBank::BankDb::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, Entity*> & Controller::Dpa::Nodes::Node::InternalTcamResources::NpuTcam::TcamBank::BankDb::get_children()
{
    return children;
}
...
Entity* Controller::Dpa::Nodes::Node::InternalTcamResources::NpuTcam::TcamBank::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "bank-db")
    {
        ...
        auto c = std::make_unique<Controller::Dpa::Nodes::Node::InternalTcamResources::NpuTcam::TcamBank::BankDb>();
        c->parent = this;
        bank_db.push_back(std::move(c));
        children[segment_path] = bank_db.back().get();
        return children.at(segment_path);
    }
...
std::map<std::string, Entity*> & Controller::Dpa::Nodes::Node::InternalTcamResources::NpuTcam::TcamBank::get_children()
{
    for (auto const & c : bank_db)
    {
...
Entity* Controller::Dpa::Nodes::Node::InternalTcamResources::NpuTcam::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "tcam-bank")
    {
        ...
        auto c = std::make_unique<Controller::Dpa::Nodes::Node::InternalTcamResources::NpuTcam::TcamBank>();
        c->parent = this;
        tcam_bank.push_back(std::move(c));
        children[segment_path] = tcam_bank.back().get();
        return children.at(segment_path);
    }
...
std::map<std::string, Entity*> & Controller::Dpa::Nodes::Node::InternalTcamResources::NpuTcam::get_children()
{
    for (auto const & c : tcam_bank)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
Entity* Controller::Dpa::Nodes::Node::InternalTcamResources::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "npu-tcam")
    {
        ...
        auto c = std::make_unique<Controller::Dpa::Nodes::Node::InternalTcamResources::NpuTcam>();
        c->parent = this;
        npu_tcam.push_back(std::move(c));
        children[segment_path] = npu_tcam.back().get();
        return children.at(segment_path);
    }
...
std::map<std::string, Entity*> & Controller::Dpa::Nodes::Node::InternalTcamResources::get_children()
{
    for (auto const & c : npu_tcam)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
Controller::Dpa::Nodes::Node::Node()
    :
    	...
    	,
    internal_tcam_resources(std::make_unique<Controller::Dpa::Nodes::Node::InternalTcamResources>())
{
    internal_tcam_resources->parent = this;
    children["internal-tcam-resources"] = internal_tcam_resources.get();
...
Entity* Controller::Dpa::Nodes::Node::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & Controller::Dpa::Nodes::Node::get_children()
{
...
Entity* Controller::Dpa::Nodes::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "node")
    {
        ...
        auto c = std::make_unique<Controller::Dpa::Nodes::Node>();
        c->parent = this;
...
std::map<std::string, Entity*> & Controller::Dpa::Nodes::get_children()
{
    for (auto const & c : node)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
Controller::Dpa::Dpa()
    :
    nodes(std::make_unique<Controller::Dpa::Nodes>())
{
    nodes->parent = this;
...
Entity* Controller::Dpa::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & Controller::Dpa::get_children()
{
...
Controller::Controller()
    :
    dpa(std::make_unique<Controller::Dpa>())
{
    dpa->parent = this;
    children["dpa"] = dpa.get();
...
Entity* Controller::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & Controller::get_children()
{
...
std::unique_ptr<Entity> Controller::clone_ptr()
{
