...
std::string ArpGmp::VrfInfos::VrfInfo::get_segment_path() const
{
    ...
    path_buffer << "vrf-info" <<"[vrf-name='" <<vrf_name.get() <<"']";
...
Entity* ArpGmp::VrfInfos::VrfInfo::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, Entity*> & ArpGmp::VrfInfos::VrfInfo::get_children()
{
    return children;
}
...
Entity* ArpGmp::VrfInfos::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "vrf-info")
    {
        ...
        auto c = std::make_unique<ArpGmp::VrfInfos::VrfInfo>();
        c->parent = this;
        vrf_info.push_back(std::move(c));
        children[segment_path] = vrf_info.back().get();
        return children.at(segment_path);
    }
...
std::map<std::string, Entity*> & ArpGmp::VrfInfos::get_children()
{
    for (auto const & c : vrf_info)
    {
...
{
    std::ostringstream path_buffer;
    path_buffer << "configured-ip-address" <<"[address='" <<address.get() <<"']";
...
Entity* ArpGmp::Vrfs::Vrf::ConfiguredIpAddresses::ConfiguredIpAddress::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & ArpGmp::Vrfs::Vrf::ConfiguredIpAddresses::ConfiguredIpAddress::get_children()
{
    return children;
}
...
Entity* ArpGmp::Vrfs::Vrf::ConfiguredIpAddresses::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "configured-ip-address")
    {
        ...
        auto c = std::make_unique<ArpGmp::Vrfs::Vrf::ConfiguredIpAddresses::ConfiguredIpAddress>();
        c->parent = this;
        configured_ip_address.push_back(std::move(c));
        children[segment_path] = configured_ip_address.back().get();
        return children.at(segment_path);
    }
...
std::map<std::string, Entity*> & ArpGmp::Vrfs::Vrf::ConfiguredIpAddresses::get_children()
{
    for (auto const & c : configured_ip_address)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
Entity* ArpGmp::Vrfs::Vrf::Routes::Route::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & ArpGmp::Vrfs::Vrf::Routes::Route::get_children()
{
    return children;
}
...
Entity* ArpGmp::Vrfs::Vrf::Routes::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "route")
    {
        ...
        auto c = std::make_unique<ArpGmp::Vrfs::Vrf::Routes::Route>();
        c->parent = this;
        route.push_back(std::move(c));
        children[segment_path] = route.back().get();
        return children.at(segment_path);
    }
...
std::map<std::string, Entity*> & ArpGmp::Vrfs::Vrf::Routes::get_children()
{
    for (auto const & c : route)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
Entity* ArpGmp::Vrfs::Vrf::InterfaceConfiguredIps::InterfaceConfiguredIp::AssociatedConfigurationEntry::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & ArpGmp::Vrfs::Vrf::InterfaceConfiguredIps::InterfaceConfiguredIp::AssociatedConfigurationEntry::get_children()
{
    return children;
}
...
ArpGmp::Vrfs::Vrf::InterfaceConfiguredIps::InterfaceConfiguredIp::InterfaceConfiguredIp()
    :
	 ...
	 reference_count{YType::uint32, "reference-count"}
    	,
    associated_configuration_entry(std::make_unique<ArpGmp::Vrfs::Vrf::InterfaceConfiguredIps::InterfaceConfiguredIp::AssociatedConfigurationEntry>())
{
    associated_configuration_entry->parent = this;
    children["associated-configuration-entry"] = associated_configuration_entry.get();
...
Entity* ArpGmp::Vrfs::Vrf::InterfaceConfiguredIps::InterfaceConfiguredIp::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & ArpGmp::Vrfs::Vrf::InterfaceConfiguredIps::InterfaceConfiguredIp::get_children()
{
...
Entity* ArpGmp::Vrfs::Vrf::InterfaceConfiguredIps::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "interface-configured-ip")
    {
        ...
        auto c = std::make_unique<ArpGmp::Vrfs::Vrf::InterfaceConfiguredIps::InterfaceConfiguredIp>();
        c->parent = this;
        interface_configured_ip.push_back(std::move(c));
        children[segment_path] = interface_configured_ip.back().get();
        return children.at(segment_path);
    }
...
std::map<std::string, Entity*> & ArpGmp::Vrfs::Vrf::InterfaceConfiguredIps::get_children()
{
    for (auto const & c : interface_configured_ip)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
ArpGmp::Vrfs::Vrf::Vrf()
    :
    	vrf_name{YType::str, "vrf-name"}
    	,
    configured_ip_addresses(std::make_unique<ArpGmp::Vrfs::Vrf::ConfiguredIpAddresses>())
	,interface_configured_ips(std::make_unique<ArpGmp::Vrfs::Vrf::InterfaceConfiguredIps>())
	,routes(std::make_unique<ArpGmp::Vrfs::Vrf::Routes>())
{
    configured_ip_addresses->parent = this;
    children["configured-ip-addresses"] = configured_ip_addresses.get();

    interface_configured_ips->parent = this;
    children["interface-configured-ips"] = interface_configured_ips.get();
...
Entity* ArpGmp::Vrfs::Vrf::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & ArpGmp::Vrfs::Vrf::get_children()
{
...
Entity* ArpGmp::Vrfs::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "vrf")
    {
        ...
        auto c = std::make_unique<ArpGmp::Vrfs::Vrf>();
        c->parent = this;
...
std::map<std::string, Entity*> & ArpGmp::Vrfs::get_children()
{
    for (auto const & c : vrf)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
ArpGmp::ArpGmp()
    :
    vrf_infos(std::make_unique<ArpGmp::VrfInfos>())
	,vrfs(std::make_unique<ArpGmp::Vrfs>())
{
    vrf_infos->parent = this;
    children["vrf-infos"] = vrf_infos.get();
...
Entity* Arp::Nodes::Node::ResolutionHistoryDynamic::ArpEntry::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & Arp::Nodes::Node::ResolutionHistoryDynamic::ArpEntry::get_children()
{
    return children;
}
...
Entity* Arp::Nodes::Node::ResolutionHistoryDynamic::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "arp-entry")
    {
        ...
        auto c = std::make_unique<Arp::Nodes::Node::ResolutionHistoryDynamic::ArpEntry>();
        c->parent = this;
        arp_entry.push_back(std::move(c));
        children[segment_path] = arp_entry.back().get();
        return children.at(segment_path);
    }
...
std::map<std::string, Entity*> & Arp::Nodes::Node::ResolutionHistoryDynamic::get_children()
{
    for (auto const & c : arp_entry)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
{
    std::ostringstream path_buffer;
    path_buffer << "traffic-vrf" <<"[vrf-name='" <<vrf_name.get() <<"']";
...
Entity* Arp::Nodes::Node::TrafficVrfs::TrafficVrf::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & Arp::Nodes::Node::TrafficVrfs::TrafficVrf::get_children()
{
    return children;
}
...
Entity* Arp::Nodes::Node::TrafficVrfs::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "traffic-vrf")
    {
        ...
        auto c = std::make_unique<Arp::Nodes::Node::TrafficVrfs::TrafficVrf>();
        c->parent = this;
        traffic_vrf.push_back(std::move(c));
        children[segment_path] = traffic_vrf.back().get();
        return children.at(segment_path);
    }
...
std::map<std::string, Entity*> & Arp::Nodes::Node::TrafficVrfs::get_children()
{
    for (auto const & c : traffic_vrf)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
Entity* Arp::Nodes::Node::TrafficNode::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & Arp::Nodes::Node::TrafficNode::get_children()
{
    return children;
}
...
Entity* Arp::Nodes::Node::ResolutionHistoryClient::ArpEntry::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & Arp::Nodes::Node::ResolutionHistoryClient::ArpEntry::get_children()
{
    return children;
}
...
Entity* Arp::Nodes::Node::ResolutionHistoryClient::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "arp-entry")
    {
        ...
        auto c = std::make_unique<Arp::Nodes::Node::ResolutionHistoryClient::ArpEntry>();
        c->parent = this;
        arp_entry.push_back(std::move(c));
        children[segment_path] = arp_entry.back().get();
        return children.at(segment_path);
...
std::map<std::string, Entity*> & Arp::Nodes::Node::ResolutionHistoryClient::get_children()
{
    for (auto const & c : arp_entry)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
{
    std::ostringstream path_buffer;
    path_buffer << "entry" <<"[address='" <<address.get() <<"']" <<"[interface-name='" <<interface_name.get() <<"']";
...
Entity* Arp::Nodes::Node::Entries::Entry::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & Arp::Nodes::Node::Entries::Entry::get_children()
{
    return children;
}
...
Entity* Arp::Nodes::Node::Entries::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "entry")
    {
        ...
        auto c = std::make_unique<Arp::Nodes::Node::Entries::Entry>();
        c->parent = this;
...
std::map<std::string, Entity*> & Arp::Nodes::Node::Entries::get_children()
{
    for (auto const & c : entry)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
{
    std::ostringstream path_buffer;
    path_buffer << "traffic-interface" <<"[interface-name='" <<interface_name.get() <<"']";
...
Entity* Arp::Nodes::Node::TrafficInterfaces::TrafficInterface::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & Arp::Nodes::Node::TrafficInterfaces::TrafficInterface::get_children()
{
    return children;
}
...
Entity* Arp::Nodes::Node::TrafficInterfaces::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "traffic-interface")
    {
        ...
        auto c = std::make_unique<Arp::Nodes::Node::TrafficInterfaces::TrafficInterface>();
        c->parent = this;
        traffic_interface.push_back(std::move(c));
        children[segment_path] = traffic_interface.back().get();
        return children.at(segment_path);
    }
...
std::map<std::string, Entity*> & Arp::Nodes::Node::TrafficInterfaces::get_children()
{
    for (auto const & c : traffic_interface)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
Arp::Nodes::Node::Node()
    :
    	...
    	,
    entries(std::make_unique<Arp::Nodes::Node::Entries>())
	,resolution_history_client(std::make_unique<Arp::Nodes::Node::ResolutionHistoryClient>())
	,resolution_history_dynamic(std::make_unique<Arp::Nodes::Node::ResolutionHistoryDynamic>())
	,traffic_interfaces(std::make_unique<Arp::Nodes::Node::TrafficInterfaces>())
	,traffic_node(std::make_unique<Arp::Nodes::Node::TrafficNode>())
	,traffic_vrfs(std::make_unique<Arp::Nodes::Node::TrafficVrfs>())
{
    entries->parent = this;
    children["entries"] = entries.get();

    resolution_history_client->parent = this;
    children["resolution-history-client"] = resolution_history_client.get();

    resolution_history_dynamic->parent = this;
    children["resolution-history-dynamic"] = resolution_history_dynamic.get();

    traffic_interfaces->parent = this;
    children["traffic-interfaces"] = traffic_interfaces.get();

    traffic_node->parent = this;
    children["traffic-node"] = traffic_node.get();

    traffic_vrfs->parent = this;
    children["traffic-vrfs"] = traffic_vrfs.get();
...
Entity* Arp::Nodes::Node::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & Arp::Nodes::Node::get_children()
{
...
Entity* Arp::Nodes::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "node")
    {
        ...
        auto c = std::make_unique<Arp::Nodes::Node>();
        c->parent = this;
...
std::map<std::string, Entity*> & Arp::Nodes::get_children()
{
    for (auto const & c : node)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
Arp::Arp()
    :
    nodes(std::make_unique<Arp::Nodes>())
{
    nodes->parent = this;
