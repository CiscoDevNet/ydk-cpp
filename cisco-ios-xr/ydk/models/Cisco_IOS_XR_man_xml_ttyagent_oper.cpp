...
Entity* Netconf::Agent::Tty::Sessions::Session::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, Entity*> & Netconf::Agent::Tty::Sessions::Session::get_children()
{
    return children;
}
...
Entity* Netconf::Agent::Tty::Sessions::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "session")
    {
        ...
        auto c = std::make_unique<Netconf::Agent::Tty::Sessions::Session>();
        c->parent = this;
...
std::map<std::string, Entity*> & Netconf::Agent::Tty::Sessions::get_children()
{
    for (auto const & c : session)
    {
...
Netconf::Agent::Tty::Tty()
    :
    sessions(std::make_unique<Netconf::Agent::Tty::Sessions>())
{
...
Entity* XrXml::Agent::Tty::Sessions::Session::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & XrXml::Agent::Tty::Sessions::Session::get_children()
{
    return children;
}
...
Entity* XrXml::Agent::Tty::Sessions::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "session")
    {
        ...
        auto c = std::make_unique<XrXml::Agent::Tty::Sessions::Session>();
        c->parent = this;
        session.push_back(std::move(c));
        children[segment_path] = session.back().get();
        return children.at(segment_path);
...
std::map<std::string, Entity*> & XrXml::Agent::Tty::Sessions::get_children()
{
    for (auto const & c : session)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
XrXml::Agent::Tty::Tty()
    :
    sessions(std::make_unique<XrXml::Agent::Tty::Sessions>())
{
    sessions->parent = this;
    children["sessions"] = sessions.get();
...
Entity* XrXml::Agent::Default_::Sessions::Session::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & XrXml::Agent::Default_::Sessions::Session::get_children()
{
    return children;
}
...
Entity* XrXml::Agent::Default_::Sessions::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "session")
    {
        ...
        auto c = std::make_unique<XrXml::Agent::Default_::Sessions::Session>();
        c->parent = this;
        session.push_back(std::move(c));
        children[segment_path] = session.back().get();
        return children.at(segment_path);
...
std::map<std::string, Entity*> & XrXml::Agent::Default_::Sessions::get_children()
{
    for (auto const & c : session)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
XrXml::Agent::Default_::Default_()
    :
    sessions(std::make_unique<XrXml::Agent::Default_::Sessions>())
{
    sessions->parent = this;
    children["sessions"] = sessions.get();
...
Entity* XrXml::Agent::Ssl::Sessions::Session::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & XrXml::Agent::Ssl::Sessions::Session::get_children()
{
    return children;
}
...
Entity* XrXml::Agent::Ssl::Sessions::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "session")
    {
        ...
        auto c = std::make_unique<XrXml::Agent::Ssl::Sessions::Session>();
        c->parent = this;
        session.push_back(std::move(c));
        children[segment_path] = session.back().get();
        return children.at(segment_path);
...
std::map<std::string, Entity*> & XrXml::Agent::Ssl::Sessions::get_children()
{
    for (auto const & c : session)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
XrXml::Agent::Ssl::Ssl()
    :
    sessions(std::make_unique<XrXml::Agent::Ssl::Sessions>())
{
    sessions->parent = this;
    children["sessions"] = sessions.get();
