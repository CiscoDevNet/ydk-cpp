...
std::string EtherLinkOam::DiscoveryInfoInterfaces::DiscoveryInfoInterface::get_segment_path() const
{
    ...
    path_buffer << "discovery-info-interface" <<"[member-interface='" <<member_interface.get() <<"']";
...
Entity* EtherLinkOam::DiscoveryInfoInterfaces::DiscoveryInfoInterface::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, Entity*> & EtherLinkOam::DiscoveryInfoInterfaces::DiscoveryInfoInterface::get_children()
{
    return children;
}
...
Entity* EtherLinkOam::DiscoveryInfoInterfaces::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "discovery-info-interface")
    {
        ...
        auto c = std::make_unique<EtherLinkOam::DiscoveryInfoInterfaces::DiscoveryInfoInterface>();
        c->parent = this;
        discovery_info_interface.push_back(std::move(c));
        children[segment_path] = discovery_info_interface.back().get();
        return children.at(segment_path);
    }
...
std::map<std::string, Entity*> & EtherLinkOam::DiscoveryInfoInterfaces::get_children()
{
    for (auto const & c : discovery_info_interface)
    {
...
Entity* EtherLinkOam::InterfaceStateInterfaces::InterfaceStateInterface::Errors::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & EtherLinkOam::InterfaceStateInterfaces::InterfaceStateInterface::Errors::get_children()
{
    return children;
}
...
Entity* EtherLinkOam::InterfaceStateInterfaces::InterfaceStateInterface::EfdTriggers::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & EtherLinkOam::InterfaceStateInterfaces::InterfaceStateInterface::EfdTriggers::get_children()
{
    return children;
}
...
EtherLinkOam::InterfaceStateInterfaces::InterfaceStateInterface::InterfaceStateInterface()
    :
	 ...
	 rx_fault{YType::boolean, "rx-fault"}
    	,
    efd_triggers(std::make_unique<EtherLinkOam::InterfaceStateInterfaces::InterfaceStateInterface::EfdTriggers>())
	,errors(std::make_unique<EtherLinkOam::InterfaceStateInterfaces::InterfaceStateInterface::Errors>())
{
    efd_triggers->parent = this;
    children["efd-triggers"] = efd_triggers.get();

    errors->parent = this;
    children["errors"] = errors.get();
...
{
    std::ostringstream path_buffer;
    path_buffer << "interface-state-interface" <<"[member-interface='" <<member_interface.get() <<"']";
...
Entity* EtherLinkOam::InterfaceStateInterfaces::InterfaceStateInterface::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & EtherLinkOam::InterfaceStateInterfaces::InterfaceStateInterface::get_children()
{
...
Entity* EtherLinkOam::InterfaceStateInterfaces::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "interface-state-interface")
    {
        ...
        auto c = std::make_unique<EtherLinkOam::InterfaceStateInterfaces::InterfaceStateInterface>();
        c->parent = this;
        interface_state_interface.push_back(std::move(c));
        children[segment_path] = interface_state_interface.back().get();
        return children.at(segment_path);
    }
...
std::map<std::string, Entity*> & EtherLinkOam::InterfaceStateInterfaces::get_children()
{
    for (auto const & c : interface_state_interface)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
EtherLinkOam::RunningConfigInterfaces::RunningConfigInterface::RunningConfigInterface()
    :
	 ...
	 frame_period_threshold_high{YType::uint32, "frame-period-threshold-high"},
	 frame_period_threshold_high_overridden{YType::boolean, "frame-period-threshold-high-overridden"},
	 frame_period_threshold_low{YType::uint32, "frame-period-threshold-low"},
	 ...
	 frame_seconds_threshold_high{YType::uint32, "frame-seconds-threshold-high"},
	 frame_seconds_threshold_high_overridden{YType::boolean, "frame-seconds-threshold-high-overridden"},
	 frame_seconds_threshold_low{YType::uint32, "frame-seconds-threshold-low"},
	 ...
	 frame_threshold_high{YType::uint32, "frame-threshold-high"},
	 frame_threshold_high_overridden{YType::boolean, "frame-threshold-high-overridden"},
	 frame_threshold_low{YType::uint32, "frame-threshold-low"},
...
{
    std::ostringstream path_buffer;
    path_buffer << "running-config-interface" <<"[member-interface='" <<member_interface.get() <<"']";
...
Entity* EtherLinkOam::RunningConfigInterfaces::RunningConfigInterface::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & EtherLinkOam::RunningConfigInterfaces::RunningConfigInterface::get_children()
{
    return children;
}
...
Entity* EtherLinkOam::RunningConfigInterfaces::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "running-config-interface")
    {
        ...
        auto c = std::make_unique<EtherLinkOam::RunningConfigInterfaces::RunningConfigInterface>();
        c->parent = this;
        running_config_interface.push_back(std::move(c));
        children[segment_path] = running_config_interface.back().get();
        return children.at(segment_path);
    }
...
std::map<std::string, Entity*> & EtherLinkOam::RunningConfigInterfaces::get_children()
{
    for (auto const & c : running_config_interface)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
Entity* EtherLinkOam::Nodes::Node::Summary::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & EtherLinkOam::Nodes::Node::Summary::get_children()
{
    return children;
}
...
EtherLinkOam::Nodes::Node::Node()
    :
    	...
    	,
    summary(std::make_unique<EtherLinkOam::Nodes::Node::Summary>())
{
    summary->parent = this;
...
Entity* EtherLinkOam::Nodes::Node::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & EtherLinkOam::Nodes::Node::get_children()
{
...
Entity* EtherLinkOam::Nodes::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "node")
    {
        ...
        auto c = std::make_unique<EtherLinkOam::Nodes::Node>();
        c->parent = this;
...
std::map<std::string, Entity*> & EtherLinkOam::Nodes::get_children()
{
    for (auto const & c : node)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
{
    std::ostringstream path_buffer;
    path_buffer << "event-log-entry-index" <<"[event-log-entry-index='" <<event_log_entry_index.get() <<"']";
...
Entity* EtherLinkOam::EventLogEntryInterfaces::EventLogEntryInterface::EventLogEntryIndexes::EventLogEntryIndex::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & EtherLinkOam::EventLogEntryInterfaces::EventLogEntryInterface::EventLogEntryIndexes::EventLogEntryIndex::get_children()
{
    return children;
}
...
Entity* EtherLinkOam::EventLogEntryInterfaces::EventLogEntryInterface::EventLogEntryIndexes::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "event-log-entry-index")
    {
        ...
        auto c = std::make_unique<EtherLinkOam::EventLogEntryInterfaces::EventLogEntryInterface::EventLogEntryIndexes::EventLogEntryIndex>();
        c->parent = this;
        event_log_entry_index.push_back(std::move(c));
        children[segment_path] = event_log_entry_index.back().get();
        return children.at(segment_path);
    }
...
std::map<std::string, Entity*> & EtherLinkOam::EventLogEntryInterfaces::EventLogEntryInterface::EventLogEntryIndexes::get_children()
{
    for (auto const & c : event_log_entry_index)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
EtherLinkOam::EventLogEntryInterfaces::EventLogEntryInterface::EventLogEntryInterface()
    :
    	member_interface{YType::str, "member-interface"}
    	,
    event_log_entry_indexes(std::make_unique<EtherLinkOam::EventLogEntryInterfaces::EventLogEntryInterface::EventLogEntryIndexes>())
{
    event_log_entry_indexes->parent = this;
    children["event-log-entry-indexes"] = event_log_entry_indexes.get();
...
{
    std::ostringstream path_buffer;
    path_buffer << "event-log-entry-interface" <<"[member-interface='" <<member_interface.get() <<"']";
...
Entity* EtherLinkOam::EventLogEntryInterfaces::EventLogEntryInterface::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & EtherLinkOam::EventLogEntryInterfaces::EventLogEntryInterface::get_children()
{
...
Entity* EtherLinkOam::EventLogEntryInterfaces::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "event-log-entry-interface")
    {
        ...
        auto c = std::make_unique<EtherLinkOam::EventLogEntryInterfaces::EventLogEntryInterface>();
        c->parent = this;
        event_log_entry_interface.push_back(std::move(c));
        children[segment_path] = event_log_entry_interface.back().get();
        return children.at(segment_path);
    }
...
std::map<std::string, Entity*> & EtherLinkOam::EventLogEntryInterfaces::get_children()
{
    for (auto const & c : event_log_entry_interface)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
{
    std::ostringstream path_buffer;
    path_buffer << "stats-interface" <<"[member-interface='" <<member_interface.get() <<"']";
...
Entity* EtherLinkOam::StatsInterfaces::StatsInterface::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & EtherLinkOam::StatsInterfaces::StatsInterface::get_children()
{
    return children;
}
...
Entity* EtherLinkOam::StatsInterfaces::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "stats-interface")
    {
        ...
        auto c = std::make_unique<EtherLinkOam::StatsInterfaces::StatsInterface>();
        c->parent = this;
        stats_interface.push_back(std::move(c));
        children[segment_path] = stats_interface.back().get();
        return children.at(segment_path);
    }
...
std::map<std::string, Entity*> & EtherLinkOam::StatsInterfaces::get_children()
{
    for (auto const & c : stats_interface)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
EtherLinkOam::EtherLinkOam()
    :
    discovery_info_interfaces(std::make_unique<EtherLinkOam::DiscoveryInfoInterfaces>())
	,event_log_entry_interfaces(std::make_unique<EtherLinkOam::EventLogEntryInterfaces>())
	,interface_state_interfaces(std::make_unique<EtherLinkOam::InterfaceStateInterfaces>())
	,nodes(std::make_unique<EtherLinkOam::Nodes>())
	,running_config_interfaces(std::make_unique<EtherLinkOam::RunningConfigInterfaces>())
	,stats_interfaces(std::make_unique<EtherLinkOam::StatsInterfaces>())
{
    discovery_info_interfaces->parent = this;
    children["discovery-info-interfaces"] = discovery_info_interfaces.get();

    event_log_entry_interfaces->parent = this;
    children["event-log-entry-interfaces"] = event_log_entry_interfaces.get();

    interface_state_interfaces->parent = this;
    children["interface-state-interfaces"] = interface_state_interfaces.get();

    nodes->parent = this;
    children["nodes"] = nodes.get();

    running_config_interfaces->parent = this;
    children["running-config-interfaces"] = running_config_interfaces.get();

    stats_interfaces->parent = this;
    children["stats-interfaces"] = stats_interfaces.get();
...
Entity* EtherLinkOam::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & EtherLinkOam::get_children()
{
...
std::unique_ptr<Entity> EtherLinkOam::clone_ptr()
{
