...
Entity* Dpa::Stats::Nodes::Node::HwResourcesDatas::HwResourcesData::NpuHwr::LtHwr::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & Dpa::Stats::Nodes::Node::HwResourcesDatas::HwResourcesData::NpuHwr::LtHwr::get_children()
{
    return children;
}
...
Entity* Dpa::Stats::Nodes::Node::HwResourcesDatas::HwResourcesData::NpuHwr::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "lt-hwr")
    {
        ...
        auto c = std::make_unique<Dpa::Stats::Nodes::Node::HwResourcesDatas::HwResourcesData::NpuHwr::LtHwr>();
        c->parent = this;
        lt_hwr.push_back(std::move(c));
        children[segment_path] = lt_hwr.back().get();
        return children.at(segment_path);
    }
...
std::map<std::string, Entity*> & Dpa::Stats::Nodes::Node::HwResourcesDatas::HwResourcesData::NpuHwr::get_children()
{
    for (auto const & c : lt_hwr)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
{
    std::ostringstream path_buffer;
    path_buffer << "hw-resources-data" <<"[resource='" <<resource.get() <<"']";
...
Entity* Dpa::Stats::Nodes::Node::HwResourcesDatas::HwResourcesData::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "npu-hwr")
    {
        ...
        auto c = std::make_unique<Dpa::Stats::Nodes::Node::HwResourcesDatas::HwResourcesData::NpuHwr>();
        c->parent = this;
        npu_hwr.push_back(std::move(c));
        children[segment_path] = npu_hwr.back().get();
        return children.at(segment_path);
    }
...
std::map<std::string, Entity*> & Dpa::Stats::Nodes::Node::HwResourcesDatas::HwResourcesData::get_children()
{
    for (auto const & c : npu_hwr)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
Entity* Dpa::Stats::Nodes::Node::HwResourcesDatas::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "hw-resources-data")
    {
        ...
        auto c = std::make_unique<Dpa::Stats::Nodes::Node::HwResourcesDatas::HwResourcesData>();
        c->parent = this;
        hw_resources_data.push_back(std::move(c));
        children[segment_path] = hw_resources_data.back().get();
        return children.at(segment_path);
    }
...
std::map<std::string, Entity*> & Dpa::Stats::Nodes::Node::HwResourcesDatas::get_children()
{
    for (auto const & c : hw_resources_data)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
Dpa::Stats::Nodes::Node::Node()
	...
	,hw_resources_datas(std::make_unique<Dpa::Stats::Nodes::Node::HwResourcesDatas>())
	...
	,voq_base_numbers(std::make_unique<Dpa::Stats::Nodes::Node::VoqBaseNumbers>())
{
    ...
    children["hw-resources-datas"] = hw_resources_datas.get();
...
bool Dpa::Stats::Nodes::Node::has_operation() const
{
	...
	|| (clear_voq_data_for_npu_numbers !=  nullptr && is_set(clear_voq_data_for_npu_numbers->operation))
	|| (hw_resources_datas !=  nullptr && is_set(hw_resources_datas->operation))
	|| (npu_number_for_trap_data_clears !=  nullptr && is_set(npu_number_for_trap_data_clears->operation))
	|| (npu_number_for_trap_datas !=  nullptr && is_set(npu_number_for_trap_datas->operation))
	|| (npu_number_for_voq_datas !=  nullptr && is_set(npu_number_for_voq_datas->operation))
	|| (voq_base_number_stats_clears !=  nullptr && is_set(voq_base_number_stats_clears->operation))
	|| (voq_base_numbers !=  nullptr && is_set(voq_base_numbers->operation));
}
...
const Enum::Value ResourceEnum::lem {0, "lem"};
const Enum::Value ResourceEnum::lpm {1, "lpm"};
const Enum::Value ResourceEnum::encap {2, "encap"};
const Enum::Value ResourceEnum::ext_tcam {3, "ext-tcam"};
const Enum::Value ResourceEnum::all {4, "all"};
