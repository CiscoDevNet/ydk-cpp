...
Entity* Sr::GlobalBlock::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, Entity*> & Sr::GlobalBlock::get_children()
{
    return children;
}
...
{
    std::ostringstream path_buffer;
    path_buffer << "mapping" <<"[af='" <<af.get() <<"']" <<"[ip='" <<ip.get() <<"']" <<"[mask='" <<mask.get() <<"']";
...
Entity* Sr::Mappings::Mapping::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & Sr::Mappings::Mapping::get_children()
{
    return children;
}
...
Entity* Sr::Mappings::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "mapping")
    {
        ...
        auto c = std::make_unique<Sr::Mappings::Mapping>();
        c->parent = this;
        mapping.push_back(std::move(c));
        children[segment_path] = mapping.back().get();
        return children.at(segment_path);
    }
...
std::map<std::string, Entity*> & Sr::Mappings::get_children()
{
    for (auto const & c : mapping)
    {
...
Sr::Sr()
	...
	,mappings(std::make_unique<Sr::Mappings>())
{
    mappings->parent = this;
    children["mappings"] = mappings.get();
...
Entity* Sr::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "global-block")
    {
        ...
        {
            children["global-block"] = global_block.get();
        }
        else
        {
            global_block = std::make_unique<Sr::GlobalBlock>();
...
std::map<std::string, Entity*> & Sr::get_children()
{
...
std::unique_ptr<Entity> Sr::clone_ptr()
{
