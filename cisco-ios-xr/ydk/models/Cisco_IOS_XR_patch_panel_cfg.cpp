...
Entity* PatchPanel::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, Entity*> & PatchPanel::get_children()
{
    return children;
}
...
std::unique_ptr<Entity> PatchPanel::clone_ptr()
{
    return std::make_unique<PatchPanel>();
}
