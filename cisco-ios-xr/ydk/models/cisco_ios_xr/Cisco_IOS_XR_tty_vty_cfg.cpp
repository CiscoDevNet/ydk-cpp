...
std::shared_ptr<ydk::Entity> Vty::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, std::shared_ptr<ydk::Entity>> Vty::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(vty_pools != nullptr)
    {
        _children["vty-pools"] = vty_pools;
    }
...
std::shared_ptr<ydk::Entity> Vty::clone_ptr() const
{
...
std::shared_ptr<ydk::Entity> Vty::VtyPools::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "vty-pool")
    {
        auto ent_ = std::make_shared<Vty::VtyPools::VtyPool>();
        ent_->parent = this;
        vty_pool.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> Vty::VtyPools::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : vty_pool.entities())
    {
...
std::shared_ptr<ydk::Entity> Vty::VtyPools::VtyPool::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> Vty::VtyPools::VtyPool::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
