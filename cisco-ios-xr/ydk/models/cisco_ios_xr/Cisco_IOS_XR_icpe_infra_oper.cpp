...
NvSatellite::NvSatellite()
    ...
    , install_image_reference_info(std::make_shared<NvSatellite::InstallImageReferenceInfo>())
    ...
    , sdacp_controls(std::make_shared<NvSatellite::SdacpControls>())
{
    ...
    install_image_reference_info->parent = this;
...
bool NvSatellite::has_data() const
{
	...
	|| (install_image_reference_info !=  nullptr && install_image_reference_info->has_data())
...
bool NvSatellite::has_operation() const
{
	...
	|| (install_image_reference_info !=  nullptr && install_image_reference_info->has_operation())
...
std::shared_ptr<ydk::Entity> NvSatellite::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    ...
    if(child_yang_name == "install-image-reference-info")
    {
        if(install_image_reference_info == nullptr)
        {
            install_image_reference_info = std::make_shared<NvSatellite::InstallImageReferenceInfo>();
        }
        return install_image_reference_info;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(reload_op_statuses != nullptr)
    {
        _children["reload-op-statuses"] = reload_op_statuses;
    }

    if(sdacp_redundancies != nullptr)
    {
        _children["sdacp-redundancies"] = sdacp_redundancies;
    }

    if(install_shows != nullptr)
    {
        _children["install-shows"] = install_shows;
    }

    if(satellite_statuses != nullptr)
    {
        _children["satellite-statuses"] = satellite_statuses;
    }

    if(satellite_priorities != nullptr)
    {
        _children["satellite-priorities"] = satellite_priorities;
    }

    if(satellite_versions != nullptr)
    {
        _children["satellite-versions"] = satellite_versions;
    }

    if(satellite_topologies != nullptr)
    {
        _children["satellite-topologies"] = satellite_topologies;
    }

    if(install_reference_info != nullptr)
    {
        _children["install-reference-info"] = install_reference_info;
    }

    if(install_op_progresses != nullptr)
    {
        _children["install-op-progresses"] = install_op_progresses;
    }
    ...
    if(install_op_statuses != nullptr)
    {
        _children["install-op-statuses"] = install_op_statuses;
    }

    if(install_image_reference_info != nullptr)
    {
        _children["install-image-reference-info"] = install_image_reference_info;
    }

    if(satellite_properties != nullptr)
    {
        _children["satellite-properties"] = satellite_properties;
    }

    if(sdacp_discovery2s != nullptr)
    {
        _children["Cisco-IOS-XR-icpe-sdacp-oper:sdacp-discovery2s"] = sdacp_discovery2s;
    }

    if(icpe_dpms != nullptr)
    {
        _children["Cisco-IOS-XR-icpe-sdacp-oper:icpe-dpms"] = icpe_dpms;
    }

    if(sdacp_controls != nullptr)
    {
        _children["Cisco-IOS-XR-icpe-sdacp-oper:sdacp-controls"] = sdacp_controls;
    }
...
std::shared_ptr<ydk::Entity> NvSatellite::clone_ptr() const
{
...
{
    if(name == "reload-op-statuses" || name == "sdacp-redundancies" || name == "install-shows" || name == "satellite-statuses" || name == "satellite-priorities" || name == "satellite-versions" || name == "satellite-topologies" || name == "install-reference-info" || name == "install-op-progresses" || name == "install" || name == "install-op-statuses" || name == "install-image-reference-info" || name == "satellite-properties" || name == "sdacp-discovery2s" || name == "icpe-dpms" || name == "sdacp-controls")
        return true;
    return false;
}
...
std::shared_ptr<ydk::Entity> NvSatellite::ReloadOpStatuses::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "reload-op-status")
    {
        auto ent_ = std::make_shared<NvSatellite::ReloadOpStatuses::ReloadOpStatus>();
        ent_->parent = this;
        reload_op_status.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::ReloadOpStatuses::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : reload_op_status.entities())
    {
...
std::shared_ptr<ydk::Entity> NvSatellite::ReloadOpStatuses::ReloadOpStatus::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::ReloadOpStatuses::ReloadOpStatus::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::SdacpRedundancies::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "sdacp-redundancy")
    {
        auto ent_ = std::make_shared<NvSatellite::SdacpRedundancies::SdacpRedundancy>();
        ent_->parent = this;
        sdacp_redundancy.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SdacpRedundancies::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : sdacp_redundancy.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> NvSatellite::SdacpRedundancies::SdacpRedundancy::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    ...
    {
        auto ent_ = std::make_shared<NvSatellite::SdacpRedundancies::SdacpRedundancy::Channel>();
        ent_->parent = this;
        channel.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SdacpRedundancies::SdacpRedundancy::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(protocol_state_timestamp != nullptr)
    {
        _children["protocol-state-timestamp"] = protocol_state_timestamp;
    }
...
std::shared_ptr<ydk::Entity> NvSatellite::SdacpRedundancies::SdacpRedundancy::ProtocolStateTimestamp::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SdacpRedundancies::SdacpRedundancy::ProtocolStateTimestamp::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::SdacpRedundancies::SdacpRedundancy::Channel::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SdacpRedundancies::SdacpRedundancy::Channel::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(channel_state_timestamp != nullptr)
    {
        _children["channel-state-timestamp"] = channel_state_timestamp;
    }

    if(resync_state_timestamp != nullptr)
    {
        _children["resync-state-timestamp"] = resync_state_timestamp;
    }
...
std::shared_ptr<ydk::Entity> NvSatellite::SdacpRedundancies::SdacpRedundancy::Channel::ChannelStateTimestamp::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SdacpRedundancies::SdacpRedundancy::Channel::ChannelStateTimestamp::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::SdacpRedundancies::SdacpRedundancy::Channel::ResyncStateTimestamp::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SdacpRedundancies::SdacpRedundancy::Channel::ResyncStateTimestamp::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::InstallShows::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "install-show")
    {
        auto ent_ = std::make_shared<NvSatellite::InstallShows::InstallShow>();
        ent_->parent = this;
        install_show.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::InstallShows::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : install_show.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
NvSatellite::InstallShows::InstallShow::InstallShow()
    ...
    reference_op{YType::boolean, "reference-op"},
    ref_type{YType::enumeration, "ref-type"},
    ref_name{YType::str, "ref-name"},
...
bool NvSatellite::InstallShows::InstallShow::has_operation() const
{
	...
	|| ydk::is_set(reference_op.yfilter)
	|| ydk::is_set(ref_type.yfilter)
	|| ydk::is_set(ref_name.yfilter)
...
std::vector<std::pair<std::string, LeafData> > NvSatellite::InstallShows::InstallShow::get_name_leaf_data() const
{
    ...
    if (reference_op.is_set || is_set(reference_op.yfilter)) leaf_name_data.push_back(reference_op.get_name_leafdata());
    if (ref_type.is_set || is_set(ref_type.yfilter)) leaf_name_data.push_back(ref_type.get_name_leafdata());
    if (ref_name.is_set || is_set(ref_name.yfilter)) leaf_name_data.push_back(ref_name.get_name_leafdata());
...
std::shared_ptr<ydk::Entity> NvSatellite::InstallShows::InstallShow::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "satellite")
    {
        auto ent_ = std::make_shared<NvSatellite::InstallShows::InstallShow::Satellite>();
        ent_->parent = this;
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::InstallShows::InstallShow::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
...
void NvSatellite::InstallShows::InstallShow::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
    ...
    if(value_path == "reference-op")
    {
        reference_op = value;
        reference_op.value_namespace = name_space;
        reference_op.value_namespace_prefix = name_space_prefix;
    }
    if(value_path == "ref-type")
    {
        ref_type = value;
        ref_type.value_namespace = name_space;
        ref_type.value_namespace_prefix = name_space_prefix;
    }
    if(value_path == "ref-name")
    {
        ref_name = value;
        ref_name.value_namespace = name_space;
        ref_name.value_namespace_prefix = name_space_prefix;
    }
...
void NvSatellite::InstallShows::InstallShow::set_filter(const std::string & value_path, YFilter yfilter)
{
    ...
    if(value_path == "reference-op")
    {
        reference_op.yfilter = yfilter;
    }
    if(value_path == "ref-type")
    {
        ref_type.yfilter = yfilter;
    }
    if(value_path == "ref-name")
    {
        ref_name.yfilter = yfilter;
    }
    if(value_path == "ref-state")
    {
...
{
    if(name == "satellite" || name == "operation-id" || name == "operation-id-xr" || name == "satellite-range" || name == "operation-type" || name == "progress-percentage" || name == "start-time" || name == "end-time" || name == "reference-op" || name == "ref-type" || name == "ref-name" || name == "ref-state" || name == "sats-not-initiated" || name == "sats-transferring" || name == "sats-activating" || name == "sats-updating" || name == "sats-replacing" || name == "sats-deactivating" || name == "sats-removing" || name == "sats-transfer-failed" || name == "sats-activate-failed" || name == "sats-update-failed" || name == "sats-replace-failed" || name == "sats-deactivate-failed" || name == "sats-remove-failed" || name == "sats-transfer-aborted" || name == "sats-activate-aborted" || name == "sats-update-aborted" || name == "sats-replace-aborted" || name == "sats-deactivate-aborted" || name == "sats-remove-aborted" || name == "sats-no-operation" || name == "sats-completed" || name == "name-string")
        return true;
    return false;
}
...
std::shared_ptr<ydk::Entity> NvSatellite::InstallShows::InstallShow::Satellite::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::InstallShows::InstallShow::Satellite::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::SatelliteStatuses::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "satellite-status")
    {
        auto ent_ = std::make_shared<NvSatellite::SatelliteStatuses::SatelliteStatus>();
        ent_->parent = this;
        satellite_status.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatelliteStatuses::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : satellite_status.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> NvSatellite::SatelliteStatuses::SatelliteStatus::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    ...
    {
        auto ent_ = std::make_shared<NvSatellite::SatelliteStatuses::SatelliteStatus::ReloadData>();
        ent_->parent = this;
        reload_data.append(ent_);
        return ent_;
    }

    if(child_yang_name == "configured-link")
    {
        auto ent_ = std::make_shared<NvSatellite::SatelliteStatuses::SatelliteStatus::ConfiguredLink>();
        ent_->parent = this;
        configured_link.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatelliteStatuses::SatelliteStatus::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(candidate_fabric_ports != nullptr)
    ...
    if(optical_status != nullptr)
    {
        _children["optical-status"] = optical_status;
    }

    if(redundancy_out_of_sync_timestamp != nullptr)
    {
        _children["redundancy-out-of-sync-timestamp"] = redundancy_out_of_sync_timestamp;
    }

    count_ = 0;
    for (auto ent_ : reload_data.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
    ...
    count_ = 0;
    for (auto ent_ : configured_link.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> NvSatellite::SatelliteStatuses::SatelliteStatus::CandidateFabricPorts::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "configured-port")
    {
        auto ent_ = std::make_shared<NvSatellite::SatelliteStatuses::SatelliteStatus::CandidateFabricPorts::ConfiguredPort>();
        ent_->parent = this;
        configured_port.append(ent_);
        return ent_;
    }

    if(child_yang_name == "current-port")
    {
        auto ent_ = std::make_shared<NvSatellite::SatelliteStatuses::SatelliteStatus::CandidateFabricPorts::CurrentPort>();
        ent_->parent = this;
        current_port.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatelliteStatuses::SatelliteStatus::CandidateFabricPorts::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : configured_port.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
    ...
    count_ = 0;
    for (auto ent_ : current_port.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> NvSatellite::SatelliteStatuses::SatelliteStatus::CandidateFabricPorts::ConfiguredPort::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatelliteStatuses::SatelliteStatus::CandidateFabricPorts::ConfiguredPort::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::SatelliteStatuses::SatelliteStatus::CandidateFabricPorts::CurrentPort::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatelliteStatuses::SatelliteStatus::CandidateFabricPorts::CurrentPort::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::SatelliteStatuses::SatelliteStatus::OpticalStatus::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "application")
    {
        auto ent_ = std::make_shared<NvSatellite::SatelliteStatuses::SatelliteStatus::OpticalStatus::Application>();
        ent_->parent = this;
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatelliteStatuses::SatelliteStatus::OpticalStatus::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
...
std::shared_ptr<ydk::Entity> NvSatellite::SatelliteStatuses::SatelliteStatus::OpticalStatus::Application::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatelliteStatuses::SatelliteStatus::OpticalStatus::Application::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::SatelliteStatuses::SatelliteStatus::RedundancyOutOfSyncTimestamp::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatelliteStatuses::SatelliteStatus::RedundancyOutOfSyncTimestamp::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::SatelliteStatuses::SatelliteStatus::ReloadData::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatelliteStatuses::SatelliteStatus::ReloadData::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::SatelliteStatuses::SatelliteStatus::ConfiguredLink::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "port-range")
    {
        auto ent_ = std::make_shared<NvSatellite::SatelliteStatuses::SatelliteStatus::ConfiguredLink::PortRange>();
        ent_->parent = this;
    ...
    {
        auto ent_ = std::make_shared<NvSatellite::SatelliteStatuses::SatelliteStatus::ConfiguredLink::DiscoveredLink>();
        ent_->parent = this;
        discovered_link.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatelliteStatuses::SatelliteStatus::ConfiguredLink::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    ...
    count_ = 0;
    for (auto ent_ : discovered_link.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> NvSatellite::SatelliteStatuses::SatelliteStatus::ConfiguredLink::PortRange::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatelliteStatuses::SatelliteStatus::ConfiguredLink::PortRange::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::SatelliteStatuses::SatelliteStatus::ConfiguredLink::DiscoveredLink::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatelliteStatuses::SatelliteStatus::ConfiguredLink::DiscoveredLink::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::SatellitePriorities::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "satellite-priority")
    {
        auto ent_ = std::make_shared<NvSatellite::SatellitePriorities::SatellitePriority>();
        ent_->parent = this;
        satellite_priority.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatellitePriorities::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : satellite_priority.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> NvSatellite::SatellitePriorities::SatellitePriority::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatellitePriorities::SatellitePriority::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::SatelliteVersions::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "satellite-version")
    {
        auto ent_ = std::make_shared<NvSatellite::SatelliteVersions::SatelliteVersion>();
        ent_->parent = this;
        satellite_version.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatelliteVersions::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : satellite_version.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> NvSatellite::SatelliteVersions::SatelliteVersion::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatelliteVersions::SatelliteVersion::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(active_version != nullptr)
    {
        _children["active-version"] = active_version;
    }

    if(transferred_version != nullptr)
    {
        _children["transferred-version"] = transferred_version;
    }

    if(committed_version != nullptr)
    {
        _children["committed-version"] = committed_version;
    }
...
std::shared_ptr<ydk::Entity> NvSatellite::SatelliteVersions::SatelliteVersion::ActiveVersion::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatelliteVersions::SatelliteVersion::ActiveVersion::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::SatelliteVersions::SatelliteVersion::TransferredVersion::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatelliteVersions::SatelliteVersion::TransferredVersion::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::SatelliteVersions::SatelliteVersion::CommittedVersion::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatelliteVersions::SatelliteVersion::CommittedVersion::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::SatelliteTopologies::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "satellite-topology")
    {
        auto ent_ = std::make_shared<NvSatellite::SatelliteTopologies::SatelliteTopology>();
        ent_->parent = this;
        satellite_topology.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatelliteTopologies::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : satellite_topology.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> NvSatellite::SatelliteTopologies::SatelliteTopology::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "discovered-link")
    {
        auto ent_ = std::make_shared<NvSatellite::SatelliteTopologies::SatelliteTopology::DiscoveredLink>();
        ent_->parent = this;
        discovered_link.append(ent_);
        return ent_;
    }
    ...
    if(child_yang_name == "satellite")
    {
        auto ent_ = std::make_shared<NvSatellite::SatelliteTopologies::SatelliteTopology::Satellite>();
        ent_->parent = this;
        satellite.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatelliteTopologies::SatelliteTopology::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
...
std::shared_ptr<ydk::Entity> NvSatellite::SatelliteTopologies::SatelliteTopology::DiscoveredLink::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatelliteTopologies::SatelliteTopology::DiscoveredLink::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::SatelliteTopologies::SatelliteTopology::Satellite::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "fabric-link")
    {
        auto ent_ = std::make_shared<NvSatellite::SatelliteTopologies::SatelliteTopology::Satellite::FabricLink>();
        ent_->parent = this;
        fabric_link.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatelliteTopologies::SatelliteTopology::Satellite::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : fabric_link.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> NvSatellite::SatelliteTopologies::SatelliteTopology::Satellite::FabricLink::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "remote-device")
    {
        auto ent_ = std::make_shared<NvSatellite::SatelliteTopologies::SatelliteTopology::Satellite::FabricLink::RemoteDevice>();
        ent_->parent = this;
        remote_device.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatelliteTopologies::SatelliteTopology::Satellite::FabricLink::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : remote_device.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> NvSatellite::SatelliteTopologies::SatelliteTopology::Satellite::FabricLink::RemoteDevice::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatelliteTopologies::SatelliteTopology::Satellite::FabricLink::RemoteDevice::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::InstallReferenceInfo::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::InstallReferenceInfo::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(references != nullptr)
    {
        _children["references"] = references;
    }
...
std::shared_ptr<ydk::Entity> NvSatellite::InstallReferenceInfo::References::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "reference")
    {
        auto ent_ = std::make_shared<NvSatellite::InstallReferenceInfo::References::Reference>();
        ent_->parent = this;
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::InstallReferenceInfo::References::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
...
std::shared_ptr<ydk::Entity> NvSatellite::InstallReferenceInfo::References::Reference::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::InstallReferenceInfo::References::Reference::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::InstallOpProgresses::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "install-op-progress")
    {
        auto ent_ = std::make_shared<NvSatellite::InstallOpProgresses::InstallOpProgress>();
        ent_->parent = this;
        install_op_progress.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::InstallOpProgresses::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : install_op_progress.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> NvSatellite::InstallOpProgresses::InstallOpProgress::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::InstallOpProgresses::InstallOpProgress::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::Install::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::Install::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(satellite_software_versions != nullptr)
    {
        _children["satellite-software-versions"] = satellite_software_versions;
    }
...
std::shared_ptr<ydk::Entity> NvSatellite::Install::SatelliteSoftwareVersions::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "satellite-software-version")
    {
        auto ent_ = std::make_shared<NvSatellite::Install::SatelliteSoftwareVersions::SatelliteSoftwareVersion>();
        ent_->parent = this;
        satellite_software_version.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::Install::SatelliteSoftwareVersions::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : satellite_software_version.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> NvSatellite::Install::SatelliteSoftwareVersions::SatelliteSoftwareVersion::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::Install::SatelliteSoftwareVersions::SatelliteSoftwareVersion::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(install_package_info != nullptr)
    {
        _children["install-package-info"] = install_package_info;
    }
...
std::shared_ptr<ydk::Entity> NvSatellite::Install::SatelliteSoftwareVersions::SatelliteSoftwareVersion::InstallPackageInfo::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::Install::SatelliteSoftwareVersions::SatelliteSoftwareVersion::InstallPackageInfo::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(active_packages != nullptr)
    {
        _children["active-packages"] = active_packages;
    }

    if(inactive_packages != nullptr)
    {
        _children["inactive-packages"] = inactive_packages;
    }

    if(committed_packages != nullptr)
    {
        _children["committed-packages"] = committed_packages;
    }
...
std::shared_ptr<ydk::Entity> NvSatellite::Install::SatelliteSoftwareVersions::SatelliteSoftwareVersion::InstallPackageInfo::ActivePackages::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "package")
    {
        auto ent_ = std::make_shared<NvSatellite::Install::SatelliteSoftwareVersions::SatelliteSoftwareVersion::InstallPackageInfo::ActivePackages::Package>();
        ent_->parent = this;
        package.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::Install::SatelliteSoftwareVersions::SatelliteSoftwareVersion::InstallPackageInfo::ActivePackages::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
...
std::shared_ptr<ydk::Entity> NvSatellite::Install::SatelliteSoftwareVersions::SatelliteSoftwareVersion::InstallPackageInfo::ActivePackages::Package::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::Install::SatelliteSoftwareVersions::SatelliteSoftwareVersion::InstallPackageInfo::ActivePackages::Package::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::Install::SatelliteSoftwareVersions::SatelliteSoftwareVersion::InstallPackageInfo::InactivePackages::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "package")
    {
        auto ent_ = std::make_shared<NvSatellite::Install::SatelliteSoftwareVersions::SatelliteSoftwareVersion::InstallPackageInfo::InactivePackages::Package>();
        ent_->parent = this;
        package.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::Install::SatelliteSoftwareVersions::SatelliteSoftwareVersion::InstallPackageInfo::InactivePackages::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
...
std::shared_ptr<ydk::Entity> NvSatellite::Install::SatelliteSoftwareVersions::SatelliteSoftwareVersion::InstallPackageInfo::InactivePackages::Package::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::Install::SatelliteSoftwareVersions::SatelliteSoftwareVersion::InstallPackageInfo::InactivePackages::Package::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::Install::SatelliteSoftwareVersions::SatelliteSoftwareVersion::InstallPackageInfo::CommittedPackages::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "package")
    {
        auto ent_ = std::make_shared<NvSatellite::Install::SatelliteSoftwareVersions::SatelliteSoftwareVersion::InstallPackageInfo::CommittedPackages::Package>();
        ent_->parent = this;
        package.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::Install::SatelliteSoftwareVersions::SatelliteSoftwareVersion::InstallPackageInfo::CommittedPackages::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
...
std::shared_ptr<ydk::Entity> NvSatellite::Install::SatelliteSoftwareVersions::SatelliteSoftwareVersion::InstallPackageInfo::CommittedPackages::Package::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::Install::SatelliteSoftwareVersions::SatelliteSoftwareVersion::InstallPackageInfo::CommittedPackages::Package::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::InstallOpStatuses::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "install-op-status")
    {
        auto ent_ = std::make_shared<NvSatellite::InstallOpStatuses::InstallOpStatus>();
        ent_->parent = this;
        install_op_status.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::InstallOpStatuses::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : install_op_status.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
NvSatellite::InstallOpStatuses::InstallOpStatus::InstallOpStatus()
    ...
    operation_id_xr{YType::uint32, "operation-id-xr"},
    satellite_range{YType::str, "satellite-range"},
    arg_is_reference{YType::boolean, "arg-is-reference"},
    reference_op{YType::boolean, "reference-op"},
    sats_not_initiated{YType::uint16, "sats-not-initiated"},
    sats_transferring{YType::uint16, "sats-transferring"},
    sats_activating{YType::uint16, "sats-activating"},
...
bool NvSatellite::InstallOpStatuses::InstallOpStatus::has_operation() const
{
	...
	|| ydk::is_set(operation_id_xr.yfilter)
	|| ydk::is_set(satellite_range.yfilter)
	|| ydk::is_set(arg_is_reference.yfilter)
	|| ydk::is_set(reference_op.yfilter)
	|| ydk::is_set(sats_not_initiated.yfilter)
	|| ydk::is_set(sats_transferring.yfilter)
	|| ydk::is_set(sats_activating.yfilter)
...
std::vector<std::pair<std::string, LeafData> > NvSatellite::InstallOpStatuses::InstallOpStatus::get_name_leaf_data() const
{
    ...
    if (operation_id_xr.is_set || is_set(operation_id_xr.yfilter)) leaf_name_data.push_back(operation_id_xr.get_name_leafdata());
    if (satellite_range.is_set || is_set(satellite_range.yfilter)) leaf_name_data.push_back(satellite_range.get_name_leafdata());
    if (arg_is_reference.is_set || is_set(arg_is_reference.yfilter)) leaf_name_data.push_back(arg_is_reference.get_name_leafdata());
    if (reference_op.is_set || is_set(reference_op.yfilter)) leaf_name_data.push_back(reference_op.get_name_leafdata());
...
std::shared_ptr<ydk::Entity> NvSatellite::InstallOpStatuses::InstallOpStatus::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::InstallOpStatuses::InstallOpStatus::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
void NvSatellite::InstallOpStatuses::InstallOpStatus::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
    ...
    if(value_path == "arg-is-reference")
    {
        arg_is_reference = value;
        arg_is_reference.value_namespace = name_space;
        arg_is_reference.value_namespace_prefix = name_space_prefix;
    }
    if(value_path == "reference-op")
    {
        reference_op = value;
...
void NvSatellite::InstallOpStatuses::InstallOpStatus::set_filter(const std::string & value_path, YFilter yfilter)
{
    ...
    if(value_path == "arg-is-reference")
    {
        arg_is_reference.yfilter = yfilter;
    }
    if(value_path == "reference-op")
    {
        reference_op.yfilter = yfilter;
...
{
    if(name == "operation-id" || name == "operation-id-xr" || name == "satellite-range" || name == "arg-is-reference" || name == "reference-op" || name == "sats-not-initiated" || name == "sats-transferring" || name == "sats-activating" || name == "sats-updating" || name == "sats-replacing" || name == "sats-deactivating" || name == "sats-removing" || name == "sats-transfer-failed" || name == "sats-activate-failed" || name == "sats-update-failed" || name == "sats-replace-failed" || name == "sats-deactivate-failed" || name == "sats-remove-failed" || name == "sats-transfer-aborted" || name == "sats-activate-aborted" || name == "sats-update-aborted" || name == "sats-replace-aborted" || name == "sats-deactivate-aborted" || name == "sats-remove-aborted" || name == "sats-no-operation" || name == "sats-completed")
        return true;
    return false;
}

NvSatellite::InstallImageReferenceInfo::InstallImageReferenceInfo()
    :
    references(std::make_shared<NvSatellite::InstallImageReferenceInfo::References>())
{
    references->parent = this;

    yang_name = "install-image-reference-info"; yang_parent_name = "nv-satellite"; is_top_level_class = false; has_list_ancestor = false; 
}
...
bool NvSatellite::InstallImageReferenceInfo::has_data() const
{
    if (is_presence_container) return true;
    return (references !=  nullptr && references->has_data());
}

bool NvSatellite::InstallImageReferenceInfo::has_operation() const
{
    return is_set(yfilter)
	|| (references !=  nullptr && references->has_operation());
}

std::string NvSatellite::InstallImageReferenceInfo::get_absolute_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "Cisco-IOS-XR-icpe-infra-oper:nv-satellite/" << get_segment_path();
    return path_buffer.str();
...
std::string NvSatellite::InstallImageReferenceInfo::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "install-image-reference-info";
    return path_buffer.str();
}

std::vector<std::pair<std::string, LeafData> > NvSatellite::InstallImageReferenceInfo::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};
...
std::shared_ptr<ydk::Entity> NvSatellite::InstallImageReferenceInfo::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "references")
    {
        if(references == nullptr)
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::InstallImageReferenceInfo::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(references != nullptr)
...
void NvSatellite::InstallImageReferenceInfo::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
}

void NvSatellite::InstallImageReferenceInfo::set_filter(const std::string & value_path, YFilter yfilter)
{
}

bool NvSatellite::InstallImageReferenceInfo::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "references")
        return true;
    return false;
...
NvSatellite::InstallImageReferenceInfo::References::References()
    :
    reference(this, {"reference_name"})
{

    yang_name = "references"; yang_parent_name = "install-image-reference-info"; is_top_level_class = false; has_list_ancestor = false; 
}
...
bool NvSatellite::InstallImageReferenceInfo::References::has_data() const
{
    if (is_presence_container) return true;
    for (std::size_t index=0; index<reference.len(); index++)
    {
...
bool NvSatellite::InstallImageReferenceInfo::References::has_operation() const
{
    for (std::size_t index=0; index<reference.len(); index++)
    {
        if(reference[index]->has_operation())
...
std::string NvSatellite::InstallImageReferenceInfo::References::get_absolute_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "Cisco-IOS-XR-icpe-infra-oper:nv-satellite/install-image-reference-info/" << get_segment_path();
    return path_buffer.str();
}

std::string NvSatellite::InstallImageReferenceInfo::References::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "references";
    return path_buffer.str();
...
std::vector<std::pair<std::string, LeafData> > NvSatellite::InstallImageReferenceInfo::References::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};
...
std::shared_ptr<ydk::Entity> NvSatellite::InstallImageReferenceInfo::References::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "reference")
    {
        auto ent_ = std::make_shared<NvSatellite::InstallImageReferenceInfo::References::Reference>();
        ent_->parent = this;
        reference.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::InstallImageReferenceInfo::References::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
...
void NvSatellite::InstallImageReferenceInfo::References::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
}

void NvSatellite::InstallImageReferenceInfo::References::set_filter(const std::string & value_path, YFilter yfilter)
{
}

bool NvSatellite::InstallImageReferenceInfo::References::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "reference")
        return true;
    return false;
...
NvSatellite::InstallImageReferenceInfo::References::Reference::Reference()
    :
    reference_name{YType::str, "reference-name"},
    reference_name_xr{YType::str, "reference-name-xr"},
    reference_file{YType::str, "reference-file"}
...
bool NvSatellite::InstallImageReferenceInfo::References::Reference::has_data() const
{
    if (is_presence_container) return true;
    for (auto const & leaf : reference_file.getYLeafs())
    {
...
bool NvSatellite::InstallImageReferenceInfo::References::Reference::has_operation() const
{
    for (auto const & leaf : reference_file.getYLeafs())
    {
        if(is_set(leaf.yfilter))
...
std::string NvSatellite::InstallImageReferenceInfo::References::Reference::get_absolute_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "Cisco-IOS-XR-icpe-infra-oper:nv-satellite/install-image-reference-info/references/" << get_segment_path();
    return path_buffer.str();
}

std::string NvSatellite::InstallImageReferenceInfo::References::Reference::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "reference";
    ADD_KEY_TOKEN(reference_name, "reference-name");
...
std::vector<std::pair<std::string, LeafData> > NvSatellite::InstallImageReferenceInfo::References::Reference::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};
...
std::shared_ptr<ydk::Entity> NvSatellite::InstallImageReferenceInfo::References::Reference::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::InstallImageReferenceInfo::References::Reference::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
void NvSatellite::InstallImageReferenceInfo::References::Reference::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
    if(value_path == "reference-name")
    {
        reference_name = value;
...
void NvSatellite::InstallImageReferenceInfo::References::Reference::set_filter(const std::string & value_path, YFilter yfilter)
{
    if(value_path == "reference-name")
    {
        reference_name.yfilter = yfilter;
...
bool NvSatellite::InstallImageReferenceInfo::References::Reference::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "reference-name" || name == "reference-name-xr" || name == "reference-file")
        return true;
    return false;
...
std::shared_ptr<ydk::Entity> NvSatellite::SatelliteProperties::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatelliteProperties::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(id_ranges != nullptr)
    {
        _children["id-ranges"] = id_ranges;
    }
...
std::shared_ptr<ydk::Entity> NvSatellite::SatelliteProperties::IdRanges::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "id-range")
    {
        auto ent_ = std::make_shared<NvSatellite::SatelliteProperties::IdRanges::IdRange>();
        ent_->parent = this;
        id_range.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatelliteProperties::IdRanges::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : id_range.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> NvSatellite::SatelliteProperties::IdRanges::IdRange::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SatelliteProperties::IdRanges::IdRange::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::SdacpDiscovery2s::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "sdacp-discovery2")
    {
        auto ent_ = std::make_shared<NvSatellite::SdacpDiscovery2s::SdacpDiscovery2>();
        ent_->parent = this;
        sdacp_discovery2.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SdacpDiscovery2s::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : sdacp_discovery2.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> NvSatellite::SdacpDiscovery2s::SdacpDiscovery2::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "interface")
    {
        auto ent_ = std::make_shared<NvSatellite::SdacpDiscovery2s::SdacpDiscovery2::Interface>();
        ent_->parent = this;
    ...
    if(child_yang_name == "satellite")
    {
        auto ent_ = std::make_shared<NvSatellite::SdacpDiscovery2s::SdacpDiscovery2::Satellite>();
        ent_->parent = this;
        satellite.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SdacpDiscovery2s::SdacpDiscovery2::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
...
std::shared_ptr<ydk::Entity> NvSatellite::SdacpDiscovery2s::SdacpDiscovery2::Interface::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SdacpDiscovery2s::SdacpDiscovery2::Interface::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::SdacpDiscovery2s::SdacpDiscovery2::Satellite::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "interface")
    {
        auto ent_ = std::make_shared<NvSatellite::SdacpDiscovery2s::SdacpDiscovery2::Satellite::Interface>();
        ent_->parent = this;
        interface.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SdacpDiscovery2s::SdacpDiscovery2::Satellite::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
...
std::shared_ptr<ydk::Entity> NvSatellite::SdacpDiscovery2s::SdacpDiscovery2::Satellite::Interface::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SdacpDiscovery2s::SdacpDiscovery2::Satellite::Interface::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::IcpeDpms::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "icpe-dpm")
    {
        auto ent_ = std::make_shared<NvSatellite::IcpeDpms::IcpeDpm>();
        ent_->parent = this;
        icpe_dpm.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::IcpeDpms::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : icpe_dpm.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> NvSatellite::IcpeDpms::IcpeDpm::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "satellite")
    {
        auto ent_ = std::make_shared<NvSatellite::IcpeDpms::IcpeDpm::Satellite>();
        ent_->parent = this;
        satellite.append(ent_);
        return ent_;
    }
    ...
    {
        auto ent_ = std::make_shared<NvSatellite::IcpeDpms::IcpeDpm::RemoteHost>();
        ent_->parent = this;
        remote_host.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::IcpeDpms::IcpeDpm::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    ...
    count_ = 0;
    for (auto ent_ : remote_host.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> NvSatellite::IcpeDpms::IcpeDpm::Satellite::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::IcpeDpms::IcpeDpm::Satellite::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::IcpeDpms::IcpeDpm::RemoteHost::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::IcpeDpms::IcpeDpm::RemoteHost::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::SdacpControls::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "sdacp-control")
    {
        auto ent_ = std::make_shared<NvSatellite::SdacpControls::SdacpControl>();
        ent_->parent = this;
        sdacp_control.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SdacpControls::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : sdacp_control.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> NvSatellite::SdacpControls::SdacpControl::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "protocol-state-timestamp")
    {
        if(protocol_state_timestamp == nullptr)
    ...
    if(child_yang_name == "channel")
    {
        auto ent_ = std::make_shared<NvSatellite::SdacpControls::SdacpControl::Channel>();
        ent_->parent = this;
        channel.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SdacpControls::SdacpControl::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(protocol_state_timestamp != nullptr)
    ...
    if(transport_error_timestamp != nullptr)
    {
        _children["transport-error-timestamp"] = transport_error_timestamp;
    }
...
std::shared_ptr<ydk::Entity> NvSatellite::SdacpControls::SdacpControl::ProtocolStateTimestamp::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SdacpControls::SdacpControl::ProtocolStateTimestamp::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::SdacpControls::SdacpControl::TransportErrorTimestamp::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SdacpControls::SdacpControl::TransportErrorTimestamp::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::SdacpControls::SdacpControl::Channel::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SdacpControls::SdacpControl::Channel::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(capabilities != nullptr)
...
std::shared_ptr<ydk::Entity> NvSatellite::SdacpControls::SdacpControl::Channel::Capabilities::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "tl-vs")
    {
        auto ent_ = std::make_shared<NvSatellite::SdacpControls::SdacpControl::Channel::Capabilities::TlVs>();
        ent_->parent = this;
        tl_vs.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SdacpControls::SdacpControl::Channel::Capabilities::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : tl_vs.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> NvSatellite::SdacpControls::SdacpControl::Channel::Capabilities::TlVs::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SdacpControls::SdacpControl::Channel::Capabilities::TlVs::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::SdacpControls::SdacpControl::Channel::ResyncStateTimestamp::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SdacpControls::SdacpControl::Channel::ResyncStateTimestamp::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> NvSatellite::SdacpControls::SdacpControl::Channel::ChannelStateTimestamp::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> NvSatellite::SdacpControls::SdacpControl::Channel::ChannelStateTimestamp::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
