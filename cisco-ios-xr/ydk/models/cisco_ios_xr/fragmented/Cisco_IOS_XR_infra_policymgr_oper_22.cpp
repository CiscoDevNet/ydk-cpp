...
std::shared_ptr<Entity> PolicyManager::Global::TransientPolicyMapAppliedTypes::TransientPolicyMapAppliedType::TransientInterfaceNames::TransientInterfaceName::TransientInput::Detail::PmapVarList::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "var-list")
    {
        auto c = std::make_shared<PolicyManager::Global::TransientPolicyMapAppliedTypes::TransientPolicyMapAppliedType::TransientInterfaceNames::TransientInterfaceName::TransientInput::Detail::PmapVarList::VarList>();
        c->parent = this;
        var_list.append(c);
        return c;
    }
...
std::map<std::string, std::shared_ptr<Entity>> PolicyManager::Global::TransientPolicyMapAppliedTypes::TransientPolicyMapAppliedType::TransientInterfaceNames::TransientInterfaceName::TransientInput::Detail::PmapVarList::get_children() const
{
    std::map<std::string, std::shared_ptr<Entity>> children{};
    char count=0;
    count = 0;
...
std::shared_ptr<Entity> PolicyManager::Global::TransientPolicyMapAppliedTypes::TransientPolicyMapAppliedType::TransientInterfaceNames::TransientInterfaceName::TransientInput::Detail::PmapVarList::VarList::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<Entity>> PolicyManager::Global::TransientPolicyMapAppliedTypes::TransientPolicyMapAppliedType::TransientInterfaceNames::TransientInterfaceName::TransientInput::Detail::PmapVarList::VarList::get_children() const
{
    std::map<std::string, std::shared_ptr<Entity>> children{};
    char count=0;
    return children;
...
std::shared_ptr<Entity> PolicyManager::Global::TargetPolicyMapTypes::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "target-policy-map-type")
    {
        ...
        c->parent = this;
        target_policy_map_type.append(c);
        return c;
    }
...
std::map<std::string, std::shared_ptr<Entity>> PolicyManager::Global::TargetPolicyMapTypes::get_children() const
{
    ...
    char count=0;
    count = 0;
    for (auto c : target_policy_map_type.entities())
    {
        if(children.find(c->get_segment_path()) == children.end())
            children[c->get_segment_path()] = c;
        else
...
std::map<std::string, std::shared_ptr<Entity>> PolicyManager::Global::TargetPolicyMapTypes::TargetPolicyMapType::PolicyMapTargets::get_children() const
{
    ...
    char count=0;
    count = 0;
    for (auto c : policy_map_target.entities())
    {
        if(children.find(c->get_segment_path()) == children.end())
            children[c->get_segment_path()] = c;
        else
...
std::shared_ptr<Entity> PolicyManager::Global::TargetPolicyMapTypes::TargetPolicyMapType::TargetSummaryPerPolicyMaps::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "target-summary-per-policy-map")
    {
        ...
        c->parent = this;
        target_summary_per_policy_map.append(c);
        return c;
    }
...
std::map<std::string, std::shared_ptr<Entity>> PolicyManager::Global::TargetPolicyMapTypes::TargetPolicyMapType::TargetSummaryPerPolicyMaps::get_children() const
{
    ...
    char count=0;
    count = 0;
    for (auto c : target_summary_per_policy_map.entities())
    {
        if(children.find(c->get_segment_path()) == children.end())
            children[c->get_segment_path()] = c;
        else
...
std::map<std::string, std::shared_ptr<Entity>> PolicyManager::Global::ClassMap::ClassMapTypes::get_children() const
{
    ...
    char count=0;
    count = 0;
    for (auto c : class_map_type.entities())
    {
        if(children.find(c->get_segment_path()) == children.end())
            children[c->get_segment_path()] = c;
        else
...
std::map<std::string, std::shared_ptr<Entity>> PolicyManager::Global::ClassMap::ClassMapTypes::ClassMapType::ClassMapDetails::get_children() const
{
    ...
    char count=0;
    count = 0;
    for (auto c : class_map_detail.entities())
    {
        if(children.find(c->get_segment_path()) == children.end())
            children[c->get_segment_path()] = c;
        else
