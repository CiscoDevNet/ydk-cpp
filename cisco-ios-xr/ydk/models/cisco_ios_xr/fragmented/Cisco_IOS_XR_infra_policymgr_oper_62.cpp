...
std::shared_ptr<Entity> PolicyManager::Nodes::Node::PolicyMap::PolicyMapUnusedTypes::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "policy-map-unused-type")
    {
        ...
        c->parent = this;
        policy_map_unused_type.append(c);
        return c;
    }
...
std::map<std::string, std::shared_ptr<Entity>> PolicyManager::Nodes::Node::PolicyMap::PolicyMapUnusedTypes::get_children() const
{
    ...
    char count=0;
    count = 0;
    for (auto c : policy_map_unused_type.entities())
    {
        if(children.find(c->get_segment_path()) == children.end())
            children[c->get_segment_path()] = c;
        else
...
std::map<std::string, std::shared_ptr<Entity>> PolicyManager::Nodes::Node::PolicyMap::PolicyMapTypes::get_children() const
{
    ...
    char count=0;
    count = 0;
    for (auto c : policy_map_type.entities())
    {
        if(children.find(c->get_segment_path()) == children.end())
            children[c->get_segment_path()] = c;
        else
