...
std::map<std::string, std::shared_ptr<ydk::Entity>> HwModule::get_children() const
{
    ...
    if(slice_ids != nullptr)
    {
        _children["slice-ids"] = slice_ids;
    }

    if(slice_all != nullptr)
    {
        _children["slice-all"] = slice_all;
    }
...
std::shared_ptr<ydk::Entity> HwModule::SliceIds::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "slice-id")
    {
        auto ent_ = std::make_shared<HwModule::SliceIds::SliceId>();
        ent_->parent = this;
        slice_id.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> HwModule::SliceIds::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : slice_id.entities())
    {
...
{
    if (is_presence_container) return true;
    for (std::size_t index=0; index<client_port.len(); index++)
    {
...
std::string HwModule::SliceIds::SliceId::get_segment_path() const
{
    ...
    ADD_KEY_TOKEN(slice_num, "slice-num");
    return path_buffer.str();
}
...
std::shared_ptr<ydk::Entity> HwModule::SliceIds::SliceId::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "client-port")
    {
        auto ent_ = std::make_shared<HwModule::SliceIds::SliceId::ClientPort>();
        ent_->parent = this;
        client_port.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> HwModule::SliceIds::SliceId::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : client_port.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
{
    if (is_presence_container) return true;
    for (std::size_t index=0; index<trunk_port.len(); index++)
    {
...
std::shared_ptr<ydk::Entity> HwModule::SliceIds::SliceId::ClientPort::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "trunk-port")
    {
        auto ent_ = std::make_shared<HwModule::SliceIds::SliceId::ClientPort::TrunkPort>();
        ent_->parent = this;
        trunk_port.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> HwModule::SliceIds::SliceId::ClientPort::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : trunk_port.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> HwModule::SliceIds::SliceId::ClientPort::TrunkPort::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HwModule::SliceIds::SliceId::ClientPort::TrunkPort::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
{
    if (is_presence_container) return true;
    for (std::size_t index=0; index<slice_info.len(); index++)
    {
...
std::shared_ptr<ydk::Entity> HwModule::SliceAll::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "slice-info")
    {
        auto ent_ = std::make_shared<HwModule::SliceAll::SliceInfo>();
        ent_->parent = this;
        slice_info.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> HwModule::SliceAll::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : slice_info.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> HwModule::SliceAll::SliceInfo::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "client-port")
    {
        auto ent_ = std::make_shared<HwModule::SliceAll::SliceInfo::ClientPort>();
        ent_->parent = this;
        client_port.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> HwModule::SliceAll::SliceInfo::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
...
std::shared_ptr<ydk::Entity> HwModule::SliceAll::SliceInfo::ClientPort::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "trunk-port")
    {
        auto ent_ = std::make_shared<HwModule::SliceAll::SliceInfo::ClientPort::TrunkPort>();
        ent_->parent = this;
        trunk_port.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> HwModule::SliceAll::SliceInfo::ClientPort::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
...
std::shared_ptr<ydk::Entity> HwModule::SliceAll::SliceInfo::ClientPort::TrunkPort::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HwModule::SliceAll::SliceInfo::ClientPort::TrunkPort::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
