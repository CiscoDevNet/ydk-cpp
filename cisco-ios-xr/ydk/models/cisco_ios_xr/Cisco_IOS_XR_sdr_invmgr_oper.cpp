...
SdrInventory::SdrInventory()
    ...
    , memory(std::make_shared<SdrInventory::Memory>())
{
    racks->parent = this;
    memory->parent = this;
...
SdrInventory::Memory::Memory()
    :
    racks(std::make_shared<SdrInventory::Memory::Racks>())
{
    racks->parent = this;

    yang_name = "memory"; yang_parent_name = "sdr-inventory"; is_top_level_class = false; has_list_ancestor = false; 
}
...
bool SdrInventory::Memory::has_data() const
{
    if (is_presence_container) return true;
    return (racks !=  nullptr && racks->has_data());
}

bool SdrInventory::Memory::has_operation() const
{
    return is_set(yfilter)
	|| (racks !=  nullptr && racks->has_operation());
}

std::string SdrInventory::Memory::get_absolute_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "Cisco-IOS-XR-sdr-invmgr-oper:sdr-inventory/" << get_segment_path();
    return path_buffer.str();
...
std::string SdrInventory::Memory::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "memory";
    return path_buffer.str();
...
std::vector<std::pair<std::string, LeafData> > SdrInventory::Memory::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};
...
std::shared_ptr<ydk::Entity> SdrInventory::Memory::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "racks")
    {
        if(racks == nullptr)
...
std::map<std::string, std::shared_ptr<ydk::Entity>> SdrInventory::Memory::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(racks != nullptr)
...
void SdrInventory::Memory::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
}

void SdrInventory::Memory::set_filter(const std::string & value_path, YFilter yfilter)
{
}

bool SdrInventory::Memory::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "racks")
        return true;
    return false;
...
SdrInventory::Memory::Racks::Racks()
    :
    rack(this, {"name"})
{

    yang_name = "racks"; yang_parent_name = "memory"; is_top_level_class = false; has_list_ancestor = false; 
}
...
bool SdrInventory::Memory::Racks::has_data() const
{
    if (is_presence_container) return true;
    for (std::size_t index=0; index<rack.len(); index++)
    {
...
bool SdrInventory::Memory::Racks::has_operation() const
{
    for (std::size_t index=0; index<rack.len(); index++)
    {
        if(rack[index]->has_operation())
...
std::string SdrInventory::Memory::Racks::get_absolute_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "Cisco-IOS-XR-sdr-invmgr-oper:sdr-inventory/memory/" << get_segment_path();
    return path_buffer.str();
}

std::string SdrInventory::Memory::Racks::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "racks";
    return path_buffer.str();
...
std::vector<std::pair<std::string, LeafData> > SdrInventory::Memory::Racks::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};
...
std::shared_ptr<ydk::Entity> SdrInventory::Memory::Racks::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "rack")
    {
        auto ent_ = std::make_shared<SdrInventory::Memory::Racks::Rack>();
        ent_->parent = this;
        rack.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> SdrInventory::Memory::Racks::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
...
void SdrInventory::Memory::Racks::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
}

void SdrInventory::Memory::Racks::set_filter(const std::string & value_path, YFilter yfilter)
{
}

bool SdrInventory::Memory::Racks::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "rack")
        return true;
    return false;
...
SdrInventory::Memory::Racks::Rack::Rack()
    :
    name{YType::str, "name"}
        ,
    node_ids(std::make_shared<SdrInventory::Memory::Racks::Rack::NodeIds>())
{
    node_ids->parent = this;
...
bool SdrInventory::Memory::Racks::Rack::has_data() const
{
    if (is_presence_container) return true;
    return name.is_set
	|| (node_ids !=  nullptr && node_ids->has_data());
}

bool SdrInventory::Memory::Racks::Rack::has_operation() const
{
    return is_set(yfilter)
	|| ydk::is_set(name.yfilter)
	|| (node_ids !=  nullptr && node_ids->has_operation());
}

std::string SdrInventory::Memory::Racks::Rack::get_absolute_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "Cisco-IOS-XR-sdr-invmgr-oper:sdr-inventory/memory/racks/" << get_segment_path();
    return path_buffer.str();
}

std::string SdrInventory::Memory::Racks::Rack::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "rack";
    ADD_KEY_TOKEN(name, "name");
...
std::vector<std::pair<std::string, LeafData> > SdrInventory::Memory::Racks::Rack::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};
...
std::shared_ptr<ydk::Entity> SdrInventory::Memory::Racks::Rack::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "node-ids")
    {
        if(node_ids == nullptr)
        {
            node_ids = std::make_shared<SdrInventory::Memory::Racks::Rack::NodeIds>();
...
std::map<std::string, std::shared_ptr<ydk::Entity>> SdrInventory::Memory::Racks::Rack::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(node_ids != nullptr)
    {
        _children["node-ids"] = node_ids;
    }

    return _children;
}

void SdrInventory::Memory::Racks::Rack::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
    if(value_path == "name")
    {
        name = value;
...
void SdrInventory::Memory::Racks::Rack::set_filter(const std::string & value_path, YFilter yfilter)
{
    if(value_path == "name")
    {
        name.yfilter = yfilter;
...
bool SdrInventory::Memory::Racks::Rack::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "node-ids" || name == "name")
        return true;
    return false;
}

SdrInventory::Memory::Racks::Rack::NodeIds::NodeIds()
    :
    node_id(this, {"name"})
{

    yang_name = "node-ids"; yang_parent_name = "rack"; is_top_level_class = false; has_list_ancestor = true; 
}
...
bool SdrInventory::Memory::Racks::Rack::NodeIds::has_data() const
{
    if (is_presence_container) return true;
    for (std::size_t index=0; index<node_id.len(); index++)
    {
        if(node_id[index]->has_data())
            return true;
    }
    return false;
}

bool SdrInventory::Memory::Racks::Rack::NodeIds::has_operation() const
{
    for (std::size_t index=0; index<node_id.len(); index++)
    {
        if(node_id[index]->has_operation())
            return true;
    }
    return is_set(yfilter);
}

std::string SdrInventory::Memory::Racks::Rack::NodeIds::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "node-ids";
    return path_buffer.str();
}

std::vector<std::pair<std::string, LeafData> > SdrInventory::Memory::Racks::Rack::NodeIds::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};
...
std::shared_ptr<ydk::Entity> SdrInventory::Memory::Racks::Rack::NodeIds::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "node-id")
    {
        auto ent_ = std::make_shared<SdrInventory::Memory::Racks::Rack::NodeIds::NodeId>();
        ent_->parent = this;
        node_id.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> SdrInventory::Memory::Racks::Rack::NodeIds::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : node_id.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
void SdrInventory::Memory::Racks::Rack::NodeIds::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
}

void SdrInventory::Memory::Racks::Rack::NodeIds::set_filter(const std::string & value_path, YFilter yfilter)
{
}

bool SdrInventory::Memory::Racks::Rack::NodeIds::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "node-id")
        return true;
    return false;
...
SdrInventory::Memory::Racks::Rack::NodeIds::NodeId::NodeId()
    :
    name{YType::str, "name"},
    total_memory{YType::uint32, "total-memory"},
    available_memory{YType::uint32, "available-memory"}
{

    yang_name = "node-id"; yang_parent_name = "node-ids"; is_top_level_class = false; has_list_ancestor = true; 
}
...
bool SdrInventory::Memory::Racks::Rack::NodeIds::NodeId::has_data() const
{
    if (is_presence_container) return true;
    return name.is_set
	|| total_memory.is_set
	|| available_memory.is_set;
}

bool SdrInventory::Memory::Racks::Rack::NodeIds::NodeId::has_operation() const
{
    return is_set(yfilter)
	|| ydk::is_set(name.yfilter)
	|| ydk::is_set(total_memory.yfilter)
	|| ydk::is_set(available_memory.yfilter);
}

std::string SdrInventory::Memory::Racks::Rack::NodeIds::NodeId::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "node-id";
    ADD_KEY_TOKEN(name, "name");
...
std::vector<std::pair<std::string, LeafData> > SdrInventory::Memory::Racks::Rack::NodeIds::NodeId::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};

    if (name.is_set || is_set(name.yfilter)) leaf_name_data.push_back(name.get_name_leafdata());
    if (total_memory.is_set || is_set(total_memory.yfilter)) leaf_name_data.push_back(total_memory.get_name_leafdata());
    if (available_memory.is_set || is_set(available_memory.yfilter)) leaf_name_data.push_back(available_memory.get_name_leafdata());

    return leaf_name_data;

}

std::shared_ptr<ydk::Entity> SdrInventory::Memory::Racks::Rack::NodeIds::NodeId::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> SdrInventory::Memory::Racks::Rack::NodeIds::NodeId::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
void SdrInventory::Memory::Racks::Rack::NodeIds::NodeId::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
    if(value_path == "name")
    {
        name = value;
    ...
    if(value_path == "available-memory")
    {
        available_memory = value;
        available_memory.value_namespace = name_space;
        available_memory.value_namespace_prefix = name_space_prefix;
    }
}

void SdrInventory::Memory::Racks::Rack::NodeIds::NodeId::set_filter(const std::string & value_path, YFilter yfilter)
{
    if(value_path == "name")
    {
        name.yfilter = yfilter;
    ...
    if(value_path == "available-memory")
    {
        available_memory.yfilter = yfilter;
    }
}

bool SdrInventory::Memory::Racks::Rack::NodeIds::NodeId::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "name" || name == "total-memory" || name == "available-memory")
        return true;
    return false;
}
