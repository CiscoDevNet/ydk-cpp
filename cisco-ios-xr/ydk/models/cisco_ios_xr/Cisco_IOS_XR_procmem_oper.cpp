...
std::shared_ptr<Entity> ProcessesMemory::Nodes::Node::ProcessIds::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    ...
    if(child_yang_name == "process-id")
    {
        ...
        children[segment_path] = process_id.back();
        return children.at(segment_path);
    }
...
std::map<std::string, std::shared_ptr<Entity>> & ProcessesMemory::Nodes::Node::ProcessIds::get_children()
{
    for (auto const & c : process_id)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
std::map<std::string, std::shared_ptr<Entity>> & ProcessesMemory::Nodes::Node::ProcessIds::ProcessId::get_children()
{
    return children;
}
