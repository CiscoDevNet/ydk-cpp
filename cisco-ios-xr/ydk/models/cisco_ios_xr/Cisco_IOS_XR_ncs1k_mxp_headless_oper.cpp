...
{
    if (is_presence_container) return true;
    for (std::size_t index=0; index<otn_port_name.len(); index++)
    {
...
{
    if (is_presence_container) return true;
    for (std::size_t index=0; index<ethernet_port_name.len(); index++)
    {
...
{
    std::ostringstream path_buffer;
    path_buffer << "ethernet-port-name";
    ADD_KEY_TOKEN(name, "name");
    return path_buffer.str();
}
