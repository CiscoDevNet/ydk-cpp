...
{
    if(name == "fan-traies" || name == "power-supplies" || name == "slots" || name == "chassis" || name == "summary" || name == "rack-name")
        return true;
    return false;
}
...
std::map<std::string, std::shared_ptr<Entity>> Diag::Racks::Rack::FanTraies::FanTray::Detail::Node::HardwareRevision::get_children() const
{
    ...
    char count=0;
    count = 0;
    for (auto c : hardware_revision.entities())
    {
        if(children.find(c->get_segment_path()) == children.end())
            children[c->get_segment_path()] = c;
        else
...
std::map<std::string, std::shared_ptr<Entity>> Diag::Racks::Rack::Slots::Slot::Detail::get_children() const
{
    ...
    char count=0;
    count = 0;
    for (auto c : node_detail.entities())
    {
        if(children.find(c->get_segment_path()) == children.end())
            children[c->get_segment_path()] = c;
        else
    ...
    count = 0;
    for (auto c : spa_detail.entities())
    {
        if(children.find(c->get_segment_path()) == children.end())
            children[c->get_segment_path()] = c;
        else
