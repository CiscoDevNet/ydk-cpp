...
std::shared_ptr<ydk::Entity> HardwareModuleNp::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::get_children() const
{
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::clone_ptr() const
{
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "node")
    {
        auto ent_ = std::make_shared<HardwareModuleNp::Nodes::Node>();
        ent_->parent = this;
        node.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(nps != nullptr)
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "np")
    {
        auto ent_ = std::make_shared<HardwareModuleNp::Nodes::Node::Nps::Np>();
        ent_->parent = this;
        np.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
...
HardwareModuleNp::Nodes::Node::Nps::Np::Np()
    ...
    np_name{YType::str, "np-name"}
        ,
    np_uidb(std::make_shared<HardwareModuleNp::Nodes::Node::Nps::Np::NpUidb>())
    , chn_load(std::make_shared<HardwareModuleNp::Nodes::Node::Nps::Np::ChnLoad>())
    , load_utilization(std::make_shared<HardwareModuleNp::Nodes::Node::Nps::Np::LoadUtilization>())
    , tcam_summary(std::make_shared<HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary>())
    , l2rm_hw_resource(std::make_shared<HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource>())
    , profile(std::make_shared<HardwareModuleNp::Nodes::Node::Nps::Np::Profile>())
    ...
    , efd(std::make_shared<HardwareModuleNp::Nodes::Node::Nps::Np::Efd>())
{
    np_uidb->parent = this;
    chn_load->parent = this;
    load_utilization->parent = this;
    tcam_summary->parent = this;
    l2rm_hw_resource->parent = this;
    profile->parent = this;
...
bool HardwareModuleNp::Nodes::Node::Nps::Np::has_data() const
{
	...
	|| (np_uidb !=  nullptr && np_uidb->has_data())
	|| (chn_load !=  nullptr && chn_load->has_data())
	|| (load_utilization !=  nullptr && load_utilization->has_data())
	|| (tcam_summary !=  nullptr && tcam_summary->has_data())
	|| (l2rm_hw_resource !=  nullptr && l2rm_hw_resource->has_data())
	|| (profile !=  nullptr && profile->has_data())
	|| (counters !=  nullptr && counters->has_data())
	|| (fast_drop !=  nullptr && fast_drop->has_data())
	|| (efd !=  nullptr && efd->has_data());
}
...
bool HardwareModuleNp::Nodes::Node::Nps::Np::has_operation() const
{
	...
	|| (np_uidb !=  nullptr && np_uidb->has_operation())
	|| (chn_load !=  nullptr && chn_load->has_operation())
	|| (load_utilization !=  nullptr && load_utilization->has_operation())
	|| (tcam_summary !=  nullptr && tcam_summary->has_operation())
	|| (l2rm_hw_resource !=  nullptr && l2rm_hw_resource->has_operation())
	|| (profile !=  nullptr && profile->has_operation())
	|| (counters !=  nullptr && counters->has_operation())
	|| (fast_drop !=  nullptr && fast_drop->has_operation())
	|| (efd !=  nullptr && efd->has_operation());
}
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "np-uidb")
    {
        if(np_uidb == nullptr)
        {
            np_uidb = std::make_shared<HardwareModuleNp::Nodes::Node::Nps::Np::NpUidb>();
    ...
    if(child_yang_name == "load-utilization")
    {
        if(load_utilization == nullptr)
        {
            load_utilization = std::make_shared<HardwareModuleNp::Nodes::Node::Nps::Np::LoadUtilization>();
        }
    ...
    if(child_yang_name == "l2rm-hw-resource")
    {
        if(l2rm_hw_resource == nullptr)
        {
            l2rm_hw_resource = std::make_shared<HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource>();
        }
        return l2rm_hw_resource;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(np_uidb != nullptr)
    {
        _children["np-uidb"] = np_uidb;
    }

    if(chn_load != nullptr)
    {
        _children["chn-load"] = chn_load;
    }

    if(load_utilization != nullptr)
    {
        _children["load-utilization"] = load_utilization;
    }

    if(tcam_summary != nullptr)
    {
        _children["tcam-summary"] = tcam_summary;
    }

    if(l2rm_hw_resource != nullptr)
    {
        _children["l2rm-hw-resource"] = l2rm_hw_resource;
    }
...
{
    if(name == "np-uidb" || name == "chn-load" || name == "load-utilization" || name == "tcam-summary" || name == "l2rm-hw-resource" || name == "profile" || name == "counters" || name == "fast-drop" || name == "efd" || name == "np-name")
        return true;
    return false;
}

HardwareModuleNp::Nodes::Node::Nps::Np::NpUidb::NpUidb()
    :
    uidb_index(this, {})
{

    yang_name = "np-uidb"; yang_parent_name = "np"; is_top_level_class = false; has_list_ancestor = true; 
}
...
bool HardwareModuleNp::Nodes::Node::Nps::Np::NpUidb::has_data() const
{
    if (is_presence_container) return true;
    for (std::size_t index=0; index<uidb_index.len(); index++)
    {
        if(uidb_index[index]->has_data())
            return true;
    }
    return false;
}

bool HardwareModuleNp::Nodes::Node::Nps::Np::NpUidb::has_operation() const
{
    for (std::size_t index=0; index<uidb_index.len(); index++)
    {
        if(uidb_index[index]->has_operation())
            return true;
    }
    return is_set(yfilter);
}

std::string HardwareModuleNp::Nodes::Node::Nps::Np::NpUidb::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "np-uidb";
    return path_buffer.str();
...
std::vector<std::pair<std::string, LeafData> > HardwareModuleNp::Nodes::Node::Nps::Np::NpUidb::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::NpUidb::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "uidb-index")
    {
        auto ent_ = std::make_shared<HardwareModuleNp::Nodes::Node::Nps::Np::NpUidb::UidbIndex>();
        ent_->parent = this;
        uidb_index.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::NpUidb::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : uidb_index.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
void HardwareModuleNp::Nodes::Node::Nps::Np::NpUidb::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
}

void HardwareModuleNp::Nodes::Node::Nps::Np::NpUidb::set_filter(const std::string & value_path, YFilter yfilter)
{
}

bool HardwareModuleNp::Nodes::Node::Nps::Np::NpUidb::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "uidb-index")
        return true;
    return false;
...
HardwareModuleNp::Nodes::Node::Nps::Np::NpUidb::UidbIndex::UidbIndex()
    :
    ...
    interface_type{YType::str, "interface-type"}
{
    ...
    yang_name = "uidb-index"; yang_parent_name = "np-uidb"; is_top_level_class = false; has_list_ancestor = true; 
}
...
bool HardwareModuleNp::Nodes::Node::Nps::Np::NpUidb::UidbIndex::has_data() const
{
    if (is_presence_container) return true;
...
bool HardwareModuleNp::Nodes::Node::Nps::Np::NpUidb::UidbIndex::has_operation() const
{
    return is_set(yfilter)
...
std::string HardwareModuleNp::Nodes::Node::Nps::Np::NpUidb::UidbIndex::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "uidb-index";
    path_buffer << "[" << get_ylist_key() << "]";
    return path_buffer.str();
}

std::vector<std::pair<std::string, LeafData> > HardwareModuleNp::Nodes::Node::Nps::Np::NpUidb::UidbIndex::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::NpUidb::UidbIndex::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::NpUidb::UidbIndex::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
void HardwareModuleNp::Nodes::Node::Nps::Np::NpUidb::UidbIndex::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
...
void HardwareModuleNp::Nodes::Node::Nps::Np::NpUidb::UidbIndex::set_filter(const std::string & value_path, YFilter yfilter)
{
    if(value_path == "interface-name")
    {
...
bool HardwareModuleNp::Nodes::Node::Nps::Np::NpUidb::UidbIndex::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "interface-name" || name == "interface-handle" || name == "index" || name == "interface-type")
        return true;
    return false;
}
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::ChnLoad::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "np-chn-load")
    {
        auto ent_ = std::make_shared<HardwareModuleNp::Nodes::Node::Nps::Np::ChnLoad::NpChnLoad>();
        ent_->parent = this;
        np_chn_load.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::ChnLoad::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : np_chn_load.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::ChnLoad::NpChnLoad::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::ChnLoad::NpChnLoad::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
HardwareModuleNp::Nodes::Node::Nps::Np::LoadUtilization::LoadUtilization()
    :
    utilization{YType::uint32, "utilization"},
    packet_per_second_rate{YType::uint32, "packet-per-second-rate"}
{

    yang_name = "load-utilization"; yang_parent_name = "np"; is_top_level_class = false; has_list_ancestor = true; 
}
...
bool HardwareModuleNp::Nodes::Node::Nps::Np::LoadUtilization::has_data() const
{
    if (is_presence_container) return true;
    return utilization.is_set
	|| packet_per_second_rate.is_set;
}

bool HardwareModuleNp::Nodes::Node::Nps::Np::LoadUtilization::has_operation() const
{
    return is_set(yfilter)
	|| ydk::is_set(utilization.yfilter)
	|| ydk::is_set(packet_per_second_rate.yfilter);
}

std::string HardwareModuleNp::Nodes::Node::Nps::Np::LoadUtilization::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "load-utilization";
    return path_buffer.str();
}

std::vector<std::pair<std::string, LeafData> > HardwareModuleNp::Nodes::Node::Nps::Np::LoadUtilization::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};

    if (utilization.is_set || is_set(utilization.yfilter)) leaf_name_data.push_back(utilization.get_name_leafdata());
    if (packet_per_second_rate.is_set || is_set(packet_per_second_rate.yfilter)) leaf_name_data.push_back(packet_per_second_rate.get_name_leafdata());

    return leaf_name_data;

}

std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::LoadUtilization::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::LoadUtilization::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
void HardwareModuleNp::Nodes::Node::Nps::Np::LoadUtilization::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
    ...
    if(value_path == "packet-per-second-rate")
    {
        packet_per_second_rate = value;
        packet_per_second_rate.value_namespace = name_space;
        packet_per_second_rate.value_namespace_prefix = name_space_prefix;
    }
}

void HardwareModuleNp::Nodes::Node::Nps::Np::LoadUtilization::set_filter(const std::string & value_path, YFilter yfilter)
{
    if(value_path == "utilization")
    {
    ...
    if(value_path == "packet-per-second-rate")
    {
        packet_per_second_rate.yfilter = yfilter;
    }
}

bool HardwareModuleNp::Nodes::Node::Nps::Np::LoadUtilization::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "utilization" || name == "packet-per-second-rate")
        return true;
    return false;
}
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(internal_tcam_info != nullptr)
    {
        _children["internal-tcam-info"] = internal_tcam_info;
    }

    if(tcam_info != nullptr)
    {
        _children["tcam-info"] = tcam_info;
    }
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    ...
    {
        auto ent_ = std::make_shared<HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtL2>();
        ent_->parent = this;
        tcam_lt_l2.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(tcam_lt_ods2 != nullptr)
    {
        _children["tcam-lt-ods2"] = tcam_lt_ods2;
    }

    if(tcam_lt_ods8 != nullptr)
    {
        _children["tcam-lt-ods8"] = tcam_lt_ods8;
    }

    count_ = 0;
    for (auto ent_ : tcam_lt_l2.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds2::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds2::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(app_id_ifib != nullptr)
    {
        _children["app-id-ifib"] = app_id_ifib;
    }

    if(app_id_qos != nullptr)
    {
        _children["app-id-qos"] = app_id_qos;
    }

    if(app_id_acl != nullptr)
    {
        _children["app-id-acl"] = app_id_acl;
    }

    if(app_id_afmon != nullptr)
    {
        _children["app-id-afmon"] = app_id_afmon;
    }

    if(app_id_li != nullptr)
    {
        _children["app-id-li"] = app_id_li;
    }

    if(app_id_pbr != nullptr)
    {
        _children["app-id-pbr"] = app_id_pbr;
    }

    if(application_edpl_entry != nullptr)
    {
        _children["application-edpl-entry"] = application_edpl_entry;
    }
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds2::AppIdIfib::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds2::AppIdIfib::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds2::AppIdQos::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds2::AppIdQos::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds2::AppIdAcl::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds2::AppIdAcl::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds2::AppIdAfmon::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds2::AppIdAfmon::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds2::AppIdLi::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds2::AppIdLi::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds2::AppIdPbr::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds2::AppIdPbr::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds2::ApplicationEdplEntry::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds2::ApplicationEdplEntry::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds8::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "app-id-ifib")
    {
        if(app_id_ifib == nullptr)
...
std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds8::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(app_id_ifib != nullptr)
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds8::AppIdIfib::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds8::AppIdIfib::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds8::AppIdQos::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds8::AppIdQos::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds8::AppIdAcl::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds8::AppIdAcl::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds8::AppIdAfmon::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds8::AppIdAfmon::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds8::AppIdLi::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds8::AppIdLi::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds8::AppIdPbr::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds8::AppIdPbr::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds8::ApplicationEdplEntry::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtOds8::ApplicationEdplEntry::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtL2::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::InternalTcamInfo::TcamLtL2::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "tcam-lt-ods2")
    {
        if(tcam_lt_ods2 == nullptr)
    ...
    if(child_yang_name == "tcam-lt-l2")
    {
        auto ent_ = std::make_shared<HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtL2>();
        ent_->parent = this;
        tcam_lt_l2.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(tcam_lt_ods2 != nullptr)
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds2::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds2::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(acl_common != nullptr)
    {
        _children["acl-common"] = acl_common;
    }
    ...
    if(app_id_edpl != nullptr)
    {
        _children["app-id-edpl"] = app_id_edpl;
    }
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds2::AclCommon::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds2::AclCommon::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds2::AppIdIfib::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds2::AppIdIfib::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds2::AppIdQos::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds2::AppIdQos::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds2::AppIdAcl::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds2::AppIdAcl::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds2::AppIdAfmon::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds2::AppIdAfmon::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds2::AppIdLi::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds2::AppIdLi::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds2::AppIdPbr::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds2::AppIdPbr::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds2::AppIdEdpl::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds2::AppIdEdpl::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds8::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "acl-common")
    {
        if(acl_common == nullptr)
...
std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds8::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(acl_common != nullptr)
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds8::AclCommon::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds8::AclCommon::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds8::AppIdIfib::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds8::AppIdIfib::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds8::AppIdQos::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds8::AppIdQos::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds8::AppIdAcl::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds8::AppIdAcl::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds8::AppIdAfmon::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds8::AppIdAfmon::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds8::AppIdLi::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds8::AppIdLi::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds8::AppIdPbr::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds8::AppIdPbr::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds8::AppIdEdpl::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtOds8::AppIdEdpl::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtL2::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::TcamSummary::TcamInfo::TcamLtL2::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::L2rmHwResource()
    :
    ppt_alloc{YType::uint64, "ppt-alloc"},
    ppt_write{YType::uint64, "ppt-write"},
    ppt_free{YType::uint64, "ppt-free"}
        ,
    hw_resource(std::make_shared<HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource>())
{
    hw_resource->parent = this;

    yang_name = "l2rm-hw-resource"; yang_parent_name = "np"; is_top_level_class = false; has_list_ancestor = true; 
}
...
bool HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::has_data() const
{
    if (is_presence_container) return true;
    return ppt_alloc.is_set
	|| ppt_write.is_set
	|| ppt_free.is_set
	|| (hw_resource !=  nullptr && hw_resource->has_data());
}

bool HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::has_operation() const
{
    return is_set(yfilter)
	|| ydk::is_set(ppt_alloc.yfilter)
	|| ydk::is_set(ppt_write.yfilter)
	|| ydk::is_set(ppt_free.yfilter)
	|| (hw_resource !=  nullptr && hw_resource->has_operation());
}

std::string HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "l2rm-hw-resource";
    return path_buffer.str();
}

std::vector<std::pair<std::string, LeafData> > HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};

    if (ppt_alloc.is_set || is_set(ppt_alloc.yfilter)) leaf_name_data.push_back(ppt_alloc.get_name_leafdata());
    if (ppt_write.is_set || is_set(ppt_write.yfilter)) leaf_name_data.push_back(ppt_write.get_name_leafdata());
    if (ppt_free.is_set || is_set(ppt_free.yfilter)) leaf_name_data.push_back(ppt_free.get_name_leafdata());

    return leaf_name_data;

}

std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "hw-resource")
    {
...
std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(hw_resource != nullptr)
    {
        _children["hw-resource"] = hw_resource;
    }

    return _children;
}

void HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
    if(value_path == "ppt-alloc")
    {
        ppt_alloc = value;
        ppt_alloc.value_namespace = name_space;
        ppt_alloc.value_namespace_prefix = name_space_prefix;
    }
    if(value_path == "ppt-write")
    {
        ppt_write = value;
        ppt_write.value_namespace = name_space;
        ppt_write.value_namespace_prefix = name_space_prefix;
    }
    if(value_path == "ppt-free")
    {
        ppt_free = value;
        ppt_free.value_namespace = name_space;
        ppt_free.value_namespace_prefix = name_space_prefix;
    }
}

void HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::set_filter(const std::string & value_path, YFilter yfilter)
{
    if(value_path == "ppt-alloc")
    {
        ppt_alloc.yfilter = yfilter;
    }
    if(value_path == "ppt-write")
    {
        ppt_write.yfilter = yfilter;
    }
    if(value_path == "ppt-free")
    {
        ppt_free.yfilter = yfilter;
    }
}

bool HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "hw-resource" || name == "ppt-alloc" || name == "ppt-write" || name == "ppt-free")
        return true;
    return false;
}

HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::HwResource()
    :
    hash_block(this, {})
    , tcam_partition(this, {})
{

    yang_name = "hw-resource"; yang_parent_name = "l2rm-hw-resource"; is_top_level_class = false; has_list_ancestor = true; 
}
...
bool HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::has_data() const
{
    if (is_presence_container) return true;
    for (std::size_t index=0; index<hash_block.len(); index++)
    {
        if(hash_block[index]->has_data())
            return true;
    }
    for (std::size_t index=0; index<tcam_partition.len(); index++)
    {
        if(tcam_partition[index]->has_data())
            return true;
    }
    return false;
}

bool HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::has_operation() const
{
    for (std::size_t index=0; index<hash_block.len(); index++)
    {
        if(hash_block[index]->has_operation())
            return true;
    }
    for (std::size_t index=0; index<tcam_partition.len(); index++)
    {
        if(tcam_partition[index]->has_operation())
            return true;
    }
    return is_set(yfilter);
}

std::string HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "hw-resource";
    return path_buffer.str();
}

std::vector<std::pair<std::string, LeafData> > HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "hash-block")
    {
        auto ent_ = std::make_shared<HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::HashBlock>();
        ent_->parent = this;
        hash_block.append(ent_);
        return ent_;
    }

    if(child_yang_name == "tcam-partition")
    {
        auto ent_ = std::make_shared<HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::TcamPartition>();
        ent_->parent = this;
        tcam_partition.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : hash_block.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
    ...
    count_ = 0;
    for (auto ent_ : tcam_partition.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
void HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
}

void HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::set_filter(const std::string & value_path, YFilter yfilter)
{
}

bool HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "hash-block" || name == "tcam-partition")
        return true;
    return false;
}

HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::HashBlock::HashBlock()
    :
    hash_blk{YType::uint32, "hash-blk"},
    total{YType::uint32, "total"},
    free{YType::uint32, "free"}
{

    yang_name = "hash-block"; yang_parent_name = "hw-resource"; is_top_level_class = false; has_list_ancestor = true; 
}
...
bool HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::HashBlock::has_data() const
{
    if (is_presence_container) return true;
    return hash_blk.is_set
	|| total.is_set
	|| free.is_set;
}

bool HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::HashBlock::has_operation() const
{
    return is_set(yfilter)
	|| ydk::is_set(hash_blk.yfilter)
	|| ydk::is_set(total.yfilter)
	|| ydk::is_set(free.yfilter);
}

std::string HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::HashBlock::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "hash-block";
    path_buffer << "[" << get_ylist_key() << "]";
    return path_buffer.str();
}

std::vector<std::pair<std::string, LeafData> > HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::HashBlock::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};

    if (hash_blk.is_set || is_set(hash_blk.yfilter)) leaf_name_data.push_back(hash_blk.get_name_leafdata());
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::HashBlock::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::HashBlock::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
void HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::HashBlock::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
    if(value_path == "hash-blk")
    {
        hash_blk = value;
        hash_blk.value_namespace = name_space;
        hash_blk.value_namespace_prefix = name_space_prefix;
    }
    if(value_path == "total")
    {
        total = value;
...
void HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::HashBlock::set_filter(const std::string & value_path, YFilter yfilter)
{
    if(value_path == "hash-blk")
    {
        hash_blk.yfilter = yfilter;
    }
    if(value_path == "total")
    {
...
bool HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::HashBlock::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "hash-blk" || name == "total" || name == "free")
        return true;
    return false;
}

HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::TcamPartition::TcamPartition()
    :
    tcam_par{YType::uint32, "tcam-par"},
    total{YType::uint32, "total"},
    free{YType::uint32, "free"}
{

    yang_name = "tcam-partition"; yang_parent_name = "hw-resource"; is_top_level_class = false; has_list_ancestor = true; 
}
...
bool HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::TcamPartition::has_data() const
{
    if (is_presence_container) return true;
    return tcam_par.is_set
	|| total.is_set
	|| free.is_set;
}

bool HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::TcamPartition::has_operation() const
{
    return is_set(yfilter)
	|| ydk::is_set(tcam_par.yfilter)
	|| ydk::is_set(total.yfilter)
	|| ydk::is_set(free.yfilter);
}

std::string HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::TcamPartition::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "tcam-partition";
    path_buffer << "[" << get_ylist_key() << "]";
    return path_buffer.str();
}

std::vector<std::pair<std::string, LeafData> > HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::TcamPartition::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};

    if (tcam_par.is_set || is_set(tcam_par.yfilter)) leaf_name_data.push_back(tcam_par.get_name_leafdata());
    if (total.is_set || is_set(total.yfilter)) leaf_name_data.push_back(total.get_name_leafdata());
    if (free.is_set || is_set(free.yfilter)) leaf_name_data.push_back(free.get_name_leafdata());
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::TcamPartition::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::TcamPartition::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
void HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::TcamPartition::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
    if(value_path == "tcam-par")
    {
        tcam_par = value;
        tcam_par.value_namespace = name_space;
        tcam_par.value_namespace_prefix = name_space_prefix;
    }
    if(value_path == "total")
    {
        total = value;
...
void HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::TcamPartition::set_filter(const std::string & value_path, YFilter yfilter)
{
    if(value_path == "tcam-par")
    {
        tcam_par.yfilter = yfilter;
    }
    if(value_path == "total")
    {
        total.yfilter = yfilter;
...
bool HardwareModuleNp::Nodes::Node::Nps::Np::L2rmHwResource::HwResource::TcamPartition::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "tcam-par" || name == "total" || name == "free")
        return true;
    return false;
}

HardwareModuleNp::Nodes::Node::Nps::Np::Profile::Profile()
    :
    scale{YType::str, "scale"}
{

    yang_name = "profile"; yang_parent_name = "np"; is_top_level_class = false; has_list_ancestor = true; 
}
...
bool HardwareModuleNp::Nodes::Node::Nps::Np::Profile::has_data() const
{
    if (is_presence_container) return true;
    return scale.is_set;
}

bool HardwareModuleNp::Nodes::Node::Nps::Np::Profile::has_operation() const
{
    return is_set(yfilter)
	|| ydk::is_set(scale.yfilter);
}

std::string HardwareModuleNp::Nodes::Node::Nps::Np::Profile::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "profile";
    return path_buffer.str();
...
std::vector<std::pair<std::string, LeafData> > HardwareModuleNp::Nodes::Node::Nps::Np::Profile::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::Profile::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::Profile::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
void HardwareModuleNp::Nodes::Node::Nps::Np::Profile::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
    if(value_path == "scale")
    {
        scale = value;
...
void HardwareModuleNp::Nodes::Node::Nps::Np::Profile::set_filter(const std::string & value_path, YFilter yfilter)
{
    if(value_path == "scale")
    {
...
bool HardwareModuleNp::Nodes::Node::Nps::Np::Profile::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "scale")
        return true;
    return false;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::Counters::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "np-counter")
    {
        auto ent_ = std::make_shared<HardwareModuleNp::Nodes::Node::Nps::Np::Counters::NpCounter>();
        ent_->parent = this;
        np_counter.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::Counters::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : np_counter.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::Counters::NpCounter::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::Counters::NpCounter::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::FastDrop::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "np-fast-drop")
    {
        auto ent_ = std::make_shared<HardwareModuleNp::Nodes::Node::Nps::Np::FastDrop::NpFastDrop>();
        ent_->parent = this;
        np_fast_drop.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::FastDrop::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : np_fast_drop.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::FastDrop::NpFastDrop::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::FastDrop::NpFastDrop::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
HardwareModuleNp::Nodes::Node::Nps::Np::Efd::Efd()
    :
    hp_list_is_supported{YType::uint32, "hp-list-is-supported"},
    vlan_cos_mapping{YType::uint32, "vlan-cos-mapping"},
    mplscos_mapping{YType::uint32, "mplscos-mapping"},
    ipcos_mapping{YType::uint32, "ipcos-mapping"}
{

    yang_name = "efd"; yang_parent_name = "np"; is_top_level_class = false; has_list_ancestor = true; 
}
...
bool HardwareModuleNp::Nodes::Node::Nps::Np::Efd::has_data() const
{
    if (is_presence_container) return true;
    for (auto const & leaf : vlan_cos_mapping.getYLeafs())
    {
        if(leaf.is_set)
            return true;
    }
    for (auto const & leaf : mplscos_mapping.getYLeafs())
    {
        if(leaf.is_set)
            return true;
    }
    for (auto const & leaf : ipcos_mapping.getYLeafs())
    {
        if(leaf.is_set)
            return true;
    }
    return hp_list_is_supported.is_set;
}

bool HardwareModuleNp::Nodes::Node::Nps::Np::Efd::has_operation() const
{
    for (auto const & leaf : vlan_cos_mapping.getYLeafs())
    {
    ...
    return is_set(yfilter)
	|| ydk::is_set(hp_list_is_supported.yfilter)
	|| ydk::is_set(vlan_cos_mapping.yfilter)
	|| ydk::is_set(mplscos_mapping.yfilter)
	|| ydk::is_set(ipcos_mapping.yfilter);
}

std::string HardwareModuleNp::Nodes::Node::Nps::Np::Efd::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "efd";
    return path_buffer.str();
...
std::vector<std::pair<std::string, LeafData> > HardwareModuleNp::Nodes::Node::Nps::Np::Efd::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};

    if (hp_list_is_supported.is_set || is_set(hp_list_is_supported.yfilter)) leaf_name_data.push_back(hp_list_is_supported.get_name_leafdata());

    auto vlan_cos_mapping_name_datas = vlan_cos_mapping.get_name_leafdata();
    leaf_name_data.insert(leaf_name_data.end(), vlan_cos_mapping_name_datas.begin(), vlan_cos_mapping_name_datas.end());
    auto mplscos_mapping_name_datas = mplscos_mapping.get_name_leafdata();
    leaf_name_data.insert(leaf_name_data.end(), mplscos_mapping_name_datas.begin(), mplscos_mapping_name_datas.end());
    auto ipcos_mapping_name_datas = ipcos_mapping.get_name_leafdata();
    leaf_name_data.insert(leaf_name_data.end(), ipcos_mapping_name_datas.begin(), ipcos_mapping_name_datas.end());
    return leaf_name_data;

}

std::shared_ptr<ydk::Entity> HardwareModuleNp::Nodes::Node::Nps::Np::Efd::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> HardwareModuleNp::Nodes::Node::Nps::Np::Efd::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
void HardwareModuleNp::Nodes::Node::Nps::Np::Efd::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
    if(value_path == "hp-list-is-supported")
    {
        hp_list_is_supported = value;
        hp_list_is_supported.value_namespace = name_space;
        hp_list_is_supported.value_namespace_prefix = name_space_prefix;
    }
    if(value_path == "vlan-cos-mapping")
    {
        vlan_cos_mapping.append(value);
    }
    if(value_path == "mplscos-mapping")
    {
        mplscos_mapping.append(value);
    }
    if(value_path == "ipcos-mapping")
    {
        ipcos_mapping.append(value);
    }
}

void HardwareModuleNp::Nodes::Node::Nps::Np::Efd::set_filter(const std::string & value_path, YFilter yfilter)
{
    if(value_path == "hp-list-is-supported")
    {
        hp_list_is_supported.yfilter = yfilter;
    }
    if(value_path == "vlan-cos-mapping")
    {
        vlan_cos_mapping.yfilter = yfilter;
    }
    if(value_path == "mplscos-mapping")
    {
        mplscos_mapping.yfilter = yfilter;
    }
    if(value_path == "ipcos-mapping")
    {
        ipcos_mapping.yfilter = yfilter;
    }
}

bool HardwareModuleNp::Nodes::Node::Nps::Np::Efd::has_leaf_or_child_of_name(const std::string & name) const
{
    if(name == "hp-list-is-supported" || name == "vlan-cos-mapping" || name == "mplscos-mapping" || name == "ipcos-mapping")
        return true;
    return false;
}
