...
Sr::TrafficEngineering::OutOfResources::OutOfResources()
    :
    maximum_paths_batch{YType::uint32, "maximum-paths-batch"},
...
{
    if (is_presence_container) return true;
    return maximum_paths_batch.is_set
...
{
    return is_set(yfilter)
	|| ydk::is_set(maximum_paths_batch.yfilter)
...
std::vector<std::pair<std::string, LeafData> > Sr::TrafficEngineering::OutOfResources::get_name_leaf_data() const
{
    ...
    if (maximum_paths_batch.is_set || is_set(maximum_paths_batch.yfilter)) leaf_name_data.push_back(maximum_paths_batch.get_name_leafdata());
...
{
    if(value_path == "maximum-paths-batch")
    {
        maximum_paths_batch = value;
        maximum_paths_batch.value_namespace = name_space;
        maximum_paths_batch.value_namespace_prefix = name_space_prefix;
    }
...
void Sr::TrafficEngineering::OutOfResources::set_filter(const std::string & value_path, YFilter yfilter)
{
    if(value_path == "maximum-paths-batch")
    {
        maximum_paths_batch.yfilter = yfilter;
    }
    if(value_path == "maximum-paths")
    {
...
{
    if(name == "maximum-paths-batch" || name == "maximum-paths")
        return true;
    return false;
}
...
Sr::TrafficEngineering::Timers::Timers()
    :
    candidate_path_cleanup_delay{YType::uint32, "candidate-path-cleanup-delay"},
...
{
    if (is_presence_container) return true;
    return candidate_path_cleanup_delay.is_set
...
{
    return is_set(yfilter)
	|| ydk::is_set(candidate_path_cleanup_delay.yfilter)
...
std::vector<std::pair<std::string, LeafData> > Sr::TrafficEngineering::Timers::get_name_leaf_data() const
{
    ...
    if (candidate_path_cleanup_delay.is_set || is_set(candidate_path_cleanup_delay.yfilter)) leaf_name_data.push_back(candidate_path_cleanup_delay.get_name_leafdata());
...
{
    if(value_path == "candidate-path-cleanup-delay")
    {
        candidate_path_cleanup_delay = value;
        candidate_path_cleanup_delay.value_namespace = name_space;
        candidate_path_cleanup_delay.value_namespace_prefix = name_space_prefix;
    }
...
void Sr::TrafficEngineering::Timers::set_filter(const std::string & value_path, YFilter yfilter)
{
    if(value_path == "candidate-path-cleanup-delay")
    {
        candidate_path_cleanup_delay.yfilter = yfilter;
    }
    if(value_path == "cleanup-delay")
    {
...
{
    if(name == "candidate-path-cleanup-delay" || name == "cleanup-delay" || name == "install-delay" || name == "periodic-reoptimization")
        return true;
    return false;
}
