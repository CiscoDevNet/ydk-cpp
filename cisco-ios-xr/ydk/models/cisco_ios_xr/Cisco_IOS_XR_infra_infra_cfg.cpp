...
std::shared_ptr<ydk::Entity> Banners::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "banner")
    {
        auto ent_ = std::make_shared<Banners::Banner>();
        ent_->parent = this;
        banner.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> Banners::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : banner.entities())
    {
...
std::shared_ptr<ydk::Entity> Banners::clone_ptr() const
{
...
std::shared_ptr<ydk::Entity> Banners::Banner::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> Banners::Banner::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
