...
std::map<std::string, std::shared_ptr<Entity>> DciFabricInterconnect::get_children() const
{
    ...
    if(opflex_session_infos != nullptr)
    {
        children["opflex-session-infos"] = opflex_session_infos;
    }

    if(fabric_vrf_dbs != nullptr)
    {
        children["fabric-vrf-dbs"] = fabric_vrf_dbs;
    }

    if(dci_vrfs != nullptr)
    {
        children["dci-vrfs"] = dci_vrfs;
    }
...
std::shared_ptr<Entity> DciFabricInterconnect::OpflexSessionInfos::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "opflex-session-info")
    {
        auto c = std::make_shared<DciFabricInterconnect::OpflexSessionInfos::OpflexSessionInfo>();
        c->parent = this;
        opflex_session_info.append(c);
        return c;
    }
...
std::map<std::string, std::shared_ptr<Entity>> DciFabricInterconnect::OpflexSessionInfos::get_children() const
{
    std::map<std::string, std::shared_ptr<Entity>> children{};
    char count=0;
    count = 0;
    for (auto c : opflex_session_info.entities())
    {
...
std::shared_ptr<Entity> DciFabricInterconnect::OpflexSessionInfos::OpflexSessionInfo::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "peer-info")
    {
        auto c = std::make_shared<DciFabricInterconnect::OpflexSessionInfos::OpflexSessionInfo::PeerInfo>();
        c->parent = this;
        peer_info.append(c);
        return c;
    }
...
std::map<std::string, std::shared_ptr<Entity>> DciFabricInterconnect::OpflexSessionInfos::OpflexSessionInfo::get_children() const
{
    std::map<std::string, std::shared_ptr<Entity>> children{};
    char count=0;
    count = 0;
...
std::shared_ptr<Entity> DciFabricInterconnect::OpflexSessionInfos::OpflexSessionInfo::PeerInfo::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<Entity>> DciFabricInterconnect::OpflexSessionInfos::OpflexSessionInfo::PeerInfo::get_children() const
{
    std::map<std::string, std::shared_ptr<Entity>> children{};
    char count=0;
    return children;
...
std::shared_ptr<Entity> DciFabricInterconnect::FabricVrfDbs::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "fabric-vrf-db")
    {
        auto c = std::make_shared<DciFabricInterconnect::FabricVrfDbs::FabricVrfDb>();
        c->parent = this;
        fabric_vrf_db.append(c);
        return c;
    }
...
std::map<std::string, std::shared_ptr<Entity>> DciFabricInterconnect::FabricVrfDbs::get_children() const
{
    std::map<std::string, std::shared_ptr<Entity>> children{};
    char count=0;
    count = 0;
    for (auto c : fabric_vrf_db.entities())
    {
        if(children.find(c->get_segment_path()) == children.end())
            children[c->get_segment_path()] = c;
        else
...
std::shared_ptr<Entity> DciFabricInterconnect::FabricVrfDbs::FabricVrfDb::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "fabric-vrf")
    {
        auto c = std::make_shared<DciFabricInterconnect::FabricVrfDbs::FabricVrfDb::FabricVrf>();
        c->parent = this;
        fabric_vrf.append(c);
        return c;
    }
...
std::map<std::string, std::shared_ptr<Entity>> DciFabricInterconnect::FabricVrfDbs::FabricVrfDb::get_children() const
{
    std::map<std::string, std::shared_ptr<Entity>> children{};
    char count=0;
    count = 0;
    for (auto c : fabric_vrf.entities())
    {
        if(children.find(c->get_segment_path()) == children.end())
            children[c->get_segment_path()] = c;
        else
...
std::shared_ptr<Entity> DciFabricInterconnect::FabricVrfDbs::FabricVrfDb::FabricVrf::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "v4-import-rt")
    {
        auto c = std::make_shared<DciFabricInterconnect::FabricVrfDbs::FabricVrfDb::FabricVrf::V4ImportRt>();
        c->parent = this;
        v4_import_rt.append(c);
        return c;
    }

    if(child_yang_name == "v4-export-rt")
    {
        auto c = std::make_shared<DciFabricInterconnect::FabricVrfDbs::FabricVrfDb::FabricVrf::V4ExportRt>();
        c->parent = this;
        v4_export_rt.append(c);
        return c;
    }

    if(child_yang_name == "v6-import-rt")
    {
        auto c = std::make_shared<DciFabricInterconnect::FabricVrfDbs::FabricVrfDb::FabricVrf::V6ImportRt>();
        c->parent = this;
        v6_import_rt.append(c);
        return c;
    }

    if(child_yang_name == "v6-export-rt")
    {
        auto c = std::make_shared<DciFabricInterconnect::FabricVrfDbs::FabricVrfDb::FabricVrf::V6ExportRt>();
        c->parent = this;
        v6_export_rt.append(c);
        return c;
    }
...
std::map<std::string, std::shared_ptr<Entity>> DciFabricInterconnect::FabricVrfDbs::FabricVrfDb::FabricVrf::get_children() const
{
    std::map<std::string, std::shared_ptr<Entity>> children{};
    char count=0;
    count = 0;
    for (auto c : v4_import_rt.entities())
    {
        if(children.find(c->get_segment_path()) == children.end())
            children[c->get_segment_path()] = c;
        else
    ...
    count = 0;
    for (auto c : v4_export_rt.entities())
    {
        if(children.find(c->get_segment_path()) == children.end())
            children[c->get_segment_path()] = c;
        else
    ...
    count = 0;
    for (auto c : v6_import_rt.entities())
    {
        if(children.find(c->get_segment_path()) == children.end())
            children[c->get_segment_path()] = c;
        else
    ...
    count = 0;
    for (auto c : v6_export_rt.entities())
    {
        if(children.find(c->get_segment_path()) == children.end())
            children[c->get_segment_path()] = c;
        else
...
std::shared_ptr<Entity> DciFabricInterconnect::FabricVrfDbs::FabricVrfDb::FabricVrf::V4ImportRt::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<Entity>> DciFabricInterconnect::FabricVrfDbs::FabricVrfDb::FabricVrf::V4ImportRt::get_children() const
{
    std::map<std::string, std::shared_ptr<Entity>> children{};
    char count=0;
    return children;
...
std::shared_ptr<Entity> DciFabricInterconnect::FabricVrfDbs::FabricVrfDb::FabricVrf::V4ExportRt::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<Entity>> DciFabricInterconnect::FabricVrfDbs::FabricVrfDb::FabricVrf::V4ExportRt::get_children() const
{
    std::map<std::string, std::shared_ptr<Entity>> children{};
    char count=0;
    return children;
...
std::shared_ptr<Entity> DciFabricInterconnect::FabricVrfDbs::FabricVrfDb::FabricVrf::V6ImportRt::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<Entity>> DciFabricInterconnect::FabricVrfDbs::FabricVrfDb::FabricVrf::V6ImportRt::get_children() const
{
    std::map<std::string, std::shared_ptr<Entity>> children{};
    char count=0;
    return children;
...
std::shared_ptr<Entity> DciFabricInterconnect::FabricVrfDbs::FabricVrfDb::FabricVrf::V6ExportRt::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<Entity>> DciFabricInterconnect::FabricVrfDbs::FabricVrfDb::FabricVrf::V6ExportRt::get_children() const
{
    std::map<std::string, std::shared_ptr<Entity>> children{};
    char count=0;
    return children;
...
std::shared_ptr<Entity> DciFabricInterconnect::DciVrfs::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "dci-vrf")
    {
        auto c = std::make_shared<DciFabricInterconnect::DciVrfs::DciVrf>();
        c->parent = this;
        dci_vrf.append(c);
        return c;
    }
...
std::map<std::string, std::shared_ptr<Entity>> DciFabricInterconnect::DciVrfs::get_children() const
{
    std::map<std::string, std::shared_ptr<Entity>> children{};
    char count=0;
    count = 0;
    for (auto c : dci_vrf.entities())
    {
        if(children.find(c->get_segment_path()) == children.end())
            children[c->get_segment_path()] = c;
        else
...
std::shared_ptr<Entity> DciFabricInterconnect::DciVrfs::DciVrf::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "v4-import-rt")
    {
        auto c = std::make_shared<DciFabricInterconnect::DciVrfs::DciVrf::V4ImportRt>();
        c->parent = this;
        v4_import_rt.append(c);
        return c;
    }
    ...
    if(child_yang_name == "v4-export-rt")
    {
        auto c = std::make_shared<DciFabricInterconnect::DciVrfs::DciVrf::V4ExportRt>();
        c->parent = this;
        v4_export_rt.append(c);
        return c;
    }
    ...
    if(child_yang_name == "v6-import-rt")
    {
        auto c = std::make_shared<DciFabricInterconnect::DciVrfs::DciVrf::V6ImportRt>();
        c->parent = this;
        v6_import_rt.append(c);
        return c;
    }
    ...
    if(child_yang_name == "v6-export-rt")
    {
        auto c = std::make_shared<DciFabricInterconnect::DciVrfs::DciVrf::V6ExportRt>();
        c->parent = this;
        v6_export_rt.append(c);
        return c;
    }
...
std::map<std::string, std::shared_ptr<Entity>> DciFabricInterconnect::DciVrfs::DciVrf::get_children() const
{
    std::map<std::string, std::shared_ptr<Entity>> children{};
    char count=0;
    count = 0;
...
std::shared_ptr<Entity> DciFabricInterconnect::DciVrfs::DciVrf::V4ImportRt::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<Entity>> DciFabricInterconnect::DciVrfs::DciVrf::V4ImportRt::get_children() const
{
    std::map<std::string, std::shared_ptr<Entity>> children{};
    char count=0;
    return children;
...
std::shared_ptr<Entity> DciFabricInterconnect::DciVrfs::DciVrf::V4ExportRt::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<Entity>> DciFabricInterconnect::DciVrfs::DciVrf::V4ExportRt::get_children() const
{
    std::map<std::string, std::shared_ptr<Entity>> children{};
    char count=0;
    return children;
...
std::shared_ptr<Entity> DciFabricInterconnect::DciVrfs::DciVrf::V6ImportRt::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<Entity>> DciFabricInterconnect::DciVrfs::DciVrf::V6ImportRt::get_children() const
{
    std::map<std::string, std::shared_ptr<Entity>> children{};
    char count=0;
    return children;
...
std::shared_ptr<Entity> DciFabricInterconnect::DciVrfs::DciVrf::V6ExportRt::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<Entity>> DciFabricInterconnect::DciVrfs::DciVrf::V6ExportRt::get_children() const
{
    std::map<std::string, std::shared_ptr<Entity>> children{};
    char count=0;
    return children;
