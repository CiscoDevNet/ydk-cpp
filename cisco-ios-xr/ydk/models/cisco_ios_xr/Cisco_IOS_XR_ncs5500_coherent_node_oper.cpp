...
{
    if(name == "coherent-time-stats" || name == "coherenthealth" || name == "node-name")
        return true;
    return false;
}
...
{
    if (is_presence_container) return true;
    for (std::size_t index=0; index<port_stat.len(); index++)
    {
...
std::vector<std::pair<std::string, LeafData> > Coherent::Nodes::Node::CoherentTimeStats::PortStat::get_name_leaf_data() const
{
    ...
    if (rsip.is_set || is_set(rsip.yfilter)) leaf_name_data.push_back(rsip.get_name_leafdata());
...
void Coherent::Nodes::Node::CoherentTimeStats::PortStat::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
    ...
    {
        rsip = value;
        rsip.value_namespace = name_space;
        rsip.value_namespace_prefix = name_space_prefix;
    }
...
{
    if(name == "laser-on-stats" || name == "laser-off-stats" || name == "wl-op-stats" || name == "txpwr-op-stats" || name == "cdmin-op-stats" || name == "cdmax-op-stats" || name == "traffictype-op-stats" || name == "rsip" || name == "fp-port-idx" || name == "laser-state" || name == "provisioned-frequency" || name == "tx-power" || name == "cd-min" || name == "cd-max" || name == "traffic-type")
        return true;
    return false;
}
...
Coherent::Nodes::Node::Coherenthealth::Coherenthealth()
    :
    is_peyto{YType::boolean, "is-peyto"},
    ...
    dsp0_version{YType::str, "dsp0-version"},
    dsp1_version{YType::str, "dsp1-version"},
    dsp2_version{YType::str, "dsp2-version"},
...
{
    if (is_presence_container) return true;
    for (std::size_t index=0; index<port_data.len(); index++)
    {
        if(port_data[index]->has_data())
            return true;
    }
    return is_peyto.is_set
	|| im_state.is_set
...
bool Coherent::Nodes::Node::Coherenthealth::has_operation() const
{
	...
	|| ydk::is_set(dsp0_version.yfilter)
	|| ydk::is_set(dsp1_version.yfilter)
	|| ydk::is_set(dsp2_version.yfilter)
...
std::vector<std::pair<std::string, LeafData> > Coherent::Nodes::Node::Coherenthealth::get_name_leaf_data() const
{
    ...
    if (is_peyto.is_set || is_set(is_peyto.yfilter)) leaf_name_data.push_back(is_peyto.get_name_leafdata());
    ...
    if (dsp0_version.is_set || is_set(dsp0_version.yfilter)) leaf_name_data.push_back(dsp0_version.get_name_leafdata());
    if (dsp1_version.is_set || is_set(dsp1_version.yfilter)) leaf_name_data.push_back(dsp1_version.get_name_leafdata());
    if (dsp2_version.is_set || is_set(dsp2_version.yfilter)) leaf_name_data.push_back(dsp2_version.get_name_leafdata());
...
{
    if(value_path == "is-peyto")
    {
        is_peyto = value;
        is_peyto.value_namespace = name_space;
        is_peyto.value_namespace_prefix = name_space_prefix;
    }
    ...
    if(value_path == "dsp0-version")
    {
        dsp0_version = value;
        dsp0_version.value_namespace = name_space;
        dsp0_version.value_namespace_prefix = name_space_prefix;
    }
    if(value_path == "dsp1-version")
    {
        dsp1_version = value;
        dsp1_version.value_namespace = name_space;
        dsp1_version.value_namespace_prefix = name_space_prefix;
    }
    if(value_path == "dsp2-version")
    {
        dsp2_version = value;
        dsp2_version.value_namespace = name_space;
        dsp2_version.value_namespace_prefix = name_space_prefix;
    }
...
void Coherent::Nodes::Node::Coherenthealth::set_filter(const std::string & value_path, YFilter yfilter)
{
    ...
    if(value_path == "dsp0-version")
    {
        dsp0_version.yfilter = yfilter;
    }
    if(value_path == "dsp1-version")
    {
        dsp1_version.yfilter = yfilter;
    }
    if(value_path == "dsp2-version")
    {
        dsp2_version.yfilter = yfilter;
    }
    if(value_path == "board-type")
    {
...
{
    if(name == "port-data" || name == "is-peyto" || name == "im-state" || name == "aipc-srvr-state" || name == "sysdb-state" || name == "pm-state" || name == "optics-ea-conn" || name == "dsp-ea-conn" || name == "vether-state" || name == "morgoth-alive" || name == "prov-infra-state" || name == "sdk-fpga-compatible" || name == "pending-provision" || name == "pulse-sent" || name == "inside-prov-loop" || name == "fpd-in-progress" || name == "prov-run-count" || name == "sdk-version" || name == "morgoth-running-version" || name == "morgoth-downloaded-version" || name == "morgoth-golden-version" || name == "dsp0-version" || name == "dsp1-version" || name == "dsp2-version" || name == "board-type" || name == "jlink-op")
        return true;
    return false;
}
...
Coherent::Nodes::Node::Coherenthealth::PortData::PortData()
    ...
    end_of_config{YType::boolean, "end-of-config"},
    optics_laser_state{YType::boolean, "optics-laser-state"},
    laser_state{YType::boolean, "laser-state"},
    ...
    dsp_hw_alarms{YType::str, "dsp-hw-alarms"},
...
bool Coherent::Nodes::Node::Coherenthealth::PortData::has_data() const
{
	...
	|| dsp_admin_up.is_set
	|| end_of_config.is_set
	|| optics_laser_state.is_set
	|| laser_state.is_set
...
bool Coherent::Nodes::Node::Coherenthealth::PortData::has_operation() const
{
	...
	|| ydk::is_set(end_of_config.yfilter)
	|| ydk::is_set(optics_laser_state.yfilter)
	|| ydk::is_set(laser_state.yfilter)
	...
	|| ydk::is_set(dsp_hw_alarms.yfilter)
...
std::vector<std::pair<std::string, LeafData> > Coherent::Nodes::Node::Coherenthealth::PortData::get_name_leaf_data() const
{
    ...
    if (end_of_config.is_set || is_set(end_of_config.yfilter)) leaf_name_data.push_back(end_of_config.get_name_leafdata());
    if (optics_laser_state.is_set || is_set(optics_laser_state.yfilter)) leaf_name_data.push_back(optics_laser_state.get_name_leafdata());
    if (laser_state.is_set || is_set(laser_state.yfilter)) leaf_name_data.push_back(laser_state.get_name_leafdata());
    ...
    if (dsp_hw_alarms.is_set || is_set(dsp_hw_alarms.yfilter)) leaf_name_data.push_back(dsp_hw_alarms.get_name_leafdata());
...
void Coherent::Nodes::Node::Coherenthealth::PortData::set_value(const std::string & value_path, const std::string & value, const std::string & name_space, const std::string & name_space_prefix)
{
    ...
    if(value_path == "end-of-config")
    {
        end_of_config = value;
        end_of_config.value_namespace = name_space;
        end_of_config.value_namespace_prefix = name_space_prefix;
    }
    if(value_path == "optics-laser-state")
    {
        optics_laser_state = value;
        optics_laser_state.value_namespace = name_space;
        optics_laser_state.value_namespace_prefix = name_space_prefix;
    }
    if(value_path == "laser-state")
    {
        laser_state = value;
    ...
    if(value_path == "dsp-hw-alarms")
    {
        dsp_hw_alarms = value;
        dsp_hw_alarms.value_namespace = name_space;
        dsp_hw_alarms.value_namespace_prefix = name_space_prefix;
    }
...
void Coherent::Nodes::Node::Coherenthealth::PortData::set_filter(const std::string & value_path, YFilter yfilter)
{
    ...
    if(value_path == "end-of-config")
    {
        end_of_config.yfilter = yfilter;
    }
    if(value_path == "optics-laser-state")
    {
        optics_laser_state.yfilter = yfilter;
    }
    if(value_path == "laser-state")
    {
        laser_state.yfilter = yfilter;
...
Coherent::Nodes::Node::Coherenthealth::PortData::CtpInfo::CtpInfo()
    ...
    pid{YType::str, "pid"},
    vid{YType::str, "vid"},
    module_hardware_version_number{YType::str, "module-hardware-version-number"},
    module_firmware_running_version_number{YType::str, "module-firmware-running-version-number"},
    module_firmware_committed_version_number{YType::str, "module-firmware-committed-version-number"},
...
{
    if (is_presence_container) return true;
    for (std::size_t index=0; index<eth_data.len(); index++)
    {
