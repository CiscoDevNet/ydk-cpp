...
std::shared_ptr<Entity> HardwareModule::Node::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    ...
    if(child_yang_name == "slice")
    {
        ...
        children[segment_path] = slice.back();
        return children.at(segment_path);
    }
...
std::map<std::string, std::shared_ptr<Entity>> & HardwareModule::Node::Slice::get_children()
{
...
std::map<std::string, std::shared_ptr<Entity>> & HardwareModule::Node::Slice::Values::get_children()
{
    return children;
}
