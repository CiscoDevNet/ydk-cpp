...
std::map<std::string, std::shared_ptr<Entity>> HardwareModule::Node::Slice::get_children() const
{
    ...
    if(client_ains != nullptr)
    {
        children["client-ains"] = client_ains;
    }
...
std::shared_ptr<Entity> HardwareModule::Node::Slice::ClientAins::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<Entity>> HardwareModule::Node::Slice::ClientAins::get_children() const
{
    std::map<std::string, std::shared_ptr<Entity>> children{};
    char count=0;
    return children;
