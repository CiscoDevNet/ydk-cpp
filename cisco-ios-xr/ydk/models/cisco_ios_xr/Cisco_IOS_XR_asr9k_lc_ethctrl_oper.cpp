...
std::shared_ptr<ydk::Entity> Mlan::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::get_children() const
{
...
std::shared_ptr<ydk::Entity> Mlan::clone_ptr() const
{
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "node")
    {
        auto ent_ = std::make_shared<Mlan::Nodes::Node>();
        ent_->parent = this;
        node.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::Node::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::Node::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(port_status_numbers != nullptr)
    {
        _children["port-status-numbers"] = port_status_numbers;
    }

    if(switch_status_table != nullptr)
    {
        _children["switch-status-table"] = switch_status_table;
    }

    if(port_counters_numbers != nullptr)
    {
        _children["port-counters-numbers"] = port_counters_numbers;
    }

    if(atu_entry_numbers != nullptr)
    {
        _children["atu-entry-numbers"] = atu_entry_numbers;
    }
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::Node::PortStatusNumbers::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "port-status-number")
    {
        auto ent_ = std::make_shared<Mlan::Nodes::Node::PortStatusNumbers::PortStatusNumber>();
        ent_->parent = this;
        port_status_number.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::Node::PortStatusNumbers::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : port_status_number.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::Node::PortStatusNumbers::PortStatusNumber::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::Node::PortStatusNumbers::PortStatusNumber::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(port_status != nullptr)
    {
        _children["port-status"] = port_status;
    }
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::Node::PortStatusNumbers::PortStatusNumber::PortStatus::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::Node::PortStatusNumbers::PortStatusNumber::PortStatus::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(config != nullptr)
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::Node::PortStatusNumbers::PortStatusNumber::PortStatus::Config::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::Node::PortStatusNumbers::PortStatusNumber::PortStatus::Config::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::Node::PortStatusNumbers::PortStatusNumber::PortStatus::Phy::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "reg")
    {
        auto ent_ = std::make_shared<Mlan::Nodes::Node::PortStatusNumbers::PortStatusNumber::PortStatus::Phy::Reg>();
        ent_->parent = this;
        reg.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::Node::PortStatusNumbers::PortStatusNumber::PortStatus::Phy::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : reg.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::Node::PortStatusNumbers::PortStatusNumber::PortStatus::Phy::Reg::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::Node::PortStatusNumbers::PortStatusNumber::PortStatus::Phy::Reg::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::Node::PortStatusNumbers::PortStatusNumber::PortStatus::Serdes::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "reg")
    {
        auto ent_ = std::make_shared<Mlan::Nodes::Node::PortStatusNumbers::PortStatusNumber::PortStatus::Serdes::Reg>();
        ent_->parent = this;
        reg.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::Node::PortStatusNumbers::PortStatusNumber::PortStatus::Serdes::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::Node::PortStatusNumbers::PortStatusNumber::PortStatus::Serdes::Reg::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::Node::PortStatusNumbers::PortStatusNumber::PortStatus::Serdes::Reg::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::Node::PortStatusNumbers::PortStatusNumber::PortStatus::Mac::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "reg")
    {
        auto ent_ = std::make_shared<Mlan::Nodes::Node::PortStatusNumbers::PortStatusNumber::PortStatus::Mac::Reg>();
        ent_->parent = this;
        reg.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::Node::PortStatusNumbers::PortStatusNumber::PortStatus::Mac::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::Node::PortStatusNumbers::PortStatusNumber::PortStatus::Mac::Reg::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::Node::PortStatusNumbers::PortStatusNumber::PortStatus::Mac::Reg::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::Node::SwitchStatusTable::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::Node::SwitchStatusTable::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(switch_status != nullptr)
    {
        _children["switch-status"] = switch_status;
    }
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::Node::SwitchStatusTable::SwitchStatus::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::Node::SwitchStatusTable::SwitchStatus::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(sw_reg_1 != nullptr)
    {
        _children["sw-reg-1"] = sw_reg_1;
    }

    if(sw_reg_2 != nullptr)
    {
        _children["sw-reg-2"] = sw_reg_2;
    }

    if(sw_status != nullptr)
    {
        _children["sw-status"] = sw_status;
    }
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::Node::SwitchStatusTable::SwitchStatus::SwReg1::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "reg")
    {
        auto ent_ = std::make_shared<Mlan::Nodes::Node::SwitchStatusTable::SwitchStatus::SwReg1::Reg>();
        ent_->parent = this;
        reg.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::Node::SwitchStatusTable::SwitchStatus::SwReg1::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::Node::SwitchStatusTable::SwitchStatus::SwReg1::Reg::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::Node::SwitchStatusTable::SwitchStatus::SwReg1::Reg::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::Node::SwitchStatusTable::SwitchStatus::SwReg2::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "reg")
    {
        auto ent_ = std::make_shared<Mlan::Nodes::Node::SwitchStatusTable::SwitchStatus::SwReg2::Reg>();
        ent_->parent = this;
        reg.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::Node::SwitchStatusTable::SwitchStatus::SwReg2::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::Node::SwitchStatusTable::SwitchStatus::SwReg2::Reg::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::Node::SwitchStatusTable::SwitchStatus::SwReg2::Reg::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::Node::SwitchStatusTable::SwitchStatus::SwStatus::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::Node::SwitchStatusTable::SwitchStatus::SwStatus::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::Node::PortCountersNumbers::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "port-counters-number")
    {
        auto ent_ = std::make_shared<Mlan::Nodes::Node::PortCountersNumbers::PortCountersNumber>();
        ent_->parent = this;
        port_counters_number.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::Node::PortCountersNumbers::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : port_counters_number.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::Node::PortCountersNumbers::PortCountersNumber::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::Node::PortCountersNumbers::PortCountersNumber::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(port_counters != nullptr)
    {
        _children["port-counters"] = port_counters;
    }
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::Node::PortCountersNumbers::PortCountersNumber::PortCounters::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::Node::PortCountersNumbers::PortCountersNumber::PortCounters::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(mlan_stats != nullptr)
    {
        _children["mlan-stats"] = mlan_stats;
    }
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::Node::PortCountersNumbers::PortCountersNumber::PortCounters::MlanStats::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::Node::PortCountersNumbers::PortCountersNumber::PortCounters::MlanStats::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::Node::AtuEntryNumbers::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "atu-entry-number")
    {
        auto ent_ = std::make_shared<Mlan::Nodes::Node::AtuEntryNumbers::AtuEntryNumber>();
        ent_->parent = this;
        atu_entry_number.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::Node::AtuEntryNumbers::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : atu_entry_number.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::Node::AtuEntryNumbers::AtuEntryNumber::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::Node::AtuEntryNumbers::AtuEntryNumber::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(switch_counters != nullptr)
    {
        _children["switch-counters"] = switch_counters;
    }
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::Node::AtuEntryNumbers::AtuEntryNumber::SwitchCounters::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::Node::AtuEntryNumbers::AtuEntryNumber::SwitchCounters::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    if(atu != nullptr)
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::Node::AtuEntryNumbers::AtuEntryNumber::SwitchCounters::Atu::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(child_yang_name == "macaddr")
    {
        auto ent_ = std::make_shared<Mlan::Nodes::Node::AtuEntryNumbers::AtuEntryNumber::SwitchCounters::Atu::Macaddr>();
        ent_->parent = this;
        macaddr.append(ent_);
        return ent_;
    }
...
std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::Node::AtuEntryNumbers::AtuEntryNumber::SwitchCounters::Atu::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    count_ = 0;
    for (auto ent_ : macaddr.entities())
    {
        if(_children.find(ent_->get_segment_path()) == _children.end())
            _children[ent_->get_segment_path()] = ent_;
        else
...
std::shared_ptr<ydk::Entity> Mlan::Nodes::Node::AtuEntryNumbers::AtuEntryNumber::SwitchCounters::Atu::Macaddr::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    return nullptr;
}

std::map<std::string, std::shared_ptr<ydk::Entity>> Mlan::Nodes::Node::AtuEntryNumbers::AtuEntryNumber::SwitchCounters::Atu::Macaddr::get_children() const
{
    std::map<std::string, std::shared_ptr<ydk::Entity>> _children{};
    char count_=0;
    return _children;
