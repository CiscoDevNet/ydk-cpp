...
{
    if(name == "power-supply-shelfs" || name == "slots" || name == "fan-traies" || name == "power-supply-zones" || name == "basic-attributes" || name == "number")
        return true;
    return false;
}
...
std::map<std::string, std::shared_ptr<Entity>> Inventory::Racks::Rack::PowerSupplyShelfs::get_children() const
{
    ...
    char count=0;
    count = 0;
    for (auto c : power_supply_shelf.entities())
    {
        if(children.find(c->get_segment_path()) == children.end())
            children[c->get_segment_path()] = c;
        else
...
std::vector<std::pair<std::string, LeafData> > Inventory::Racks::Rack::FanTraies::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};
...
std::vector<std::pair<std::string, LeafData> > Inventory::Racks::Rack::FanTraies::FanTray::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};
...
std::vector<std::pair<std::string, LeafData> > Inventory::Racks::Rack::FanTraies::FanTray::BasicAttributes::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};
...
std::vector<std::pair<std::string, LeafData> > Inventory::Racks::Rack::FanTraies::FanTray::BasicAttributes::BasicInfo::get_name_leaf_data() const
{
    std::vector<std::pair<std::string, LeafData> > leaf_name_data {};
...
std::map<std::string, std::shared_ptr<Entity>> Inventory::Racks::Rack::PowerSupplyZones::get_children() const
{
    ...
    char count=0;
    count = 0;
    for (auto c : power_supply_zone.entities())
    {
        if(children.find(c->get_segment_path()) == children.end())
            children[c->get_segment_path()] = c;
        else
