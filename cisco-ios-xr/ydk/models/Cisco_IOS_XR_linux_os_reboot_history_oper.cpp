...
Entity* RebootHistory::Node::RebootHistory_::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, Entity*> & RebootHistory::Node::RebootHistory_::get_children()
{
    return children;
}
...
Entity* RebootHistory::Node::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "reboot-history")
    {
        ...
        auto c = std::make_unique<RebootHistory::Node::RebootHistory_>();
        c->parent = this;
        reboot_history.push_back(std::move(c));
        children[segment_path] = reboot_history.back().get();
        return children.at(segment_path);
    }
...
std::map<std::string, Entity*> & RebootHistory::Node::get_children()
{
    for (auto const & c : reboot_history)
    {
...
Entity* RebootHistory::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "node")
    {
        ...
        auto c = std::make_unique<RebootHistory::Node>();
        c->parent = this;
...
std::map<std::string, Entity*> & RebootHistory::get_children()
{
    for (auto const & c : node)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
std::unique_ptr<Entity> RebootHistory::clone_ptr()
{
    return std::make_unique<RebootHistory>();
}
