...
std::string ProcessesMemory::Nodes::Node::JobIds::JobId::get_segment_path() const
{
    ...
    path_buffer << "job-id" <<"[job-id='" <<job_id.get() <<"']";
...
Entity* ProcessesMemory::Nodes::Node::JobIds::JobId::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, Entity*> & ProcessesMemory::Nodes::Node::JobIds::JobId::get_children()
{
    return children;
}
...
Entity* ProcessesMemory::Nodes::Node::JobIds::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "job-id")
    {
        ...
        auto c = std::make_unique<ProcessesMemory::Nodes::Node::JobIds::JobId>();
        c->parent = this;
        job_id.push_back(std::move(c));
        children[segment_path] = job_id.back().get();
        return children.at(segment_path);
    }
...
std::map<std::string, Entity*> & ProcessesMemory::Nodes::Node::JobIds::get_children()
{
    for (auto const & c : job_id)
    {
...
ProcessesMemory::Nodes::Node::Node()
    :
    	...
    	,
    job_ids(std::make_unique<ProcessesMemory::Nodes::Node::JobIds>())
{
    job_ids->parent = this;
    children["job-ids"] = job_ids.get();
...
bool ProcessesMemory::Nodes::Node::has_operation() const
{
	...
	|| (job_ids !=  nullptr && is_set(job_ids->operation));
}
