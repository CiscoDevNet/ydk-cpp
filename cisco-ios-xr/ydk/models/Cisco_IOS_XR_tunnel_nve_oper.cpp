...
Entity* Nve::Vnis::Vni::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, Entity*> & Nve::Vnis::Vni::get_children()
{
    return children;
}
...
Entity* Nve::Vnis::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "vni")
    {
        ...
        auto c = std::make_unique<Nve::Vnis::Vni>();
        c->parent = this;
        vni.push_back(std::move(c));
        children[segment_path] = vni.back().get();
        return children.at(segment_path);
    }
...
std::map<std::string, Entity*> & Nve::Vnis::get_children()
{
    for (auto const & c : vni)
    {
...
Entity* Nve::Interfaces::Interface::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & Nve::Interfaces::Interface::get_children()
{
    return children;
}
...
Entity* Nve::Interfaces::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "interface")
    {
        ...
        auto c = std::make_unique<Nve::Interfaces::Interface>();
        c->parent = this;
...
std::map<std::string, Entity*> & Nve::Interfaces::get_children()
{
    for (auto const & c : interface)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
Nve::Nve()
    :
    interfaces(std::make_unique<Nve::Interfaces>())
	,vnis(std::make_unique<Nve::Vnis>())
{
    interfaces->parent = this;
    children["interfaces"] = interfaces.get();

    vnis->parent = this;
    children["vnis"] = vnis.get();
...
Entity* Nve::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & Nve::get_children()
{
...
std::unique_ptr<Entity> Nve::clone_ptr()
{
    return std::make_unique<Nve>();
}
