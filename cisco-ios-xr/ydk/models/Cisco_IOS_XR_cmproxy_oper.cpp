...
std::string SdrInventoryVm::Nodes::Node::NodeEntries::NodeEntry::get_segment_path() const
{
    ...
    path_buffer << "node-entry" <<"[name='" <<name.get() <<"']";
...
Entity* SdrInventoryVm::Nodes::Node::NodeEntries::NodeEntry::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
...
std::map<std::string, Entity*> & SdrInventoryVm::Nodes::Node::NodeEntries::NodeEntry::get_children()
{
    return children;
}
...
Entity* SdrInventoryVm::Nodes::Node::NodeEntries::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "node-entry")
    {
        ...
        auto c = std::make_unique<SdrInventoryVm::Nodes::Node::NodeEntries::NodeEntry>();
        c->parent = this;
        node_entry.push_back(std::move(c));
        children[segment_path] = node_entry.back().get();
        return children.at(segment_path);
    }
...
std::map<std::string, Entity*> & SdrInventoryVm::Nodes::Node::NodeEntries::get_children()
{
    for (auto const & c : node_entry)
    {
...
SdrInventoryVm::Nodes::Node::Node()
    :
    	name{YType::str, "name"}
    	,
    node_entries(std::make_unique<SdrInventoryVm::Nodes::Node::NodeEntries>())
{
    node_entries->parent = this;
    children["node-entries"] = node_entries.get();
...
bool SdrInventoryVm::Nodes::Node::has_operation() const
{
    return is_set(operation)
	|| is_set(name.operation)
	|| (node_entries !=  nullptr && is_set(node_entries->operation));
}

std::string SdrInventoryVm::Nodes::Node::get_segment_path() const
{
    std::ostringstream path_buffer;
    path_buffer << "node" <<"[name='" <<name.get() <<"']";
...
Entity* SdrInventoryVm::Nodes::Node::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & SdrInventoryVm::Nodes::Node::get_children()
{
...
Entity* SdrInventoryVm::Nodes::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
    ...
    if(child_yang_name == "node")
    {
        ...
        auto c = std::make_unique<SdrInventoryVm::Nodes::Node>();
        c->parent = this;
...
std::map<std::string, Entity*> & SdrInventoryVm::Nodes::get_children()
{
    for (auto const & c : node)
    {
        if(children.find(c->get_segment_path()) == children.end())
...
SdrInventoryVm::SdrInventoryVm()
    :
    nodes(std::make_unique<SdrInventoryVm::Nodes>())
{
    nodes->parent = this;
...
Entity* SdrInventoryVm::get_child_by_name(const std::string & child_yang_name, const std::string & segment_path)
{
    if(children.find(child_yang_name) != children.end())
    {
        return children.at(child_yang_name);
...
std::map<std::string, Entity*> & SdrInventoryVm::get_children()
{
...
std::unique_ptr<Entity> SdrInventoryVm::clone_ptr()
{
    return std::make_unique<SdrInventoryVm>();
}
